...
export class PetDragon23ProjectileSystem {
  ...
  private waves: Wave[] = [];
  constructor(private readonly form: 2 | 3) {}
  ...
  emit(context: PetBehaviorContext, action: Action = 'normal'): void {
    ...
    if (action === 'normal') root.x += root.facingX * 30;
  ...
  private spawn(context: PetBehaviorContext, action: Action, root: Root): void {
    ...
    const symbol = action === 'normal' ? `PetDragon${this.form}Bullet1`
      : action === 'sdcc' ? 'PetDragon2Bullet2' : 'PetDragon3Bullet3';
    ...
    context.castSkill(({ roster, projectiles }) => {
      const projectile = spawnProjectileFromTuning(projectiles, { sourceId: context.pet.id, ...root,
        ...
        `dragon${this.form}-${action}`, {
          actionName: action === 'normal' ? 'hit1' : action === 'sdcc' ? 'hit2' : 'hit3',
          ...
          damage: cache.hurt, attackKind: action === 'ltwj' ? 'magic' : 'physics',
          knockbackX: action === 'normal' ? 3 : action === 'sdcc' ? 7 : 2, knockbackY: -5,
      ...
      projectile.destroyWhenSourceHurt = action === 'sdcc';
  ...
  step(context: PetBehaviorContext): void {
    for (const entry of this.bullets) {
      ...
      if (frame === asset.frameCount || (entry.action === 'sdcc' && context.animation?.action === 'hurt')) p.isExpired = true;
      // FollowBaseObjectBullet.step2 follows only after collision and last-frame/hurt cleanup.
      if (!p.isExpired && entry.action === 'sdcc') {
  ...
  private refresh(context: PetBehaviorContext, action: Action): DragonDamageCache {
    const pet = context.pet;
    const power = action === 'normal' ? pet.atk : action === 'sdcc'

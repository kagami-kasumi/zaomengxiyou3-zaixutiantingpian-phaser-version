...
export function showStage13Result(
  scene: Phaser.Scene,
  ...
  const subtitle = scene.add.text(470, 244, 'Stage 1-3 · 南天门', {
    color: '#f3f6ff', fontFamily: 'Arial, sans-serif', fontSize: '22px',
  }).setOrigin(0.5).setScrollFactor(0);
  const detailCopy = isClear
    ? saved ? '已解锁并保存 2-1' : '已解锁 2-1；当前浏览器尚无可更新存档'
    : '全员倒地 2.5 秒，关卡进度未推进';
  const detail = scene.add.text(470, 286, detailCopy, {
    color: '#9ed7b5', fontFamily: 'Arial, sans-serif', fontSize: '16px',
  }).setOrigin(0.5).setScrollFactor(0);
  const retry = createButton(scene, 350, 382, '重玩 1-3', () => scene.scene.restart({ playerCount }));
  const back = createButton(scene, 590, 382, '返回关卡入口', () => scene.scene.start('Stage11EntryScene'));

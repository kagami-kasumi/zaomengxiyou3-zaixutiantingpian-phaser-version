# 权利与来源声明

Copyright (c) 2026 kagami-kasumi. All rights reserved.

## 授权性质

本仓库采用自定义的《再续天庭现代化重写有限使用许可证》。项目源码可见，但不是
开放源代码或自由软件。完整且唯一有效的授权条件见根目录 [`LICENSE`](LICENSE)。

除许可证明确允许的个人非商业本地运行，以及为向本仓库提交 Pull Request 而进行的
GitHub Fork 和修改外，著作权人未授予独立二次开发、分发、部署、托管、再许可或
商业使用权。

## 权利范围

许可证只覆盖 kagami-kasumi 原创且有权许可的现代重写代码、构建脚本、文档和原创材料。

《造梦西游 3》《再续天庭》及相关名称、角色、剧情、美术、音频、字体、数据、Flash
资源、反编译资料和其他原游戏内容的权利归各自权利人所有。本项目是非官方的学习与
技术研究项目，与原游戏权利人不存在隶属、认可或赞助关系。

仓库中的第三方 npm 依赖继续适用其各自许可证；本项目许可证不会替代或扩大这些许可。
本地且被 Git 忽略的 `local-resources/` 研究资料不因本仓库的许可证而获得任何新授权。

## 联系与额外授权

需要额外许可时，请通过本仓库联系著作权人：

https://github.com/kagami-kasumi/zaomengxiyou3-zaixutiantingpian-phaser-version

除非著作权人另行作出明确书面授权，Issue、Discussion、Pull Request 评论、代码合入或
沉默均不构成额外许可。

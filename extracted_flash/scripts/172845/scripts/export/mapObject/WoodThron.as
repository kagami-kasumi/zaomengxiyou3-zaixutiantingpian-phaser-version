package export.mapObject
{
   ...
   public class WoodThron extends MovieClip
   {
      
      public var isWoodThron:MovieClip;
      ...
      public function step() : void
      {
         ...
         if(this.currentFrame == 1)
         {
            ...
            if(this.gc.hero1)
            {
               if(Math.abs(this.x - this.gc.hero1.x) <= 350)
               {
                  _loc2_ = true;
               }
            }
            if(!_loc2_)
            {
               if(this.gc.hero2)
               {
                  if(Math.abs(this.x - this.gc.hero2.x) <= 350)
                  {
                     _loc2_ = true;
                  }
               }
         ...
         for each(_loc1_ in this.gc.getPlayerArray())
         {
            if(_loc1_.beAttackIdArray.indexOf(this.getAttackId()) == -1)
            {
               if(!_loc1_.isYourFather())
               {
                  ...
                  {
                     _loc1_.reduceHp(45 + (Math.random() - 0.5) * 10,true);

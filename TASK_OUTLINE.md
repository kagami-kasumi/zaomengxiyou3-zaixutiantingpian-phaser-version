...
用户使用 `/goal` 时，先读取 `execution-queue.md`。治理执行项存在时只处理该项，完成后移出队列、回写 PG/治理日志并停止；不得在同一次 `/goal` 继续游戏。队列为空时，AI 才持有 `feature-lines.md` 中唯一 Active 功能线并执行 `task-board.md` 中唯一 Ready 游戏 task。游戏 task 完成后激活同线下一 task并交接，不在同一次 `/goal` 中继续跨 task。遇到阻塞只解决当前调度范围内的阻塞；compact 次数不作为停止、拆分或新开对话条件；恢复后窄读当前合同、改动和必要证据，继续原 task，不借此扩张范围。仅在存在尚未保存且影响恢复的信息或需要实际交接时更新简短检查点；已有记录足够时直接复用，不重复计数、落盘或运行无变化的检查。

同一个正式游戏 task 未完成时优先继续当前对话；compact 后按 agent-protocol 窄读恢复；不按次数强制交接，只按必要的新信息更新检查点。只有完成 task、切换明显不同机制/切片/子系统，或已读取大量 AS3/逆向/历史资料时，才在文档收尾后建议新开对话。
...
- 2026-08-17 用户实际复验反证了上述部分关闭结论：Stage 2-3 执行再次后移，先完成装备悬停数值、宠物页/战斗 UI 与真动画、五角色显示与动作流畅度同源审计/整改、战斗技能 HUD 可见与原生化。功能页已有证据不能代替正式路径的实际可见性，旧“五角色真动画已闭合”结论在跨角色帧时序、持帧、解包完整性与运行时投影重新核对前降级为待复核。
...
- 装备格与已穿戴槽的悬停数值，宠物功能页/战斗 HUD/战斗实体，以及战斗技能栏必须各自有可见、可交互、可与原版证据对照的关闭证据；“业务已存在”或“另一页已原生化”不代替这些证据。

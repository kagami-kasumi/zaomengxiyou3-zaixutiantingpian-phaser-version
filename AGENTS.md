...
- Git 交付以拉取后安装依赖即可构建、运行游戏为底线。`docs/tasks/evidence/` 的截图、trace、报告和中间产物仅本地保留；当前源码直接消费的必需 JSON 由 `.gitignore` 精确白名单保留。以后运行必需数据应放入正式源码/资源目录，不得依赖被忽略的证据文件。逆向复验仍可依赖本地语料，不能承诺仅拉取仓库即可完成全部源级复验。

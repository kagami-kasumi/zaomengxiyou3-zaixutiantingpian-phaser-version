...
- 同一个正式游戏 task 未完成且未触发首次 compact 门禁时，默认继续当前对话；上下文不足时先按 [compact 安全检查点](docs/workflow/agent-protocol.md#compact-安全检查点) 落盘。首次 compact 后只复查、结束检查、回写并拆分交接，不恢复新增实现。

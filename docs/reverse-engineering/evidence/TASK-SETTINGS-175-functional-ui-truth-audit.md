...
| --- | --- | --- | --- | --- | --- | --- |
| 1 `TASK-SETTINGS-175A` | 宠物 932 | 明确现代占位 + 证据未知 | `pets-index.md` 行为/owner 完整，但缺 932 的完整显示列表、注册点、矩阵、按钮态、动态 child 与原版逐状态基准；**不可机械升级** | `task-settings-175a.pet-page` / `task-settings-175a-pet-page.json` | 空列表、1..5 行、两页、selected、出战/休息、放生二次确认、洗练/进化、8 技能、P1/P2、关闭；对象数/父子/depth/状态集匹配，`unresolved=[]` | manifest `verified` 后生成单页实现 task，删除 `FormalPetPageView` 的现代暗层/矩形/Arial 控件并直接消费真值 |

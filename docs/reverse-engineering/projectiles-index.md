...
- `local-resources/regima/legacy-extraction/resources_by_swf/[172845].swf/scripts/export/bullet/`
- `local-resources/regima/legacy-extraction/resources_by_swf/[172845].swf/scripts/export/hero/Role2.as`
- `docs/reverse-engineering/roles-index.md`
- `docs/reverse-engineering/combat-rules-index.md`

备用包 `local-resources/regima/legacy-extraction/resources_by_swf/[25034429].swf/scripts` 本次未读；主包证据足够支撑首个切片。
...
当前 `local-resources/regima/legacy-extraction/resources_by_swf/[172845].swf/symbolClass/symbols.csv` 未列出 `Role2Bullet5` 或 `Role2_hit5`，主包导出的 PNG/JPG 也未直接暴露该素材。现代侧首批可使用占位视觉，稳定资源 key 建议：
...
- 主参考资源：`local-resources/regima/legacy-extraction/resources_by_swf/[172845].swf/symbolClass/symbols.csv`
- 主参考图片：`local-resources/regima/legacy-extraction/resources_by_swf/[172845].swf/images/`
- 备用资源：`local-resources/regima/legacy-extraction/resources_by_swf/[25034429].swf/symbolClass/symbols.csv`
- 备用图片：`local-resources/regima/legacy-extraction/resources_by_swf/[25034429].swf/images/`
...
当前 `local-resources/regima/legacy-extraction/resources_by_swf` 未提供可直接确认的 Role3 技能真素材；现代实现使用稳定占位 key，资源缺口不阻塞行为、数值和生命周期复现。
...
Role1 技能真素材未在当前 `local-resources/regima/legacy-extraction/resources_by_swf` 中形成可直接使用的完整导出；现代任务应沿用稳定占位 key，不伪造原版视觉。`Role1Shadow` 资源名为 `ROLE1_SHALLDOW`（源码拼写），持续 `gc.frameClips * 3`，自身不移动，只响应 `lyfb/zz` 两类同步动作。

...
| M-044 | 存档 | 已扒 | 部分复现 | `save-slots-index.md`、`save-party-flow-index.md`、`skills-input-index.md`、`pets-index.md`、`levels-index.md`、`User.getSaveObj()`、`MemoryClass.setStorage()`、`MainGame.levelClear()`、`GameWin.nextClick()` | `SaveSystem.ts` V6、`PartyConfigurationSystem.ts`、`SaveSlotSystem.ts`、`FormalPartyRuntimeSystem.ts`、正式 result bridges、专项测试 | V5 party、原子建槽和正式 owner 已闭合；V6 将 P1/P2 灵魂提升为 `player*.soulCount`，V1..V5 嵌套值无损迁移且拒绝损坏/双源数据；跨功能统一消费与正式旅程待 `TASK-ARCH-013B` 闭合 |

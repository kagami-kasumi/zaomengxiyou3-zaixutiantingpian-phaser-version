...
| M-022 | Role5 白龙 | 已扒 | 已复现 | `role5-combat-index.md`、`role5-combat-visuals-index.md`、`roles-index.md`、`Role5.as`、`BLMSkill5.as`、`JRJL.as` | `HeroNormalAttackSystem.ts`、`Role5SkillSystem.ts`、`Role5CombatVisualSystem.ts`、`Role5CombatVisualBridge.ts`、Role5专项测试 | `TASK-SLICE-105..109`玩法与069E/158E视觉均完成；枪/剑本体、装备、HUD、普攻、全部已实现技能/状态/阵列/瞬移对象已接入，`Role5runattack` 保持全集反证且无可见替代 |
...
| M-034 | 子弹/技能飞行物 | 已扒 | 部分复现 | `projectiles-index.md`、五角色战斗视觉索引、Stage 1/2怪物视觉索引、`BaseBullet.as`、`export/bullet/`、`Role1.as` 至 `Role5.as` | `ProjectileSystem.ts`、角色技能 systems、五角色/Stage 1/2 visual systems 与 bridges、`AssetManifest.ts` | 已完成关卡怪物和五角色当前已实现普攻/技能对象真视觉均已关闭；未达遗留对象、宠物和后续关卡仍保持机制级后续范围，因此全局状态仍为部分复现 |

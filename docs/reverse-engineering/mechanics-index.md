...
| M-026 | 关卡类命名 | 已扒 | 部分复现 | `levels-index.md`、`gameplay-index.md`、`MainGame.newGame()`、`PhysicsWorld.pWorldInit()`、RegiMA `assets/levels/level11.swf`、`assets/levels/level12.swf` 与 `assets/1.swf` | `AssetManifest.ts`、`Stage11Layout.ts`、`Stage12Layout.ts`、Stage 1-1/1-2 scene bridges、`Stage11EntryScene.ts` | Stage 1-1 与 1-2 的真场景、正式入口、普通流程和特殊入口均已接入；其他关卡仍按内容线逐关推进 |
| M-027 | 地图标记 | 已扒 | 部分复现 | `levels-index.md`、`BaseGameSence.as`、`PhysicsWorld.addSubObj()`、`StopPoint.as`、`MonsterAppearPoint.as`、`ViewControllor.as`、`BaseHero.as`、恢复 `sl12.as` | `src/systems/LevelSystem.ts`、`src/systems/Stage11Layout.ts`、`src/systems/Stage12Layout.ts`、`src/systems/Stage12TraversalSystem.ts` | Stage 1-1 已复现；Stage 1-2 的 3 个 ObsWall、1 个坠落墙、5 停点、13 刷怪点、普通门和特殊入口已接入，并以 `20..920` 屏幕边界、地面顶 `y=501.0501981` 与停点右端触发校正玩家遍历行为 |
...
| M-030 | 怪物系统 | 已扒 | 部分复现 | `monsters-index.md`、`levels-index.md`、`BaseMonster.as`、`BaseObject.as`、`BaseBullet.as`、`PhysicsWorld.as`、`MainGame.as`、`Monster2.as`、`Monster4.as` | `src/systems/Monster30System.ts`、`src/systems/Monster3System.ts`、`Stage12FlowSystem.ts`、Stage 1-2 gameplay bridge | 已有飞行小怪和地面 boss 模式；Stage 1-2 的 Monster7/8 五批 46 怪及 Monster4+Monster2 双 boss 显门门禁已按关卡 adapter 接入；其他怪物真素材与完整 AI 逐关后置 |
...
| M-035 | 资源加载策略 | 已扒 | 部分复现 | `README_extract.md`、`evb-extraction-report.md`、`modern-architecture.md`、`attack-effects-index.md`、`assets-index.md`、`projectiles-index.md`、`combat-assets-gap-plan.md`、`crafting-ui-index.md`、`levels-index.md` | `src/assets/AssetManifest.ts` 的状态化 manifest、Role1 普攻真资源、炼丹炉 UI/图标真资源、Stage 1-1/1-2 真场景资源和项目外已恢复源 SWF | Stage 1-2 character 25/135/22/52/48/51 已转换为 72 张 PNG，登记 stable key、源包/character/tag/源尺寸/帧数/栅格尺寸并纳入独立 bundle 与预加载；其他真素材仍按窄任务推进 |
...
| M-044 | 存档 | 已扒 | 部分复现 | `skills-input-index.md`、`pets-index.md`、`levels-index.md`、`User.getSaveObj()`、`MemoryClass.setStorage()`、`SaveInter.as` | `SaveSystem.ts` V3、`Stage11FlowSystem.ts`、`Stage12FlowSystem.ts`、`TestSceneSaveBridge.ts`、关卡专项测试 | 现代 V3 支持 1-1/1-2/1-3 进度，V1/V2 安全迁移默认 1-1；1-1/1-2 普通胜利幂等推进且保留玩家存档，失败和 1-2 特殊入口不推进，重玩低关卡不降级；任务/活动、完整背包和桌面多槽文件后置 |

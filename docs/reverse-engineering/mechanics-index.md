...
| M-026 | 关卡类命名 | 已扒 | 部分复现 | `levels-index.md`、`gameplay-index.md`、`MainGame.newGame()`、`PhysicsWorld.pWorldInit()`、`PhysicsWorld.addSubObj()`、`BaseHero.checkTransferDoor()`、RegiMA `assets/levels/level11.swf`、`level12.swf`、`level13.swf`、`level21.swf`、`level22.swf` 与 `assets/1.swf` / `2.swf` | `PlayableLevelRuntime.ts`、`PlayableLevelDefinition.ts`、`TransferDoorView.ts`、`LevelLifecycleSystem.ts`、`LevelResultView.ts`、Stage 1/2 layouts 与窄 adapters | 016B/C 已迁移四个横向关卡并验证不同门皮肤共用行为 owner；016D 继续迁移 1-1，最终删除其借用 1-3 门 key 的临时兼容 |
...
| M-029 | 世界主循环 | 已扒 | 部分复现 | `runtime-index.md`、五关 Scene/World/Gameplay/Flow、PG-013 | `PlayableLevelRuntime.ts`、`src/scenes/test-scene/TestSceneUpdatePipeline.ts`、`src/core/GameContext.ts`、`playable-level-runtime.md`、关卡 gameplay adapters | 016B/C 已把四个横向关卡的初始化、队伍/玩家、镜头/HUD、结果/保存/路由和销毁收进公共 Runtime；专属遭遇/机关仍为窄 adapter，仅 1-1/TestScene 待 016D |

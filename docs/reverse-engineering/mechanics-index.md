...
| M-027 | 地图标记 | 已扒 | 部分复现 | `levels-index.md`、`BaseGameSence.as`、`PhysicsWorld.addSubObj()`、`StopPoint.as`、`MonsterAppearPoint.as`、恢复 `sl12.as` | `src/systems/LevelSystem.ts`、`src/systems/Stage11Layout.ts`、`src/systems/Stage12Layout.ts` | Stage 1-1 已复现；Stage 1-2 的 3 个 ObsWall、1 个坠落墙、5 停点、13 刷怪点、普通门和特殊入口已完成数据与推进行为接入 |

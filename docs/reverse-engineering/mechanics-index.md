...
| M-040 | 等级/经验 | 已扒 | 部分复现 | `progression-index.md`、`reference/hero-progression-catalog-1.1.json`、`reference/hero-progression-catalog.schema.json`、`User.as`、`BaseHero.as`、`BaseRoleProperies.as`、`Role1.as` 至 `Role5.as`、`BaseMonster.as`、`TaskInterface.as` | `src/systems/ProgressionSystem.ts`、`src/systems/Monster30System.ts`、`src/scenes/TestScene.ts`、`src/scenes/test-scene/TestSceneCombatBridge.ts`、`tools/system-tests.ts` | 172 已以可重复生成目录闭合 5×90 基础属性、90 经验门槛、Role5 `int` 防御、跨级/回满/派生顺序、89/90 sentinel、普通怪双 owner 和原版/当前存档交接，未知为 0；复现仍为部分，因为当前实现尚未直连目录，Role5 小数防御、正式装备派生/P1/P2/HUD/V7 全集由 179 闭合 |

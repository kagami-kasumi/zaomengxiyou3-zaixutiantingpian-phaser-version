...
| M-026 | 关卡类命名 | 已扒 | 部分复现 | `levels-index.md`、`gameplay-index.md`、`MainGame.newGame()`、`PhysicsWorld.pWorldInit()`、RegiMA `assets/levels/level11.swf`、`assets/levels/level12.swf` 与 `assets/1.swf` | `AssetManifest.ts`、`Stage11Layout.ts`、`TestSceneStage11Bridge.ts`、`Stage11EntryScene.ts` | 1-1 已完整接入；Stage 1-2 已确认 `sl12/bg12/floorBg1` character 53/135/1 与组合层级，下一步 `TASK-SLICE-125` 接入 |
| M-027 | 地图标记 | 已扒 | 部分复现 | `levels-index.md`、`BaseGameSence.as`、`PhysicsWorld.addSubObj()`、`StopPoint.as`、`MonsterAppearPoint.as`、恢复 `sl12.as` | `src/systems/LevelSystem.ts`、`src/systems/Stage11Layout.ts` | 1-1 已复现；Stage 1-2 的 3 个 ObsWall、1 个坠落墙、5 停点、13 刷怪点、普通门和特殊入口已完整逆向，待 `TASK-SLICE-125` 数据接入 |
...
| M-035 | 资源加载策略 | 已扒 | 部分复现 | `README_extract.md`、`evb-extraction-report.md`、`modern-architecture.md`、`attack-effects-index.md`、`assets-index.md`、`projectiles-index.md`、`combat-assets-gap-plan.md`、`crafting-ui-index.md`、`levels-index.md` | `src/assets/AssetManifest.ts` 的状态化 manifest、Role1 普攻真资源、炼丹炉 UI/图标真资源、Stage 1-1 真场景资源和项目外已恢复源 SWF | Stage 1-1 已接入；Stage 1-2 character 25/135/22/52 与门子时间轴 48/51 已精确选择性调查派生，`TASK-SLICE-125` 负责现代资源转换、provenance 和预加载 |

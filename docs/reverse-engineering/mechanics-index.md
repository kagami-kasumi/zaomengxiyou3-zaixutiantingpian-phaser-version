...
| M-044 | 存档 | 部分已扒 | 未复现 | `skills-input-index.md`、`pets-index.md`、`User.getSaveObj()`、`MemoryClass.setStorage()`、`SaveInter.as` | 无 | 已确认技能相关存档字段、文件格式（AMF+compress+encrypt → gameData/{0-5}.sav）和顶层存储结构；宠物字段为 `petSave`，多宠物用 `}` 分隔，单宠物 26 个 `|` 字段并含 `isFight` 与 `skillSaveString`；宠物技能存档字段已补清为 `sname~sname`，空串代表无已学技能；下一步 `TASK-SLICE-048` 先实现宠物技能字段级编码/解码和面板展示，不接完整全局存档；装备/任务等字段留给对应系统任务 |

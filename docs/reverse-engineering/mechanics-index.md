...
| M-035 | 资源加载策略 | 已扒 | 部分复现 | `README_extract.md`、`evb-extraction-report.md`、`modern-architecture.md`、`attack-effects-index.md`、`assets-index.md`、`projectiles-index.md`、`combat-assets-gap-plan.md` | `src/assets/AssetManifest.ts` 的状态化 manifest、Role1 普攻真资源、炼丹炉 UI 真资源和项目外已恢复源 SWF | 已完成 Role1 普攻与炼丹炉首个 UI 真资源族；下一步 `TASK-SETTINGS-045` 窄查枯叶灵配方四图标，其余资源继续选择性接入 |

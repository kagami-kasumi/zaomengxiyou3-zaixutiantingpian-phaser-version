...
| M-026 | 关卡类命名 | 已扒 | 部分复现 | `levels-index.md`、`gameplay-index.md`、`MainGame.newGame()`、`PhysicsWorld.pWorldInit()`、RegiMA `assets/levels/level11.swf`、`level12.swf`、`level13.swf`、`level21.swf`、`level22.swf` 与 `assets/1.swf` / `2.swf` | `AssetManifest.ts`、Stage 1/2-1 layouts 与正式 scene bridges | Stage 2-2 已确认 `sl22` / `StageListener22`、character 64、34/36、279/3、63 和正式 2/2 加载链；现代接入转入 `VS-056` |
| M-027 | 地图标记 | 已扒 | 部分复现 | `levels-index.md`、`BaseGameSence.as`、`PhysicsWorld.addSubObj()`、`StopPoint.as`、`MonsterAppearPoint.as`、`ViewControllor.as`、`BaseHero.as`、恢复 `sl12/sl13/sl21/sl22.as` | `src/systems/LevelSystem.ts`、Stage 1 layout/traversal systems、`Stage21Layout.ts`、`Stage21TraversalSystem.ts` | Stage 2-2 的 3+1 墙、3 单向平台、5 停点、25 刷怪点、9 火焰、普通门和地面顶 `501.0501981` 已闭合为实现输入，待同线接入 |
...
| M-035 | 资源加载策略 | 已扒 | 部分复现 | `README_extract.md`、`evb-extraction-report.md`、`modern-architecture.md`、`assets-index.md`、`projectiles-index.md`、`levels-index.md`、`stage21-monster-visuals-index.md`、`asset-annotation/batches/stage22.md` | `src/assets/AssetManifest.ts` 的状态化 manifest、Stage 1/2-1 真场景与恢复源 SWF | Stage 2-2 的 14 条场景/机关/Boss stable key 已从 `level22.swf` / `2.swf` 选择性派生为 `derived-ready`；Monster9/10/19 明确复用 Stage 2-1 真资源，待同线实现接入 |

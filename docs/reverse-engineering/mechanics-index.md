...
| M-026 | 关卡类命名 | 已扒 | 部分复现 | `levels-index.md`、`gameplay-index.md`、`MainGame.newGame()`、`PhysicsWorld.pWorldInit()`、`PhysicsWorld.addSubObj()`、`BaseHero.checkTransferDoor()`、RegiMA `assets/levels/level11.swf`、`level12.swf`、`level13.swf`、`level21.swf`、`level22.swf` 与 `assets/1.swf` / `2.swf` | `LevelLifecycleSystem.ts`、`LevelResultView.ts`、`playable-level-runtime.md`、Stage 1/2 layouts 与正式 scene bridges | 016A 已冻结公共 Runtime/Definition/Encounter/adapter 与门视觉合同和防回填门禁；运行时骨架仍按关复制，当前由 016B 先迁移 Stage 1-2/1-3，016D 删除 1-1 借用 1-3 门 key 的临时兼容 |
...
| M-029 | 世界主循环 | 已扒 | 部分复现 | `runtime-index.md`、五关 Scene/World/Gameplay/Flow、PG-013 | `src/scenes/test-scene/TestSceneUpdatePipeline.ts`、`src/core/GameContext.ts`、`playable-level-runtime.md`、五关 gameplay bridges | 016A 已为初始化/队伍/镜头/HUD/调度/结果/保存/路由/销毁指定唯一目标 owner；运行时代码尚未迁移，016B..D 按消费者矩阵分批收敛，专属遭遇/机关只使用窄 adapter |

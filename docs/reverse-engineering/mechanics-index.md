...
| M-039 | 合成 | 已扒 | 已复现 | `crafting-index.md`、`crafting-ui-index.md`、`reference/crafting-recipes-1.1.json`、`Fusion.as`、`StrengthEquipment.as`、`AllEquipment.mixProduce()`、`getSunSutraValueEquip()`、`getMingDingHuaYanEquip()`、1.0 `crafting-recipes.csv` 辅助索引 | `src/systems/CraftingRecipeRegistry.ts`、`src/systems/CraftingSystem.ts`、`src/scenes/test-scene/TestSceneCraftingView.ts`、`src/scenes/test-scene/TestSceneUIHandlers.ts`、`tools/crafting-tests.ts` | 1.1 的 67 个现代 `direct_static`、41 个默认继承和 4 个特殊继承配方均已注册并复现核心事务；三槽 `CraftingSession` 覆盖暂存/退回/预览/原子合成与 P1/P2 隔离；真 UI 和两个真图标配方族已接入。`已复现` 只表示机制/配方规则存在，不代表玩家可见全内容覆盖完成；`LINE-CRAFTING` 继续追踪 112 配方的物品定义、真图标、正式入口和验证闭合 |

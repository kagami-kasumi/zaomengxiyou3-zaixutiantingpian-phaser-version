...
| M-035 | 资源加载策略 | 已扒 | 已复现 | `README_extract.md`、`evb-extraction-report.md`、`modern-architecture.md`、`assets-index.md`、`projectiles-index.md`、`levels-index.md`、`stage21-monster-visuals-index.md`、`asset-annotation/batches/stage22.md`、`asset-annotation/batches/inventory-items.md`、`PG-009` | `AssetManifest.ts` provenance、`SceneAssetBundles.ts` 唯一 owner、`AssetBundleCoordinator.ts`、`SceneAssetBundleBridge.ts`、场景路由与负向门禁 | Boot 仅加载 shell；地图、功能 UI、Stage 1/2 与 DEV/QA 直达按 bundle ensure；背包大资源按页懒加载，574 五入口 20 个小状态资源由唯一 `combat-common` 随正式关卡加载；不含纹理淘汰或图片转码 |
...
| M-052 | 完整功能 UI | 已扒 | 部分复现 | `full-function-ui-index.md`、`stage-feature-entry-index.md`、`equipment-workshop-index.md`、`skills-input-index.md`、`skill-ui-native-index.md`、`inventory-resource-catalog.md`、`equipment-index.md`、`pets-index.md`、`magic-weapons-index.md` 与完整 UI 资源标注 | `StageFeatureEntryRouterSystem.ts`、`StageSettingsSystem.ts`、`FormalFeatureUiEntryBridge.ts`、`StageSettingsScene.ts`、V6 与正式功能页 systems/views | 页面业务与资源合同已闭合；574 五真 pointer、371/444、全局 owner、单页互斥/同键/Escape/返回已接入，战斗可见通用 host/跨页/workshop 已移除。仍需 156C 在五关逐状态校准后升级关卡入口复现结论 |

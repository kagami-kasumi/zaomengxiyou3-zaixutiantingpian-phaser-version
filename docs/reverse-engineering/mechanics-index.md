...
| M-014 | 上/交互/通关 | 已扒 | 已复现 | `levels-index.md`、`BaseHero.checkTransferDoor()`、`Role*.myKeyDown()`、`MainGame.levelClear()`、2026-07-26 用户试玩反馈 | `LevelLifecycleSystem.ts`、`LevelLifecycleBridge.ts`、五关 Flow/bridges | `TASK-ARCH-015` 统一真实 bounds 重叠 + 对应上键、全员判负、幂等解锁与结果终态；五关完成首批迁移，特殊完成条件只能注入窄策略，静态门禁阻止新增同义 Flow |
...
| M-026 | 关卡类命名 | 已扒 | 部分复现 | `levels-index.md`、`gameplay-index.md`、`MainGame.newGame()`、`PhysicsWorld.pWorldInit()`、RegiMA `assets/levels/level11.swf`、`level12.swf`、`level13.swf`、`level21.swf`、`level22.swf` 与 `assets/1.swf` / `2.swf` | `LevelLifecycleSystem.ts`、`AssetManifest.ts`、Stage 1/2 layouts 与正式 scene bridges | 现代关卡现在共享生命周期骨架但保留逐关内容 Flow；Stage 2-2 场景闭合，其他关卡内容仍按各自证据线推进 |

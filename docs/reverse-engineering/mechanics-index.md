...
| M-042 | 宠物 | 已扒 | 部分复现 | `pets-index.md`、`BasePet`、`BaseHero.initPet()`、`User.petsAry`、`User.findCurrentPet()`、`User.catchNewPet()`、`PetInfo.as`、`PetInterface.as`、`PackThings.as`、`MagicBottle.as`、`MagicFlower.as`、`Monster70.as` 至 `Monster78.as` | `src/systems/PetSystem.ts`、`src/scenes/TestScene.ts`、`tools/system-tests.ts` | 已复现 P1 预置宠物、B 键宠物面板、单只出战/休息、跟随/远距传送、宣花葫芦捕捉、宠物消耗品、`jyhl/MagicFlower` 对出战宠物的增益状态，以及 `VS-015` 宠物经验/升级最小闭环：`Monster30` 击杀普通分成、`djyys` 共用升级入口、溢出经验、HP/MP 回满、首批属性公式刷新、升级形态 1→2 / 2→3 和运行时实体重建；宠物技能基础已扒清：`PetInfo.skill`、8 个 UI 槽、`getSkillSaveString()/setSkillSaveString()` 的 `sname~sname` 存档、升级随机学习、形态候选池、被动刷新、`BasePet` 主动技能分发和 `monkey1/xj` 最小触发链路；下一步 `TASK-SLICE-041` 实现 `VS-016` 宠物技能最小闭环；成长洗练、P2/联机、任务奖励经验、完整宠物存档、自动学习、全部被动/自动 buff 和真实资源后置 |

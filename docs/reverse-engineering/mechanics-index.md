...
| M-050 | 启动与存档槽流程 | 已扒 | 部分复现 | `save-slots-index.md`、`GMain.as`、`GameMenu.as`、`SaveInter.as`、`User.getSaveObj()`、`MemoryClass.setStorage()`、`MapMenu.as`、`OtherMat1.swf` 1149、`Common1.swf` 69/18 | `SaveSlotSystem.ts`、`SaveSlotScene.ts`、`SaveSystem.ts`、正式关卡入口/结果路由 | 六槽读写、当前槽、迁移、损坏保护与删除确认已完成；V4 未持久化人数且空槽直接创建默认角色，用户 2026-07-24 要求补新建存档人数/选角与队伍 schema，现由 `TASK-SETTINGS-065/ARCH-011/SLICE-151` 重开 |

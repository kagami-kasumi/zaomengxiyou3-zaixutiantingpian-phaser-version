...
| M-041 | 技能学习/绑定 | 已扒 | 已复现 | `skills-input-index.md`、`User.skillbykey`、`SkillControl.as`、`SkillSetControl.as`、`BuySkill.as`、`PassiveSkillControl.as` | `SkillUISystem.ts`、`FormalSkillPageSystem.ts`、`FormalSkillPageView.ts`、`FormalSkillRuntimeBridge.ts` | 技能规则、真 UI、P1/P2 五槽、HUD 与 V5 字段已完成；`TASK-SLICE-152` 已让入口、selector、事务、保存和 HUD 同步统一消费当前存档活动 owner/hero，单人 P2 三层拒绝 |
...
| M-044 | 存档 | 已扒 | 部分复现 | `save-slots-index.md`、`save-party-flow-index.md`、`skills-input-index.md`、`pets-index.md`、`levels-index.md`、`User.getSaveObj()`、`MemoryClass.setStorage()`、`MainGame.levelClear()`、`GameWin.nextClick()` | `SaveSystem.ts` V5、`PartyConfigurationSystem.ts`、`SaveSlotSystem.ts`、`FormalPartyRuntimeSystem.ts`、正式 result bridges、专项测试 | V5 party、旧档迁移、原子建槽、原生新建、技能/地图/关卡/HUD/功能页/retry owner 已闭合；其余未复现存档字段继续按对应系统后置 |

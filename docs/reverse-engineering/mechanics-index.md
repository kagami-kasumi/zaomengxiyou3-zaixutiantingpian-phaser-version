...
| M-030 | 怪物系统 | 已扒 | 部分复现 | `monsters-index.md`、`levels-index.md`、`stage1-monster-visuals-index.md`、`stage21-monster-visuals-index.md`、`BaseMonster.as`、`BaseObject.as`、`BaseBullet.as`、`PhysicsWorld.as`、`MainGame.as`、`Monster2/3/4/5/6/7/8/9/10/16/19/30.as` | `MonsterPhysicsSystem.ts`、`MonsterDefeatRewardSystem.ts`、`Stage11MonsterVisualSystem.ts`、`Stage12MonsterVisualSystem.ts`、对应 visual bridges、Stage 1 bridges、Stage 2-1 native visual owners、`Stage22FlowSystem.ts` / `Stage22Monster16VisualSystem.ts` | 五个已完成关卡的实际怪物全集已盘清；Stage 1-1/1-2 的 Monster30/3/2/4/7/8 共 136 个本体独立视觉帧、12 个攻击对象 147 帧与碰撞根已接入，Stage 1-3 只余 Monster5 新资源和共享复用；Stage 2-1/2-2 既有真视觉保持闭合 |

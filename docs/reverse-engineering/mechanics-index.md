...
| M-026 | 关卡类命名 | 已扒 | 部分复现 | `levels-index.md`、`gameplay-index.md`、`MainGame.newGame()`、`PhysicsWorld.pWorldInit()`、RegiMA `assets/levels/level11.swf`、`level12.swf`、`level13.swf`、`level21.swf` 与 `assets/1.swf` / `2.swf` | `AssetManifest.ts`、`Stage11Layout.ts`、`Stage12Layout.ts`、`Stage13Layout.ts`、`Stage21Layout.ts`、正式 scene bridges | Stage 2-1 的 `sl21`、19/21、282、3、48 与正式 2/1 入口已接入并完成 940×590 运行校准；怪物真视觉仍待补 |
| M-027 | 地图标记 | 已扒 | 部分复现 | `levels-index.md`、`BaseGameSence.as`、`PhysicsWorld.addSubObj()`、`StopPoint.as`、`MonsterAppearPoint.as`、`ViewControllor.as`、`BaseHero.as`、恢复 `sl12.as` / `sl13.as` / `sl21.as` | `src/systems/LevelSystem.ts`、Stage 1 layout/traversal systems、`Stage21Layout.ts`、`Stage21TraversalSystem.ts` | Stage 2-1 的 3+1 墙、4 单向平台、5 停点、25 刷怪点、38 冰刺、普通门和 `y=519.0501981` 已接入；五停点、两类冰刺和门已运行复验 |
...
| M-030 | 怪物系统 | 已扒 | 部分复现 | `monsters-index.md`、`levels-index.md`、`stage21-monster-visuals-index.md`、`BaseMonster.as`、`BaseObject.as`、`BaseBullet.as`、`PhysicsWorld.as`、`MainGame.as`、`Monster2/3/4/5/6/7/8/9/10/19/30.as` | `MonsterPhysicsSystem.ts`、`MonsterDefeatRewardSystem.ts`、Stage 1 bridges、`Stage21FlowSystem.ts`、`Stage21GameplayBridge.ts`、`Stage21MonsterVisualSystem.ts` | Stage 2-1 四怪的 94 个本体关键帧、七攻击对象 132 帧、100/130 碰撞根、朝向、触发 tick、注册点、死亡和 940×590 可见结果已接入；全局其他怪物真视觉仍未完全复现 |
...
| M-035 | 资源加载策略 | 已扒 | 部分复现 | `README_extract.md`、`evb-extraction-report.md`、`modern-architecture.md`、`attack-effects-index.md`、`assets-index.md`、`projectiles-index.md`、`combat-assets-gap-plan.md`、`crafting-ui-index.md`、`equipment-workshop-index.md`、`levels-index.md`、`stage21-monster-visuals-index.md` | `src/assets/AssetManifest.ts` 的状态化 manifest、Stage 1/2-1 真场景与恢复源 SWF | Stage 2-1 真视觉已选择性接入；Stage 1-1 普通门直接消费 Stage 1-3 已加载的 `transferDoor` stable key，未重复导出或增加资源副本；全局资源加载仍为增量复现 |
...
| M-044 | 存档 | 已扒 | 部分复现 | `save-slots-index.md`、`skills-input-index.md`、`pets-index.md`、`levels-index.md`、`User.getSaveObj()`、`MemoryClass.setStorage()`、`SaveInter.as` | `SaveSystem.ts` V4、`SaveSlotSystem.ts`、Stage 1/2-1 result bridges、专项测试 | 正式六槽、迁移与损坏保护保持闭合；Stage 2-1 门胜利幂等推进并保存 2/2、失败不推进、返回地图已在 940×590 正式流程复验 |
...
| M-051 | 天庭地图选关 | 已扒 | 已复现 | `heaven-map-index.md`、`SelectPLace.as`、`MapMenu.as`、`GMain.as`、`MainGame.as`、`GameWin.as`、`GameFail.as`、`OtherMat1.swf` 1343/963/1311/1297/1304/1290、用户 2026-07-23 试玩反馈 | `HeavenMapSystem.ts`、`HeavenMapScene.ts`、Stage 1/2-1 结果/退出桥、`heaven-map-tests.ts` | 四节点状态、现代 1P/2P 选择、单调进度和已接入关卡往返闭合；`TASK-SLICE-147` 修复 scene 重入后残留 chooser 引用，940×590 1-1 返回后再次选择 1-2 通过 |
| M-052 | 完整功能 UI | 部分已扒 | 部分复现 | `full-function-ui-index.md`、`equipment-workshop-index.md`、`skills-input-index.md`、`skill-ui-native-index.md`、`equipment-index.md`、`pets-index.md`、`magic-weapons-index.md` 与 11 条完整 UI 资源标注 | 共享 `FeatureUiHost`、V4、正式背包、技能、宠物、工坊四事务与法宝页面 systems/views | 工坊已按 PG-007 原生化；技能四页完整显示列表、原版基准、状态/命中区和动态槽位证据已闭合，现有现代外层 UI 由 `TASK-SLICE-143` 移除。宠物、法宝等存量页面继续由 PG-007 追踪，不以“真背景已接入”宣告 UI 原生化完成 |

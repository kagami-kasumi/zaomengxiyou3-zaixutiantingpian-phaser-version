...
| M-026 | 关卡类命名 | 已扒 | 部分复现 | `levels-index.md`、`gameplay-index.md`、`MainGame.newGame()`、`PhysicsWorld.pWorldInit()`、RegiMA `assets/levels/level11.swf`、`assets/levels/level12.swf` 与 `assets/1.swf` | `AssetManifest.ts`、`Stage11Layout.ts`、`Stage12Layout.ts`、Stage 1-1/1-2 scene bridges、`Stage11EntryScene.ts` | 1-1 已完整接入；Stage 1-2 的 `sl12/bg12/floorBg1` 组合、真资源与已解锁入口已接入，普通流程和特殊入口行为后置 |
| M-027 | 地图标记 | 已扒 | 部分复现 | `levels-index.md`、`BaseGameSence.as`、`PhysicsWorld.addSubObj()`、`StopPoint.as`、`MonsterAppearPoint.as`、恢复 `sl12.as` | `src/systems/LevelSystem.ts`、`src/systems/Stage11Layout.ts`、`src/systems/Stage12Layout.ts` | 1-1 已复现；Stage 1-2 的 3 个 ObsWall、1 个坠落墙、5 停点、13 刷怪点、普通门和特殊入口已完整数据接入，下一步接入推进行为 |
...
| M-030 | 怪物系统 | 已扒 | 部分复现 | `monsters-index.md`、`levels-index.md`、`BaseMonster.as`、`BaseObject.as`、`BaseBullet.as`、`PhysicsWorld.as`、`MainGame.as`、`Monster2.as`、`Monster4.as` | `src/systems/Monster30System.ts`、`src/systems/Monster3System.ts`、`src/scenes/TestScene.ts` | 已有飞行小怪和地面 boss 两种模式；Stage 1-2 五批 46 怪与 Monster4+Monster2 双 boss 显门门禁已逆向，待场景基础接入后实现 |
...
| M-044 | 存档 | 已扒 | 部分复现 | `skills-input-index.md`、`pets-index.md`、`levels-index.md`、`User.getSaveObj()`、`MemoryClass.setStorage()`、`SaveInter.as` | `SaveSystem.ts` V3、`Stage11FlowSystem.ts`、`TestSceneSaveBridge.ts`、`stage11-flow-tests.ts` | 已确认原版胜利推进后显式存档顺序；现代 V3 新增 `LevelUnlockProgress`，V1/V2 安全迁移默认 1-1，胜利幂等保存 1-2 且自动存档保留该值，浏览器刷新验收通过；任务/活动、完整背包和桌面多槽文件后置 |

...
| M-030 | 怪物系统 | 已扒 | 部分复现 | `monsters-index.md`、`levels-index.md`、`BaseMonster.as`、`BaseObject.as`、`BaseBullet.as`、`PhysicsWorld.as`、`MainGame.as`、`Monster2/3/4/5/7/8/30.as` | `src/systems/Monster30System.ts`、`src/systems/Monster3System.ts`、`Stage12FlowSystem.ts`、`Stage13FlowSystem.ts`、Stage gameplay bridges | Stage 1-3 已复现 9/10/12/13/61 五批、1P/2P 6/8 上限、Monster5 死亡即显门和剩余 Monster30 不阻塞进门；怪物真素材与完整 AI 仍后置 |
...
| M-049 | 正式战斗 HUD | 未扒 | 部分复现 | 原版 HUD/UI AS3 与恢复源包待 `TASK-SETTINGS-055` 窄查 | 测试状态栏、`SkillUISystem.ts`、最小宠物/背包面板 | 闭合 P1/P2 HP/MP/经验/等级、技能和重要敌人状态的布局、资源与更新语义，再接正式 Stage 1 HUD |

...
| M-026 | 关卡类命名 | 已扒 | 部分复现 | `levels-index.md`、`gameplay-index.md`、`MainGame.newGame()`、`PhysicsWorld.pWorldInit()`、`PhysicsWorld.addSubObj()`、`BaseHero.checkTransferDoor()`、RegiMA `assets/levels/level11.swf`、`level12.swf`、`level13.swf`、`level21.swf`、`level22.swf` 与 `assets/1.swf` / `2.swf` | `LevelLifecycleSystem.ts`、`LevelResultView.ts`、`AssetManifest.ts`、Stage 1/2 layouts 与正式 scene bridges | 局部生命周期/结果已共享，但 Scene/World/Gameplay/门视图仍按关复制；PG-013 与 TASK-ARCH-016A..D 将建立组合式公共 Runtime/Definition/Encounter/adapter，禁止把 SWF 打包位置当现代 owner |
...
| M-029 | 世界主循环 | 已扒 | 部分复现 | `runtime-index.md`、五关 Scene/World/Gameplay/Flow、PG-013 | `src/scenes/test-scene/TestSceneUpdatePipeline.ts`、`src/core/GameContext.ts`、五关 gameplay bridges | TestScene pipeline 与薄 GameContext 已存在，但正式关卡共同初始化/调度/退出仍按关复制；TASK-ARCH-016A..D 将以组合式 Runtime 分批收敛，专属遭遇/机关继续使用窄 adapter |
| M-030 | 怪物系统 | 已扒 | 部分复现 | `monsters-index.md`、`levels-index.md`、`stage1-monster-visuals-index.md`、`stage21-monster-visuals-index.md`、`BaseMonster.as`、`BaseObject.as`、`BaseBullet.as`、`PhysicsWorld.as`、`MainGame.as`、`Monster2/3/4/5/6/7/8/9/10/16/19/30.as` | `MonsterPhysicsSystem.ts`、`MonsterDefeatRewardSystem.ts`、`Stage11MonsterVisualSystem.ts`、`Stage12MonsterVisualSystem.ts`、`Stage13Monster5VisualSystem.ts`、对应 visual bridges、Stage 1 bridges、Stage 2-1 native visual owners、`Stage22FlowSystem.ts` / `Stage22Monster16VisualSystem.ts` | 五个已完成关卡的实际怪物全集已盘清；Stage 1 三关的 Monster2/3/4/5/7/8/30 共 167 个本体独立视觉帧、16 个攻击对象 171 帧与碰撞根已接入，Stage 1-3 复用既有四怪 identity；Stage 2-1/2-2 既有真视觉待 157D 最终防回归后关闭 VS-061 |
...
| M-034 | 子弹/技能飞行物 | 已扒 | 部分复现 | `projectiles-index.md`、`role5-combat-index.md`、`stage1-monster-visuals-index.md`、`stage21-monster-visuals-index.md`、`BaseBullet.as`、`export/bullet/`、`Role1.as` 至 `Role5.as`、`Role1Shadow.as`、`MagicPearl.as`、`MagicZLHummer.as`、`Ling.as`、`MagicBigBottle.as` | `ProjectileSystem.ts`、`HeroNormalAttackSystem.ts`、`Role1BasicSkillSystem.ts`、`Role1SkillProjectileFactory.ts`、`Role1ShadowSkillSystem.ts`、`Role1FinisherSkillSystem.ts`、`Role2*SkillSystem.ts`、`Role3*SkillSystem.ts`、`Role4*SkillSystem.ts`、`Role5SkillSystem.ts`、`Stage11MonsterVisualSystem.ts`、`Stage12MonsterVisualSystem.ts`、对应 visual bridges、`Stage21MonsterVisualSystem.ts`、`Stage21MonsterVisualBridge.ts`、`AssetManifest.ts` | Stage 1-1/1-2 的 12/16 个攻击对象、147/171 帧已按原触发 tick、生成点、注册边界、镜像与末帧生命周期接入；Stage 1-3 只余 Monster5 四对象 24 帧，Stage 2-1 七对象 132 帧保持已接入；角色 projectile 真视觉仍由 VS-062 后续盘清 |
| M-035 | 资源加载策略 | 已扒 | 已复现 | `README_extract.md`、`evb-extraction-report.md`、`modern-architecture.md`、`assets-index.md`、`projectiles-index.md`、`levels-index.md`、`level-result-ui-index.md`、`stage1-monster-visuals-index.md`、`stage21-monster-visuals-index.md`、`asset-annotation/batches/stage1-monsters.md`、`asset-annotation/batches/stage22.md`、`asset-annotation/batches/inventory-items.md`、`asset-annotation/batches/level-results.md`、`PG-009` | `AssetManifest.ts` provenance、`SceneAssetBundles.ts` 唯一 owner、`AssetBundleCoordinator.ts`、`SceneAssetBundleBridge.ts`、场景路由与负向门禁 | Boot 仅加载 shell；地图、功能 UI、Stage 1/2 与 DEV/QA 直达按 bundle ensure；原版关卡结果资源由 `combat-common` 唯一 owner；Stage 1-2 新增 13 条资源由 `stage-12` 唯一 owner，Stage 1 怪物余 8 条待 157C 接入并由 157D 审计最终共享 owner；不含纹理淘汰或图片转码 |

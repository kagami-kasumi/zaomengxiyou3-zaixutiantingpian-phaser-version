...
| M-040 | 等级/经验 | 已扒 | 未复现 | `progression-index.md`、`User.as`、`BaseHero.as`、`BaseRoleProperies.as`、`Role1.as` 至 `Role5.as`、`BaseMonster.as`、`TaskInterface.as` | 无 | 逆向已足够支撑 `VS-014` 等级/经验最小闭环；首切片先做击杀加经验、自动升级、扣本级经验、回满 HP/MP 和五角色基础属性表，任务奖励/宠物经验/难度倍率/存档后置 |

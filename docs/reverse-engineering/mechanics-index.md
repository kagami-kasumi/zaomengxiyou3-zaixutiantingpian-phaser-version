...
| M-005 | 主流程 | 已扒 | 已复现 | `gameplay-index.md`、`runtime-index.md`、`save-party-flow-index.md` | 启动/活动槽/地图/五个已接入关卡/结果主循环与 `FormalPartyRuntimeSystem.ts` | 新建人数/选角、读档分流、地图直接进关、返回/重试/重载和正式 party owner 已闭合；未接入关卡仍按各自内容线推进 |
| M-006 | 单人/双人规则 | 已扒 | 已复现 | `controls-index.md`、`gameplay-index.md`、`save-party-flow-index.md` | V5 party、双玩家输入、正式关卡/HUD/功能页 | P1→P2、不同角色、playerNum/controlPlayer/roleid 等价合同已进入存档和正式消费者；中途换人、联网明确排除 |
...
| M-016 | UI 快捷键 | 已扒 | 已复现 | `stage-feature-entry-index.md`、`skills-input-index.md`、`pets-index.md`、`KeyBoardControl.as`、`GameInfo.as`、`RoleInfo.as`、`magic-weapons-index.md` | `StageFeatureEntryRouterSystem.ts`、`StageSettingsSystem.ts`、`FormalFeatureUiEntryBridge.ts`、`StageSettingsScene.ts`、现有功能 systems | 574 五真 HUD、371/444 设置/帮助、P1 `C/V/B/N/Esc`、P2 `/ * -`、P2 技能默认 P1、全局设置 owner、固定 HUD 命中、单页同键、Escape 仅设置与返回路由已在五关完成确定性/运行校准；P2 无法宝/设置快捷键保持原版边界 |
| M-017 | 角色列表 | 已扒 | 未复现 | `roles-index.md`、`export/hero/Role1.as` 至 `Role5.as`、`User.getRoleName()` | 无 | 角色动作索引已够支撑选择首个角色，后续实现前先补移动和资源 |
| M-018 | Role1 悟空 | 已扒 | 已复现 | `roles-index.md`、`skills-input-index.md`、`projectiles-index.md`、`role1-combat-visuals-index.md`、`Role1.as`、`Role1Shadow.as`、`BaseRoleProperies.as` | Role1 技能 systems、`Role1CombatVisualBridge.ts`、`TestSceneRole1ShadowVisualBridge.ts`、独立测试 | 行为与本体/装备/影分身/普攻/全部已实现技能真视觉均由 158A 闭合；死亡按原版无 Symbol 的移除合同 |
| M-019 | Role2 唐僧 | 已扒 | 已复现 | `roles-index.md`、`skills-input-index.md`、`projectiles-index.md`、`role2-combat-visuals-index.md`、`Role2.as`、`Role2Shadow.as` | `HeroNormalAttackSystem.ts`、Role2 技能 systems、`Role2CombatVisualSystem.ts`、`Role2CombatVisualBridge.ts` | 行为与真视觉均已复现；069B/158B/162 已闭合本体/装备、Shadow、两普攻、九技能对象、唐僧 HUD 头像、单/双人方向与零 console 证据 |
| M-020 | Role3 八戒 | 已扒 | 已复现 | `roles-index.md`、`skills-input-index.md`、`projectiles-index.md`、`role3-combat-visuals-index.md`、`Role3.as`、`BaseAddEffect.as`、`BaseBitmapDataClip.as` | `Role3DefenseSkillSystem.ts`、`Role3ControlSkillSystem.ts`、`Role3ImpactSkillSystem.ts`、`Role3MobilitySkillSystem.ts`、`Role3UltimateSkillSystem.ts`、`Role3CombatVisualSystem.ts`、`Role3CombatVisualBridge.ts` | 九项主动、`rj` 被动与协同行为已复现；069C/158C 已闭合本体/装备、三普攻、全部技能对象、盾、隐藏/追踪与 HUD 真视觉 |
| M-021 | Role4 沙僧 | 已扒 | 已复现 | `role4-combat-index.md`、`role4-combat-visuals-index.md`、`roles-index.md`、`skills-input-index.md`、`projectiles-index.md`、`Role4.as`、`MonsterRole4Hit5.as`、`BaseAddEffect.as` | `HeroNormalAttackSystem.ts`、Role4六个技能系统、`Role4CombatVisualSystem.ts`、`Role4CombatVisualBridge.ts` 与独立测试 | 069D/158D已闭合铲/弓双形态、装备合成、五普攻、全部已实现技能/附属对象、SpeedUp与HUD frame4真视觉；Role4范围现代可见例外为0 |
| M-022 | Role5 白龙 | 已扒 | 已复现 | `role5-combat-index.md`、`role5-combat-visuals-index.md`、`roles-index.md`、`Role5.as`、`BLMSkill5.as`、`JRJL.as` | `HeroNormalAttackSystem.ts`、`Role5SkillSystem.ts`、`Role5SkillTuning.ts`、`Role5SkillMath.ts`、`Role5SkillTypes.ts`、Role5 场景桥接、独立测试 | `TASK-SLICE-105..109` 已完成玩法；069E 已闭合枪/剑本体、装备、HUD、普攻、全部已实现技能与附属对象视觉身份，且记录 `Role5runattack` 全集反证；158E负责真视觉接入 |
...
| M-030 | 怪物系统 | 已扒 | 部分复现 | `monsters-index.md`、`levels-index.md`、`stage1-monster-visuals-index.md`、`stage21-monster-visuals-index.md`、`BaseMonster.as`、`BaseObject.as`、`BaseBullet.as`、`PhysicsWorld.as`、`MainGame.as`、`Monster2/3/4/5/6/7/8/9/10/16/19/30.as` | `MonsterPhysicsSystem.ts`、`MonsterDefeatRewardSystem.ts`、Stage 1/2 visual systems 与 bridges、五关正式 scenes | 五个已完成关卡的实际怪物真视觉已关闭：Stage 1 七怪 167 本体帧/16 对象 171 帧，Stage 2-1 94/132，Stage 2-2 36/104；共享 owner、重入和 Arc/Text 防回填门禁通过。其余未完成关卡与通用怪物架构仍后续推进 |
...
| M-034 | 子弹/技能飞行物 | 已扒 | 部分复现 | `projectiles-index.md`、`role1-combat-visuals-index.md`、`role2-combat-visuals-index.md`、`role3-combat-visuals-index.md`、`role4-combat-visuals-index.md`、`role5-combat-index.md`、`role5-combat-visuals-index.md`、`stage1-monster-visuals-index.md`、`stage21-monster-visuals-index.md`、`hero-combat-visuals-index.md`、`BaseBullet.as`、`export/bullet/`、`Role1.as` 至 `Role5.as` | `ProjectileSystem.ts`、角色技能 systems、Stage 1/2 monster visual systems 与 bridges、`AssetManifest.ts` | 怪物与Role1..Role4真视觉已关闭；Role5对象级证据已由069E关闭，包含枪普攻 `Role5Bullet1..5` 与全部已实现技能/状态对象，158E待接入 |
| M-035 | 资源加载策略 | 已扒 | 已复现 | `README_extract.md`、`evb-extraction-report.md`、`modern-architecture.md`、`assets-index.md`、`projectiles-index.md`、`levels-index.md`、`level-result-ui-index.md`、`stage1-monster-visuals-index.md`、`stage21-monster-visuals-index.md`、资源标注批次、`PG-009` | `AssetManifest.ts` provenance、`SceneAssetBundles.ts` 唯一 owner、`AssetBundleCoordinator.ts`、`SceneAssetBundleBridge.ts`、场景路由与负向门禁 | Boot 仅加载 shell；地图、功能 UI、Stage 1/2 与 DEV/QA 直达按 bundle ensure；Stage 1 七怪/16 对象、Stage 2 怪物及 Role1..3 战斗视觉均有唯一 bundle owner；五关重入幂等且无重复 stable key。不含纹理淘汰或图片转码 |
| M-036 | 装备 | 已扒 | 已复现 | `equipment-index.md`、`equipment-workshop-index.md`、`magic-weapons-index.md`、`my/AllEquipment.as`、`my/MyEquipObj.as`、`export/pack/BackPack.as`、`export/strength/Strength/Resolution/Making/SutraInterface.as`、`base/BaseRoleProperies.as` | `EquipmentSystem.ts`、`EquipmentStrengtheningSystem.ts`、`EquipmentResolutionSystem.ts`、`EquipmentMakingSystem.ts`、正式背包/工坊/法宝页面 | 真背包页支持 P1/P2 六槽穿脱、角色限制、属性预览和 V4；真强化/分解/制作页已闭合实例成长、随机事务、78 本可达制作书、三宝石实例覆写和双 owner 保存；P1 法宝等级、材料分支、五行重置与实例保存已闭合，完整 1.1 装备基础数值表仍按后续任务闭合 |
| M-037 | 背包 | 已扒 | 已复现 | `inventory-resource-catalog.md`、`reference/inventory-resource-catalog-1.1.json`、`evidence/TASK-SLICE-160-visual-audit.md`、`equipment-index.md`、`user/User.as`、`export/pack/BackPackElement.as`、`export/pack/PackThings.as`、`config/Config.as` | `InventoryResourceCatalog.ts`、`InventorySystem.ts`、`FormalInventoryPageSystem.ts`、`FormalInventoryPageView.ts` | 431 个权威身份全部进入统一 registry；428 项精确真图标按需加载，3 项原版缺陷排除；四分类、5×25 分页、实例/99 堆叠、满包失败原子不变、P1/P2 owner 与 V6 往返均闭合。物品专属使用效果仍由对应玩法机制负责 |
...
| M-039 | 合成 | 已扒 | 已复现 | `crafting-index.md`、`crafting-ui-index.md`、`equipment-workshop-index.md`、`reference/crafting-recipes-1.1.json`、`Fusion.as`、`Making.as`、`AllEquipment.as` | `CraftingRecipeRegistry.ts`、`CraftingItemDefinitionRegistry.ts`、`CraftingSystem.ts`、`EquipmentMakingRegistry.ts`、`EquipmentMakingSystem.ts`、正式工坊页面与专项 | Fusion 112 配方/201 定义/201 真图标及 224 条 P1/P2 事务保持闭合；制作书另覆盖 78 个可达 case、1 个死分支、材料/灵魂/三宝石、实例覆写、双 owner 返还和当前槽保存 |
...
| M-041 | 技能学习/绑定 | 已扒 | 已复现 | `skills-input-index.md`、`User.skillbykey`、`SkillControl.as`、`SkillSetControl.as`、`BuySkill.as`、`PassiveSkillControl.as` | `SkillUISystem.ts`、`FormalSkillPageSystem.ts`、`FormalSkillPageView.ts`、`FormalSkillRuntimeBridge.ts` | 技能规则、五角色两心法、真 UI、P1/P2 五槽、HUD 与 V6 玩家直属灵魂已完成；技能/工坊共用动态余额投影，跨页进入从权威存档重建目标 model |
...
| M-044 | 存档 | 已扒 | 部分复现 | `save-slots-index.md`、`save-party-flow-index.md`、`skills-input-index.md`、`pets-index.md`、`levels-index.md`、`level-result-ui-index.md`、`inventory-resource-catalog.md`、`User.getSaveObj()`、`MemoryClass.setStorage()`、`MainGame.levelClear()`、`GameWin.nextClick()` | `SaveSystem.ts` V6、`PartyConfigurationSystem.ts`、`SaveSlotSystem.ts`、`FormalPartyRuntimeSystem.ts`、`PlayerSoulSystem.ts`、`LevelResultView.ts`、专项测试 | V5 party、原子建槽和正式 owner 已闭合；关卡解锁保存由公共结果 presenter 统一提交，五关不再持有私有结果保存 bridge；V6 背包、技能、工坊、法宝与灵魂 owner 继续共享。整体仍为“部分复现”只因更广泛原版字段/系统未全部进入现代范围 |
| M-045 | 多人网络 | 暂缓 | 暂缓 | `Config.as` 中 `Client` | 无 | 本地双人优先，网络不进第一批 |
| M-046 | 支付/活动/礼包 | 暂缓 | 暂缓 | `Config.as`、UI | 无 | 非核心复刻，后续按需判断 |
| M-047 | 角色普攻特效资源 | 已扒 | 部分复现 | `attack-effects-index.md`、`assets-index.md`、`combat-assets-gap-plan.md`、`evb-extraction-report.md`、`hero-combat-visuals-index.md`、`role1-combat-visuals-index.md`、`role2-combat-visuals-index.md`、`role3-combat-visuals-index.md`、`role4-combat-visuals-index.md`、`role5-combat-visuals-index.md`、`Role*.normalHit()`、`Role*.doHit*()`、`BaseBitmapDataPool.as` | Role1..Role4 真资源序列、对应 CombatVisualBridge、`HeroNormalAttackSystem.ts` | Role1..Role4已接入；069E已闭合Role5双形态动作、枪普攻 `Role5Bullet1..5` 与剑普攻对象，`Role5runattack` 为明确原包反证；158E待接入，不以占位替代 |
...
| M-049 | 正式战斗 HUD | 已扒 | 已复现 | `combat-hud-index.md`、`role2-combat-visuals-index.md`、`role3-combat-visuals-index.md`、`role4-combat-visuals-index.md`、`Stage1CombatHudSystem.ts`、`Stage1CombatHudBridge.ts`、恢复 `OtherMat1/bossblood.swf` | Stage 1 三关共享 snapshot/bridge；P1/P2 独立成长与技能状态；Role2/Role3/Role4 使用 character 505 frame 2/3/4 头像；Boss 即时/0.8s 追赶条 | 940×590固定层、P1/P2镜像/五槽键位及Role2..Role4头像已运行闭合；Role5映射仍待069E/158E |
...
| M-052 | 完整功能 UI | 已扒 | 部分复现 | `full-function-ui-index.md`、`stage-feature-entry-index.md`、`level-result-ui-index.md`、`equipment-workshop-index.md`、`skills-input-index.md`、`skill-ui-native-index.md`、`inventory-resource-catalog.md`、`equipment-index.md`、`pets-index.md`、`magic-weapons-index.md` 与完整 UI 资源标注 | `StageFeatureEntryRouterSystem.ts`、`StageSettingsSystem.ts`、`FormalFeatureUiEntryBridge.ts`、`StageSettingsScene.ts`、`LevelResultView.ts`、V6 与正式功能页 systems/views | 页面业务、战斗入口与五关校准已闭合；五关胜利/失败统一使用原版 GameWin/GameFail 及真按钮三态，现代黑框结果页为零。M-052 因本线后续怪物/角色视觉与最终旅程仍保持部分复现 |

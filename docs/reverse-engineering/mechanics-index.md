...
| M-021 | Role4 沙僧 | 已扒 | 已复现 | `role4-combat-index.md`、`role4-combat-visuals-index.md`、`attack-effects-index.md`、`Role4.as`、`MonsterRole4Hit5.as`、`BaseAddEffect.as` | `HeroNormalAttackSystem.ts`、`HeroNormalAttackGeometry.ts`、Role4六技能系统与视觉桥 | 163闭合弓形Arrow1/2固定world命中，铲形Follow对象保持角色附属；双形态方向/固定点/负向门禁与全量回归通过 |
| M-022 | Role5 白龙 | 已扒 | 部分复现 | `role5-combat-index.md`、`role5-combat-visuals-index.md`、`attack-effects-index.md`、`Role5.as`、`BLMSkill5.as`、`JRJL.as` | `HeroNormalAttackSystem.ts`、`Role5SkillSystem.ts`、`Role5NormalAttackProjectileSystem.ts`、Role5视觉桥与专项测试 | 164已复现龙魂剑hit18..20的三段EnemyMove轨迹、真资源、实际命中与双owner隔离；未达遗留对象仍使角色机制保持部分复现 |
...
| M-034 | 子弹/技能飞行物 | 已扒 | 部分复现 | `projectiles-index.md`、五角色战斗视觉索引、Stage 1/2怪物视觉索引、`BaseBullet.as`、`export/bullet/`、`Role1.as` 至 `Role5.as` | `ProjectileSystem.ts`、角色技能 systems、五角色/Stage 1/2 visual systems 与 bridges、`AssetManifest.ts` | 已完成关卡怪物和五角色当前已实现普攻/技能对象真视觉均已关闭；165A 使普攻对象先继承 Role1/2 的角色级本体视觉根再叠加既有局部 offset，未改碰撞/伤害/时序；未达遗留对象、宠物和后续关卡仍保持机制级后续范围，因此全局状态仍为部分复现 |
| M-035 | 资源加载策略 | 已扒 | 已复现 | `README_extract.md`、`evb-extraction-report.md`、`modern-architecture.md`、`assets-index.md`、`projectiles-index.md`、`levels-index.md`、`level-result-ui-index.md`、`stage1-monster-visuals-index.md`、`stage21-monster-visuals-index.md`、`equipment-visual-resource-catalog.md`、资源标注批次、`PG-009` | `AssetManifest.ts` provenance、`SceneAssetBundles.ts` 唯一 owner、`AssetBundleCoordinator.ts`、`SceneAssetBundleBridge.ts`、`EquipmentPreviewAssets.ts`、场景路由与负向门禁 | Boot 仅加载 shell；地图、功能 UI 和 Stage 1/2 按 bundle ensure；170C 已按 170B2 package/character 派生 111 个唯一穿戴/称号资源，并按当前装备选择性动态加载，避免把全集 atlas 加入背包首屏 bundle。不含纹理淘汰或图片转码 |
| M-036 | 装备 | 已扒 | 部分复现 | `equipment-index.md`、`equipment-data-catalog.md`、`reference/equipment-data-catalog-1.1.json`、`equipment-visual-resource-catalog.md`、`reference/equipment-visual-resource-catalog-1.1.json`、`evidence/TASK-SETTINGS-170B1-equipment-page.md`、`task-settings-170b1-equipment-page.json`、`equipment-workshop-index.md`、`workshop-rules-completeness-audit.md`、`magic-weapons-index.md` 与 1.1 装备/背包/角色消费者 | `EquipmentCatalog.ts`、`EquipmentPreviewSystem.ts`、`EquipmentSystem.ts`、`EquipmentStrengtheningSystem.ts`、`EquipmentResolutionSystem.ts`、`EquipmentMakingSystem.ts`、正式背包/工坊/法宝页面 | 170C 已让 164 件装备由 170A JSON 直接进入唯一 registry，保留基础范围、原比例单位和强化成长；视觉目录映射、两个原缺陷、37 个不变项与既有 transaction/V6 owner 已回归。170D 继续闭合 170B1 真值直连与五角色逐状态视觉；打造百分数单位留到事务收口 |
...
| M-044 | 存档 | 已扒 | 部分复现 | `save-slots-index.md`、`save-party-flow-index.md`、`skills-input-index.md`、`pets-index.md`、`levels-index.md`、`level-result-ui-index.md`、`inventory-resource-catalog.md`、`User.getSaveObj()`、`MemoryClass.setStorage()`、`MainGame.levelClear()`、`GameWin.nextClick()` | `SaveSystem.ts` V6、`PartyConfigurationSystem.ts`、`SaveSlotSystem.ts`、`FormalPartyRuntimeSystem.ts`、`PlayerSoulSystem.ts`、`LevelResultView.ts`、`pre-stage23-save-journey-tests.ts`、专项测试 | V5 party、原子建槽和正式 owner 已闭合；关卡解锁保存由公共结果 presenter 统一提交，五关不再持有私有结果保存 bridge；四地图服务、五战斗入口、P1/P2 事务、设置独立 owner、V6 重启与损坏槽正式旅程已闭合。整体仍为“部分复现”只因更广泛原版字段/系统未全部进入现代范围 |
| M-045 | 多人网络 | 暂缓 | 暂缓 | `Config.as` 中 `Client` | 无 | 本地双人优先，网络不进第一批 |
| M-046 | 支付/活动/礼包 | 暂缓 | 暂缓 | `Config.as`、UI | 无 | 非核心复刻，后续按需判断 |
| M-047 | 角色普攻特效资源 | 已扒 | 已复现 | `attack-effects-index.md`、`hero-combat-visuals-index.md`、五角色战斗视觉索引、`Role*.normalHit()`、`Role*.doHit*()`、`BaseBitmapDataPool.as` | 五角色真资源序列、对应 CombatVisualBridge、`HeroNormalAttackSystem.ts` | Role1..Role5枪/剑/武器分支普攻真资源均已接入；白龙 `Role5runattack` 为明确原包反证并保持不可见，不用占位替代 |
...
| M-052 | 完整功能 UI | 已扒 | 部分复现 | `full-function-ui-index.md`、`stage-feature-entry-index.md`、`level-result-ui-index.md`、`equipment-workshop-index.md`、`evidence/TASK-SETTINGS-167-workshop-left-pages.md`、四份 `task-settings-167-workshop-*.json`、`evidence/TASK-SETTINGS-170B1-equipment-page.md`、`task-settings-170b1-equipment-page.json`、`equipment-visual-resource-catalog.md`、`skills-input-index.md`、`skill-ui-native-index.md`、`inventory-resource-catalog.md`、`equipment-index.md`、`pets-index.md`、`magic-weapons-index.md` 与完整 UI 资源标注 | `StageFeatureEntryRouterSystem.ts`、`StageSettingsSystem.ts`、`FormalFeatureUiEntryBridge.ts`、`FormalInventoryPageView.ts`、`EquipmentPreviewSystem.ts`、`LevelResultView.ts`、V6 与正式功能页 systems/views | 170C 已接入全装备数据、视觉目录和代表性正式穿脱，属性刷新、原操作层、关闭/再入及零 console 通过；170D 继续完成 63 对象/9 状态真值直连与五角色/双 owner/特殊资源差异证据。四事务、关卡左下五 pointer 与用户 UI 整改仍待后续 |

...
| M-035 | 资源加载策略 | 已扒 | 部分复现 | `README_extract.md`、`evb-extraction-report.md`、`modern-architecture.md`、`assets-index.md`、`projectiles-index.md`、`levels-index.md`、`stage21-monster-visuals-index.md`、`asset-annotation/batches/stage22.md` | `src/assets/AssetManifest.ts` 的状态化 manifest、Stage 1/2-1 真场景、`public/assets/stage22/` 与恢复源 SWF | Stage 2-2 的 7 条场景/机关 stable key 已由 `150A` 升级为 ready；Monster16/六攻击对象 7 条仍为 derived-ready 并留给 `150C`，Monster9/10/19 继续复用 Stage 2-1 owner |
...
| M-044 | 存档 | 已扒 | 部分复现 | `save-slots-index.md`、`skills-input-index.md`、`pets-index.md`、`levels-index.md`、`User.getSaveObj()`、`MemoryClass.setStorage()`、`MainGame.levelClear()`、`GameWin.nextClick()` | `SaveSystem.ts` V4、`SaveSlotSystem.ts`、Stage 1/2-1 result bridges、专项测试 | Stage 2-2 胜利把最高进度 2/2 推进到 2/3 并保存当前槽，失败不推进；现代 2-3 round-trip 待 `VS-056` 接入验证 |

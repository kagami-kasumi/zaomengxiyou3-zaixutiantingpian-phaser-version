...
| M-030 | 怪物系统 | 已扒 | 部分复现 | `monsters-index.md`、`levels-index.md`、`BaseMonster.as`、`BaseObject.as`、`BaseBullet.as`、`PhysicsWorld.as`、`MainGame.as`、`Monster2/3/4/5/6/7/8/9/10/19/30.as` | `MonsterPhysicsSystem.ts`、`Monster30System.ts`、`Monster3System.ts`、Stage 1 flow/gameplay bridges | Stage 2-1 五批 10/12/14/16/1 共 53 怪、6/8 上限和 Monster6 死亡显门已闭合；9/10/19/6 的 HP、攻防、奖励与首个攻击合同可实现，真动作/弹体视觉仍后置 |
...
| M-035 | 资源加载策略 | 已扒 | 部分复现 | `README_extract.md`、`evb-extraction-report.md`、`modern-architecture.md`、`attack-effects-index.md`、`assets-index.md`、`projectiles-index.md`、`combat-assets-gap-plan.md`、`crafting-ui-index.md`、`equipment-workshop-index.md`、`levels-index.md` | `src/assets/AssetManifest.ts` 的状态化 manifest、Role1 普攻真资源、炼丹炉 UI/图标真资源、Stage 1 三关真场景和恢复源 SWF | Stage 2-1 的 19/21 场景层、282 背景、3 地面、48 门和 16 冰刺已选择性派生到 Git 忽略调查目录并标为 derived-ready；49 根布局保持 export-ready，待 `TASK-SLICE-144` 精确接入 |
...
| M-044 | 存档 | 已扒 | 部分复现 | `save-slots-index.md`、`skills-input-index.md`、`pets-index.md`、`levels-index.md`、`User.getSaveObj()`、`MemoryClass.setStorage()`、`SaveInter.as` | `SaveSystem.ts` V4、`SaveSlotSystem.ts`、`TestSceneSaveBridge.ts`、专项测试 | 正式六槽与当前槽、旧单槽导入、V1/V2/V3 迁移、删除/损坏保护已复现；Stage 2-1 胜利应按通用单调进度从 2/1 推进并保存 2/2，待 `TASK-SLICE-144` 接入 |

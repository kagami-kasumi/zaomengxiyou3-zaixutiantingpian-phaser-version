...
| M-039 | 合成 | 已扒 | 部分复现 | `crafting-index.md`、`crafting-ui-index.md`、`reference/crafting-recipes-1.1.json`、`Fusion.as`、`StrengthEquipment.as`、`AllEquipment.mixProduce()`、`getSunSutraValueEquip()`、`getMingDingHuaYanEquip()`、1.0 `crafting-recipes.csv` 辅助索引 | `src/systems/CraftingRecipeRegistry.ts`、`src/systems/CraftingSystem.ts`、`src/scenes/test-scene/TestSceneUIHandlers.ts`、`tools/crafting-tests.ts` | 1.1 的 67 个现代 `direct_static`、41 个默认继承和 4 个特殊继承配方均已复现；三槽 `CraftingSession` 已覆盖装备实例/堆叠单位移入、单槽退回、关闭全退、实时预览、成功清空、失败保留和 P1/P2 隔离；全部路径保留 1000 灵魂、容量预检和失败无副作用；炼丹炉容器/合成页/五角色选择器/首配方图标已定位到 `backpack1.swf`、`OtherMat1.swf`、`EIcon1.swf` 的精确 character、帧和坐标，下一步 `TASK-SLICE-117` 接入真资源视觉；现代版不保存时装生成时间 |

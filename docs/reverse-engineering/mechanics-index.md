...
| M-052 | 完整功能 UI | 已扒 | 部分复现 | `full-function-ui-index.md`、`stage-feature-entry-index.md`、`task-settings-175c-stage-feature-host.json`、`task-settings-175d-skill-pages.json`、`level-result-ui-index.md`、`equipment-workshop-index.md`、`evidence/TASK-SETTINGS-175-functional-ui-truth-audit.md`、既有 verified 工坊/装备 manifests、技能/背包/宠物/法宝与地图服务索引 | `StageFeatureEntryRouterSystem.ts`、`StageSettingsSystem.ts`、`FormalFeatureUiEntryBridge.ts`、正式功能页 systems/views | 宠物/法宝/功能宿主/技能已有 verified 真值，但对应现代覆盖、地图态共享 chrome 或 manifest 直连仍待 180..183；丹药等其余页面仍是旧审计迁移债务，175E..I 与实现消费完成前不恢复完整原生化结论 |

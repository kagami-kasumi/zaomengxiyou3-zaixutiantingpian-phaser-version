...
| M-039 | 合成 | 已扒 | 部分复现 | `crafting-index.md`、`reference/crafting-recipes-1.1.json`、`Fusion.as`、`AllEquipment.mixProduce()`、1.0 `crafting-recipes.csv` 辅助索引 | `src/systems/CraftingRecipeRegistry.ts`、`src/systems/CraftingSystem.ts`、`src/scenes/test-scene/TestSceneUIHandlers.ts`、`tools/crafting-tests.ts` | 1.1 权威清单的 68 个 `direct_static` 源码记录已去重为 67 个现代配方；`kyg + kyz + kys -> kyl` 已验证 `getSutraValue()` 的装备实例事务及 HP/MP/攻击/防御三材料平均继承；全部路径保留 1000 灵魂门禁、容量预检、失败无副作用和 P1/P2 隔离；现代版不保存时装生成时间，下一步扩展剩余默认继承配方，特殊继承与材料暂存 UI 后置 |

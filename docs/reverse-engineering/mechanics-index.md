...
| M-035 | 资源加载策略 | 已扒 | 部分复现 | `README_extract.md`、`evb-extraction-report.md`、`modern-architecture.md`、`attack-effects-index.md`、`assets-index.md`、`projectiles-index.md`、`combat-assets-gap-plan.md` | `src/assets/AssetManifest.ts` 的状态化 manifest 骨架和项目外已恢复源 SWF | Role1/4/5 源包已恢复；下一步从一个包选择性导出并接入最小真资源族 |
...
| M-047 | 角色普攻特效资源 | 已扒 | 部分复现 | `attack-effects-index.md`、`assets-index.md`、`combat-assets-gap-plan.md`、`evb-extraction-report.md`、`Role*.normalHit()`、`Role*.doHit*()`、`BaseBitmapDataPool.as` | `src/assets/AssetManifest.ts`、`src/systems/HeroNormalAttackSystem.ts`、`src/scenes/TestScene.ts`、项目外角色源 SWF | 普攻源包已恢复；选择一个角色导出 MovieClip/位图并替换占位，白龙枪形态 `doSingleHit(...)` 语义缺口仍单独保留 |

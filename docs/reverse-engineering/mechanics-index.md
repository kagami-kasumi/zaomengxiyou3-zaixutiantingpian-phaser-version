...
| M-036 | 装备 | 已扒 | 部分复现 | `equipment-index.md`、`equipment-workshop-index.md`、`magic-weapons-index.md`、`my/AllEquipment.as`、`my/MyEquipObj.as`、`export/pack/BackPack.as`、`export/strength/Strength/Resolution/Making/SutraInterface.as`、`base/BaseRoleProperies.as` | `EquipmentSystem.ts`、`EquipmentStrengtheningSystem.ts`、`EquipmentResolutionSystem.ts`、`EquipmentMakingSystem.ts`、正式背包/工坊/法宝页面 | 六槽穿脱、角色限制、实例成长、随机事务、78 本可达制作书、双 owner 与保存基础已有；用户将“全装备 UI 与数值”重新提为未完成范围，完整 1.1 可穿戴全集、基础数值和逐项 UI 回放待 `LINE-CORE-PROGRESSION-COMPLETION` 闭合 |
...
| M-052 | 完整功能 UI | 已扒 | 部分复现 | `full-function-ui-index.md`、`stage-feature-entry-index.md`、`level-result-ui-index.md`、`equipment-workshop-index.md`、`skills-input-index.md`、`skill-ui-native-index.md`、`inventory-resource-catalog.md`、`equipment-index.md`、`pets-index.md`、`magic-weapons-index.md` 与完整 UI 资源标注 | `StageFeatureEntryRouterSystem.ts`、`StageSettingsSystem.ts`、`FormalFeatureUiEntryBridge.ts`、`StageSettingsScene.ts`、`LevelResultView.ts`、V6 与正式功能页 systems/views | 165D 右栏背包投影保持有效；用户复验将炼丹炉左侧四页、关卡左下五 pointer 入口和部分 UI 重做重新列为未完成，旧的“缺口清零”结论降级，以 `LINE-CORE-PROGRESSION-COMPLETION` 的新证据为准 |

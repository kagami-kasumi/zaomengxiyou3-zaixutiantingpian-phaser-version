...
| M-036 | 装备 | 已扒 | 已复现 | `equipment-index.md`、`equipment-workshop-index.md`、`magic-weapons-index.md`、`my/AllEquipment.as`、`my/MyEquipObj.as`、`export/pack/BackPack.as`、`export/strength/Strength/Resolution/Making/SutraInterface.as`、`base/BaseRoleProperies.as` | `EquipmentSystem.ts`、`EquipmentStrengtheningSystem.ts`、`EquipmentResolutionSystem.ts`、正式背包/工坊页面 | 真背包页已支持 P1/P2 六槽穿脱、角色限制、属性预览和 V4 重载；真强化页与真分解页已闭合实例成长、概率/灵魂/降级/保底、品质随机产物和双 owner 保存，制作、五行重置与完整 1.1 装备表仍按后续任务闭合 |
...
| M-039 | 合成 | 已扒 | 已复现 | `crafting-index.md`、`crafting-ui-index.md`、`reference/crafting-recipes-1.1.json`、`reference/crafting-item-catalog-1.1.json`、`reference/crafting-icon-catalog-1.1.json`、`Fusion.as`、`StrengthEquipment.as`、`AllEquipment.mixProduce()`、`getSunSutraValueEquip()`、`getMingDingHuaYanEquip()` | `src/systems/CraftingRecipeRegistry.ts`、`src/systems/CraftingItemDefinitionRegistry.ts`、`src/systems/CraftingSystem.ts`、`src/systems/FormalWorkshopPageSystem.ts`、`src/scenes/feature-ui/FormalWorkshopPageView.ts`、`tools/crafting-tests.ts`、`tools/formal-workshop-host-tests.ts` | 112 配方、201 个物品定义、201/201 真图标与 17 条预览别名全部接入；112×P1/P2 共 224 条事务外，正式工坊 host 已闭合双 owner、暂存撤回、切页/换人/关闭返还和当前槽保存 |
...
| M-052 | 完整功能 UI | 部分已扒 | 部分复现 | `full-function-ui-index.md`、`equipment-workshop-index.md`、`skills-input-index.md`、`equipment-index.md`、`pets-index.md`、`magic-weapons-index.md` 与 11 条完整 UI 资源标注 | 共享 `FeatureUiHost`、V4、正式背包、技能、宠物、工坊/Fusion、强化与分解页面 systems/views | 共享 host、双玩家 V4、真背包/装备、技能四页、宠物页、工坊/Fusion、真强化与真分解页已完成；制作行为已扒，当前推进 `GOAL-004` / `TASK-SLICE-138D` |

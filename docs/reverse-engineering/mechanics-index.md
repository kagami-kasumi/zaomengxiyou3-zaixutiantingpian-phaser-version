...
| M-035 | 资源加载策略 | 已扒 | 部分复现 | `README_extract.md`、`modern-architecture.md`、`attack-effects-index.md`、`assets-index.md`、`projectiles-index.md`、`combat-assets-gap-plan.md` | `src/assets/AssetManifest.ts` 的状态化 manifest 骨架和五角色战斗占位 key 缺口登记 | 已确认 Role1/4/5 战斗真素材均不在当前导出资源中；需用户补 `WuKong` / `Role1Effect`、`ShaShen`、`bailongSword` 等角色包后再拆最小接入切片 |
...
| M-039 | 合成 | 未扒 | 未复现 | `docs/reverse-engineering/reference/equipment-spreadsheet/crafting-recipes.csv`、`export/strength/` 待读 | 无 | `TASK-SETTINGS-041`：用拆分 CSV 辅助定位配方，再按 AS3 合成 UI 校验 1.1 事实 |
...
| M-047 | 角色普攻特效资源 | 已扒 | 部分复现 | `attack-effects-index.md`、`assets-index.md`、`combat-assets-gap-plan.md`、`Role*.normalHit()`、`Role*.doHit*()`、`BaseBitmapDataPool.as`、`extracted_flash/resources_by_swf` | `src/assets/AssetManifest.ts`、`src/systems/HeroNormalAttackSystem.ts`、`src/scenes/TestScene.ts` | Role1/4/5 普攻占位 key 到 AS3 源符号的清单已闭合；当前导出均缺真素材，白龙枪形态 `doSingleHit(...)` 源符号仍需 P-code 或更完整导出 |

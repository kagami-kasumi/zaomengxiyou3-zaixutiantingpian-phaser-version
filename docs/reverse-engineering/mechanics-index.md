...
| M-035 | 资源加载策略 | 已扒 | 已复现 | `README_extract.md`、`evb-extraction-report.md`、`modern-architecture.md`、`assets-index.md`、`projectiles-index.md`、`levels-index.md`、`stage21-monster-visuals-index.md`、`asset-annotation/batches/stage22.md`、`PG-009` | `AssetManifest.ts` provenance、`SceneAssetBundles.ts` 唯一 owner、`AssetBundleCoordinator.ts`、`SceneAssetBundleBridge.ts`、场景路由与负向门禁 | Boot 仅加载 3 张 shell 图；地图、功能 UI、Stage 1/2 与 DEV/QA 直达按 bundle 首次 ensure，返回再进幂等，失败可重试；不含纹理淘汰或图片转码 |
...
| M-037 | 背包 | 已扒 | 部分复现 | `inventory-resource-catalog.md`、`reference/inventory-resource-catalog-1.1.json`、`equipment-index.md`、`user/User.as`、`export/pack/BackPackElement.as`、`export/pack/PackThings.as`、`config/Config.as` | `InventorySystem.ts`、`FormalInventoryPageSystem.ts`、`FeatureUiScene.ts` | `TASK-SETTINGS-070` 已闭合原版 1.1 的 431 个有效稳定身份：装备 164、道具 235、时装 20、技能书 12；428 项精确真图标与 3 项原版缺陷已逐项落 provenance，实例/堆叠、25 格分页、99 上限、生产者/消费者和存档语义均有机器目录。现代实现仍只覆盖其中 201 项，现由 `TASK-SLICE-160 / VS-064` 接入全量目录、事务、双 owner 与 V6 往返 |
...
| M-052 | 完整功能 UI | 部分已扒 | 部分复现 | `full-function-ui-index.md`、`equipment-workshop-index.md`、`skills-input-index.md`、`skill-ui-native-index.md`、`equipment-index.md`、`pets-index.md`、`magic-weapons-index.md` 与 11 条完整 UI 资源标注 | 共享 `FeatureUiHost`、V6、正式背包、技能、宠物、工坊四事务与法宝页面 systems/views | 技能 250/868/417/213 已按 PG-007/009 完成静态/动态 child 分离、五角色心法、P1/P2、页/角色级加载；技能/工坊灵魂余额共享同一动态组件并在跨页时按当前存档刷新，证据见 `TASK-SLICE-154-visual-audit.md`。其他存量页面继续由 PG-007 追踪 |

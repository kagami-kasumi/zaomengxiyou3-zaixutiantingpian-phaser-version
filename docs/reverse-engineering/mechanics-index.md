...
| M-034 | 子弹/技能飞行物 | 已扒 | 部分复现 | `projectiles-index.md`、五角色战斗视觉索引、Stage 1/2怪物视觉索引、`BaseBullet.as`、`export/bullet/`、`Role1.as` 至 `Role5.as` | `ProjectileSystem.ts`、角色技能 systems、五角色/Stage 1/2 visual systems 与 bridges、`AssetManifest.ts` | 173A 已闭合 Role1Shadow `hit1/hit2` 仅由 source `lyfb/zz` 达到，并在正式共享 Runtime 按证据 tick 于分身位置派生、结算和销毁；未达遗留对象、宠物和后续关卡仍使全局状态为部分复现，Role5 二维命中/共享结算由 174 继续 |

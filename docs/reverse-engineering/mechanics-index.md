...
| M-035 | 资源加载策略 | 已扒 | 部分复现 | `README_extract.md`、`evb-extraction-report.md`、`modern-architecture.md`、`attack-effects-index.md`、`assets-index.md`、`projectiles-index.md`、`combat-assets-gap-plan.md`、`crafting-ui-index.md`、`equipment-workshop-index.md`、`levels-index.md` | `src/assets/AssetManifest.ts` 的状态化 manifest、Role1 普攻真资源、炼丹炉 UI/图标真资源、Stage 1-1/1-2/1-3 真场景资源和项目外已恢复源 SWF | character 119 的按钮 95/99/109/113 已完成四态选择性派生与几何复核；现有扁平 container PNG 含 up 字形，后续须派生无按钮背景并独立组合原生状态，未扩张无关资源 |
...
| M-052 | 完整功能 UI | 部分已扒 | 部分复现 | `full-function-ui-index.md`、`equipment-workshop-index.md`、`skills-input-index.md`、`equipment-index.md`、`pets-index.md`、`magic-weapons-index.md` 与 11 条完整 UI 资源标注 | 共享 `FeatureUiHost`、V4、正式背包、技能、宠物、工坊四事务与法宝页面 systems/views | `TASK-SETTINGS-060` 已闭合左到右“强化 / 合成 / 分解 / 打造”原生按钮的四态、命中区、坐标和路由；当前执行 `TASK-SLICE-141` 移除现代覆盖导航，再恢复 `TASK-SLICE-140` 端到端关闭 |

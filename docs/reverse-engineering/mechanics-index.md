...
| M-026 | 关卡类命名 | 已扒 | 部分复现 | `levels-index.md`、`gameplay-index.md`、`MainGame.newGame()`、`PhysicsWorld.pWorldInit()`、RegiMA `assets/levels/level11.swf`、`level12.swf`、`level13.swf`、`level21.swf`、`level22.swf` 与 `assets/1.swf` / `2.swf` | `AssetManifest.ts`、Stage 1/2-1 layouts 与正式 scene bridges、`Stage22WorldBridge.ts` | Stage 2-2 的 `sl22`、character 34/36、279/3、31/63 已由 `TASK-SLICE-150A` 接入本地 DEV 验收入口；正式 2/2 路由留给 `150B` |
| M-027 | 地图标记 | 已扒 | 部分复现 | `levels-index.md`、`BaseGameSence.as`、`PhysicsWorld.addSubObj()`、`StopPoint.as`、`MonsterAppearPoint.as`、`ViewControllor.as`、`BaseHero.as`、恢复 `sl12/sl13/sl21/sl22.as` | `src/systems/LevelSystem.ts`、Stage 1 layout/traversal systems、`Stage21Layout.ts`、`Stage21TraversalSystem.ts`、`Stage22Layout.ts`、`Stage22TraversalSystem.ts` | Stage 2-2 的 3+1 墙、3 单向平台、5 停点、9 火焰、左右边界和地面顶已接入并完成 940×590 局部验收；25 刷怪点与停点流程留给 `150B` |

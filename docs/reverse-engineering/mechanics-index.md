...
| M-024 | 角色组合键 | 已扒 | 部分复现 | `roles-index.md`、`skills-input-index.md`、`Role*.myKeyDown()`、`controls-index.md` | `Role2PassiveSkillSystem.ts`、`HeroNormalAttackSystem.ts` | Role2 的持续普攻蓄力 `blb` 与 `sjt` 阈值已复现；其他角色组合键仍随各角色任务推进 |
| M-025 | 角色技能效果 | 已扒 | 部分复现 | `skills-input-index.md`、`roles-index.md`、`projectiles-index.md`、`Role*.showSkill()`、`Config.allSklName` | `HeroSkillSystem.ts`、`Role2*SkillSystem.ts`、`ProjectileSystem.ts`、`TestSceneRole2SkillBridge.ts` | Role2 十项学习树技能的主动/被动行为已完整复现；M-025 保持部分复现仅因 Role1/3/4/5 尚未完成 |
...
| M-034 | 子弹/技能飞行物 | 部分已扒 | 部分复现 | `projectiles-index.md`、`BaseBullet.as`、`export/bullet/`、`Role2.as`、`Role1.as` 至 `Role5.as`、`MagicPearl.as`、`MagicZLHummer.as`、`Ling.as`、`MagicBigBottle.as` | `ProjectileSystem.ts`、`Role2XbzSkillSystem.ts`、`Role2JhsjSkillSystem.ts`、`TestScene.ts`、`AssetManifest.ts` | Role2 `Bullet3/4/5/6/7/8/9` 与分身变体均已用稳定占位 key 复现；其他角色、真素材与通用 projectile 扩展继续后置 |

...
| M-036 | 装备 | 已扒 | 部分复现 | `equipment-index.md`、`equipment-workshop-index.md`、`evidence/TASK-SETTINGS-167-workshop-left-pages.md`、四份 `task-settings-167-workshop-*.json`、`magic-weapons-index.md`、`my/AllEquipment.as`、`my/MyEquipObj.as`、`export/pack/BackPack.as`、`export/strength/Strength/Resolution/Making/SutraInterface.as`、`base/BaseRoleProperies.as` | `EquipmentSystem.ts`、`EquipmentStrengtheningSystem.ts`、`EquipmentResolutionSystem.ts`、`EquipmentMakingSystem.ts`、正式背包/工坊/法宝页面 | 168A 已原生化强化/合成的动态 child、原字段、按钮态与全局反馈；168B 继续分解/打造联合校准。完整 1.1 可穿戴全集、基础数值和逐项 UI 回放仍待本线后续闭合 |
...
| M-039 | 合成 | 已扒 | 部分复现 | `crafting-index.md`、`crafting-ui-index.md`、`equipment-workshop-index.md`、`evidence/TASK-SETTINGS-167-workshop-left-pages.md`、四份 `task-settings-167-workshop-*.json`、`reference/crafting-recipes-1.1.json`、`Fusion.as`、`Making.as`、`AllEquipment.as` | `CraftingRecipeRegistry.ts`、`CraftingItemDefinitionRegistry.ts`、`CraftingSystem.ts`、`EquipmentMakingRegistry.ts`、`EquipmentMakingSystem.ts`、正式工坊页面与专项 | Fusion 112 配方/201 定义/201 真图标及 224 条 P1/P2 事务基础已有；168A 已闭合合成三材料、预览/产物、原字段、按钮态和全局反馈。全量完成仍依赖 168B 与后续完整装备数值，收口前不使用“已复现”结论 |
...
| M-052 | 完整功能 UI | 已扒 | 部分复现 | `full-function-ui-index.md`、`stage-feature-entry-index.md`、`level-result-ui-index.md`、`equipment-workshop-index.md`、`evidence/TASK-SETTINGS-167-workshop-left-pages.md`、四份 `task-settings-167-workshop-*.json`、`skills-input-index.md`、`skill-ui-native-index.md`、`inventory-resource-catalog.md`、`equipment-index.md`、`pets-index.md`、`magic-weapons-index.md` 与完整 UI 资源标注 | `StageFeatureEntryRouterSystem.ts`、`StageSettingsSystem.ts`、`FormalFeatureUiEntryBridge.ts`、`StageSettingsScene.ts`、`LevelResultView.ts`、V6 与正式功能页 systems/views | 165D 右栏背包投影保持有效；168A 已删除强化/合成页底摘要并直连 verified 真值恢复原字段、动态 child、按钮态和全局反馈。分解/打造 Arial/摘要差异转入 168B；关卡左下五 pointer 与用户确认 UI 整改仍待后续复验 |

...
| M-042 | 宠物 | 已扒 | 部分复现 | `pets-index.md`、`BasePet`、`BaseHero.initPet()`、`User.petsAry`、`User.findCurrentPet()`、`User.catchNewPet()`、`PetInfo.as`、`PetInterface.as`、`PackThings.as`、`MagicBottle.as`、`Monster70.as` 至 `Monster78.as` | `src/systems/PetSystem.ts`、`src/scenes/TestScene.ts` | 已复现首个最小切片：P1 预置一只宠物、B 键宠物面板、单只出战/休息、跟随和远距传送；宣花葫芦捕捉链路、Monster70-78 映射、灵魂消耗、概率和 `cwzb` 边界已扒清，可拆首个捕捉切片；宠物道具消耗、成长洗练、全部技能、P2/联机和真实资源后置 |
| M-043 | 法宝 | 部分已扒 | 未复现 | `magic-weapons-index.md`、`BaseMagicWeapon.as`、`MagicBottle.as`、`BaseHero.initMagicWeapon()`、`BaseHero.showSkillFaBao()`、`KeyBoardControl.as`、`AllEquipment.as`、`BackPack.as` | 无 | 宣花葫芦捕捉入口已够支撑首个 `MagicBottle` 捕捉切片；完整法宝技能表、法宝强化 UI、其他法宝和联机同步后续另拆 |

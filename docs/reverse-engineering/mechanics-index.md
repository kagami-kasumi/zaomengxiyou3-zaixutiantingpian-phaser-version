...
| M-016 | UI 快捷键 | 已扒 | 部分复现 | `stage-feature-entry-index.md`、`skills-input-index.md`、`pets-index.md`、`KeyBoardControl.as`、`GameInfo.as`、`RoleInfo.as`、`magic-weapons-index.md` | `FeatureUiHostSystem.ts`、`FormalFeatureUiEntryBridge.ts`、`FeatureUiScene.ts`、现有功能 systems | 原版五个 HUD pointer 与 P1 `C/V/B/N/Esc`、P2 `/ * -` 已闭合：P2 无法宝/设置快捷键，P2 `*` 仍默认 P1 技能选择，Escape 只切设置；现代键盘/暂停基础已存在，但缺真 HUD pointer/SetMenu，并错误保留通用跨页与 Escape 关闭，按 `TASK-SLICE-156A..C` 整改 |
...
| M-030 | 怪物系统 | 已扒 | 部分复现 | `monsters-index.md`、`levels-index.md`、`stage21-monster-visuals-index.md`、`BaseMonster.as`、`BaseObject.as`、`BaseBullet.as`、`PhysicsWorld.as`、`MainGame.as`、`Monster2/3/4/5/6/7/8/9/10/16/19/30.as` | `MonsterPhysicsSystem.ts`、`MonsterDefeatRewardSystem.ts`、Stage 1 bridges、Stage 2-1 native visual owners、`Stage22FlowSystem.ts` / `Stage22Monster16VisualSystem.ts` | Stage 2-2 的 53 只 Monster9/10/19 与 Monster16 24189 HP、8 动作/36 帧、六攻击 104 帧、共享物理/奖励、幂等死亡显门及 1P/2P 运行复验已闭合 |
...
| M-034 | 子弹/技能飞行物 | 已扒 | 部分复现 | `projectiles-index.md`、`role5-combat-index.md`、`stage21-monster-visuals-index.md`、`BaseBullet.as`、`export/bullet/`、`Role1.as` 至 `Role5.as`、`Role1Shadow.as`、`MagicPearl.as`、`MagicZLHummer.as`、`Ling.as`、`MagicBigBottle.as` | `ProjectileSystem.ts`、`HeroNormalAttackSystem.ts`、`Role1BasicSkillSystem.ts`、`Role1SkillProjectileFactory.ts`、`Role1ShadowSkillSystem.ts`、`Role1FinisherSkillSystem.ts`、`Role2*SkillSystem.ts`、`Role3*SkillSystem.ts`、`Role4*SkillSystem.ts`、`Role5SkillSystem.ts`、`Stage21MonsterVisualSystem.ts`、`Stage21MonsterVisualBridge.ts`、`AssetManifest.ts` | 五角色 projectile/特殊对象等价行为已复现；Stage 2-1 七攻击对象 132 帧、注册点、触发 tick/数量/偏移与生命周期已接入并运行复验；全局真实素材和通用弹体库仍未完全补齐 |
| M-035 | 资源加载策略 | 已扒 | 已复现 | `README_extract.md`、`evb-extraction-report.md`、`modern-architecture.md`、`assets-index.md`、`projectiles-index.md`、`levels-index.md`、`stage21-monster-visuals-index.md`、`asset-annotation/batches/stage22.md`、`asset-annotation/batches/inventory-items.md`、`PG-009` | `AssetManifest.ts` provenance、`SceneAssetBundles.ts` 唯一 owner、`AssetBundleCoordinator.ts`、`SceneAssetBundleBridge.ts`、场景路由与负向门禁 | Boot 仅加载 3 张 shell 图；地图、功能 UI、Stage 1/2 与 DEV/QA 直达按 bundle 首次 ensure，返回再进幂等，失败可重试；背包的 428 图标与 22 个原生控件由唯一 `feature-ui-backpack` bundle 首次进入懒加载，不含纹理淘汰或图片转码 |
...
| M-052 | 完整功能 UI | 已扒 | 部分复现 | `full-function-ui-index.md`、`stage-feature-entry-index.md`、`equipment-workshop-index.md`、`skills-input-index.md`、`skill-ui-native-index.md`、`inventory-resource-catalog.md`、`equipment-index.md`、`pets-index.md`、`magic-weapons-index.md` 与完整 UI 资源标注 | 共享 `FeatureUiHost`、V6、正式背包、技能、宠物、工坊四事务与法宝页面 systems/views | 页面内部业务与资源合同已闭合；关卡 host 的 574 五入口、371 设置、444 帮助、owner/门禁/单页互斥也已完成逆向，但现代仍缺 pointer/settings，并存在原版没有的可见通用 host 与跨页/Escape 语义，VS-060 完成后才能升级为关卡入口复现 |

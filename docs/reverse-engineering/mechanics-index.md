...
| M-014 | 上/交互/通关 | 已扒 | 部分复现 | `levels-index.md`、`BaseHero.checkTransferDoor()`、`Role*.myKeyDown()`、`MainGame.levelClear()`、2026-07-26 用户试玩反馈 | `LevelSystem.ts`、`Stage11FlowSystem.ts`、`Stage12/13/21/22FlowSystem.ts` 与五关 bridges | 原版光门 + 上键合同已确认，但现代五关分别实现通关/判负/解锁，1-1 又出现无法稳定以光门 + W 通关的回归；由 `GOAL-064 / TASK-ARCH-015 / PG-012` 建立全部后续关卡复用的通用协议，以五关迁移和新关卡防复制门禁验收 |
...
| M-026 | 关卡类命名 | 已扒 | 部分复现 | `levels-index.md`、`gameplay-index.md`、`MainGame.newGame()`、`PhysicsWorld.pWorldInit()`、RegiMA `assets/levels/level11.swf`、`level12.swf`、`level13.swf`、`level21.swf`、`level22.swf` 与 `assets/1.swf` / `2.swf` | `AssetManifest.ts`、Stage 1/2-1 layouts 与正式 scene bridges、`Stage22WorldBridge.ts`、`Stage22Scene.ts` | Stage 2-2 的 `sl22`、character 34/36、279/3、31/63、正式 2/2 地图/场景/结果路由及全程运行复验已闭合；其他关卡仍按内容线推进 |
...
| M-028 | 第一个关卡 | 已扒 | 已复现 | `levels-index.md`、`Config.initData()`、`StageListener11.as`、`Monster3.as`、`BaseHero.as`、`GameWin.as`、`GameFail.as`、用户 2026-07-23 试玩反馈 | `Monster3System.ts`、`LevelSystem.ts`、`Stage11FlowSystem.ts`、`Stage11EntryScene.ts`、`TestSceneStage11FlowBridge.ts` | `TASK-SLICE-147/148` 修复镜头推进、调试靶、地图重入、原版 W 门与最高层 Boss 门禁；`TASK-SLICE-149` 按 `-1950 + 2370 = 420` 恢复最高层 420/590 构图和 2 秒镜头过渡 |
...
| M-030 | 怪物系统 | 已扒 | 部分复现 | `monsters-index.md`、`levels-index.md`、`stage1-monster-visuals-index.md`、`stage21-monster-visuals-index.md`、`BaseMonster.as`、`BaseObject.as`、`BaseBullet.as`、`PhysicsWorld.as`、`MainGame.as`、`Monster2/3/4/5/6/7/8/9/10/16/19/30.as` | `MonsterPhysicsSystem.ts`、`MonsterDefeatRewardSystem.ts`、`Stage11MonsterVisualSystem.ts`、`Stage11MonsterVisualBridge.ts`、Stage 1 bridges、Stage 2-1 native visual owners、`Stage22FlowSystem.ts` / `Stage22Monster16VisualSystem.ts` | 五个已完成关卡的实际怪物全集已盘清；Stage 1-1 的 Monster30/3 共 40 个本体独立视觉帧、3 个攻击对象 25 帧与碰撞根已接入，Stage 1-2/1-3 仍待逐关接入；Stage 2-1/2-2 既有真视觉保持闭合 |
...
| M-034 | 子弹/技能飞行物 | 已扒 | 部分复现 | `projectiles-index.md`、`role5-combat-index.md`、`stage1-monster-visuals-index.md`、`stage21-monster-visuals-index.md`、`BaseBullet.as`、`export/bullet/`、`Role1.as` 至 `Role5.as`、`Role1Shadow.as`、`MagicPearl.as`、`MagicZLHummer.as`、`Ling.as`、`MagicBigBottle.as` | `ProjectileSystem.ts`、`HeroNormalAttackSystem.ts`、`Role1BasicSkillSystem.ts`、`Role1SkillProjectileFactory.ts`、`Role1ShadowSkillSystem.ts`、`Role1FinisherSkillSystem.ts`、`Role2*SkillSystem.ts`、`Role3*SkillSystem.ts`、`Role4*SkillSystem.ts`、`Role5SkillSystem.ts`、`Stage11MonsterVisualSystem.ts`、`Stage11MonsterVisualBridge.ts`、`Stage21MonsterVisualSystem.ts`、`Stage21MonsterVisualBridge.ts`、`AssetManifest.ts` | Stage 1-1 的 3/16 个攻击对象、25/171 帧已按原触发 tick、生成点、注册边界、镜像与末帧生命周期接入，Stage 1-2/1-3 余 13 个对象待接入；Stage 2-1 七对象 132 帧保持已接入；角色 projectile 真视觉仍由 VS-062 后续盘清 |
| M-035 | 资源加载策略 | 已扒 | 已复现 | `README_extract.md`、`evb-extraction-report.md`、`modern-architecture.md`、`assets-index.md`、`projectiles-index.md`、`levels-index.md`、`stage1-monster-visuals-index.md`、`stage21-monster-visuals-index.md`、`asset-annotation/batches/stage1-monsters.md`、`asset-annotation/batches/stage22.md`、`asset-annotation/batches/inventory-items.md`、`PG-009` | `AssetManifest.ts` provenance、`SceneAssetBundles.ts` 唯一 owner、`AssetBundleCoordinator.ts`、`SceneAssetBundleBridge.ts`、场景路由与负向门禁 | Boot 仅加载 shell；地图、功能 UI、Stage 1/2 与 DEV/QA 直达按 bundle ensure；Stage 1 怪物 26 条 stable key 中 Stage 1-1 五条已由唯一 `stage-11` owner 接入，余 21 条待 157B/157C 接入并由 157D 审计最终共享 owner；不含纹理淘汰或图片转码 |
...
| M-044 | 存档 | 已扒 | 部分复现 | `save-slots-index.md`、`save-party-flow-index.md`、`skills-input-index.md`、`pets-index.md`、`levels-index.md`、`inventory-resource-catalog.md`、`User.getSaveObj()`、`MemoryClass.setStorage()`、`MainGame.levelClear()`、`GameWin.nextClick()` | `SaveSystem.ts` V6、`PartyConfigurationSystem.ts`、`SaveSlotSystem.ts`、`FormalPartyRuntimeSystem.ts`、`PlayerSoulSystem.ts`、正式 result bridges、专项测试 | V5 party、原子建槽和正式 owner 已闭合；V6 完成 431 身份的 P1/P2 背包实例/堆叠往返，旧 V6 三个历史兼容 id 通过显式适配器读取，未知 id 拒读、损坏 inventory 子域安全回退；技能、工坊、法宝与灵魂 owner 继续共享。整体仍为“部分复现”只因更广泛原版字段/系统未全部进入现代范围 |
...
| M-049 | 正式战斗 HUD | 已扒 | 已复现 | `combat-hud-index.md`、`Stage1CombatHudSystem.ts`、`Stage1CombatHudBridge.ts`、恢复 `OtherMat1/bossblood.swf` | Stage 1 三关共享 snapshot/bridge；P1/P2 独立成长与技能状态；Boss 即时/0.8s 追赶条；12 条真资源 ready | 940×590 固定层、P1/P2 镜像/五槽键位、HP/MP/经验/等级/技能/入口与重要敌人状态已由专项测试和三关浏览器验收闭合 |
...
| M-052 | 完整功能 UI | 已扒 | 部分复现 | `full-function-ui-index.md`、`stage-feature-entry-index.md`、`equipment-workshop-index.md`、`skills-input-index.md`、`skill-ui-native-index.md`、`inventory-resource-catalog.md`、`equipment-index.md`、`pets-index.md`、`magic-weapons-index.md` 与完整 UI 资源标注 | `StageFeatureEntryRouterSystem.ts`、`StageSettingsSystem.ts`、`FormalFeatureUiEntryBridge.ts`、`StageSettingsScene.ts`、V6 与正式功能页 systems/views | 页面业务、574 五真 pointer、371/444、全局 owner、固定 HUD 命中、单页互斥/同键/Escape/返回及五关校准已闭合，战斗可见通用 host/跨页/workshop 为零；M-052 因本线后续怪物/角色视觉与最终旅程仍保持部分复现 |

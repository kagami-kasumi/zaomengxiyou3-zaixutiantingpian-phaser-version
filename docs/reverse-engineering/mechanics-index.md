...
| M-038 | 掉落 | 已扒 | 已复现 | `drops-index.md`、`BaseMonster.as`、`BaseAura.as`、`my/FallEquipObj.as`、`SmallHP.as`、`BigHP.as`、`SmallMP.as`、`Monster3.as`、`Monster7.as` 至 `Monster30.as` | `src/systems/DropSystem.ts`、`src/systems/InventorySystem.ts`、`src/scenes/TestScene.ts` | `VS-009` 最小装备/道具掉落拾取和药品即时恢复已完成；aura、强化石和完整怪物掉落表仍需拆小切片，下一步 `TASK-SLICE-016` 只实现 aura 收集反馈 |

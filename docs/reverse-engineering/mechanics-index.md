...
| M-018 | Role1 悟空 | 已扒 | 已复现 | `roles-index.md`、`skills-input-index.md`、`projectiles-index.md`、`role1-combat-visuals-index.md`、`Role1.as`、`Role1Shadow.as`、`BaseRoleProperies.as` | `Role1BasicSkillSystem.ts`、`Role1SkillProjectileFactory.ts`、`Role1ShadowSkillSystem.ts`、`Role1FinisherSkillSystem.ts`、`TestSceneRole1SkillBridge.ts`、独立测试 | 行为已复现；069A 已闭合本体/装备/影分身/普攻/技能 Symbol、帧序、几何和生命周期，视觉接入仍由 158A 承担 |
...
| M-029 | 世界主循环 | 已扒 | 部分复现 | `runtime-index.md`、五关 Scene/World/Gameplay/Flow、PG-013 | `src/scenes/test-scene/TestSceneUpdatePipeline.ts`、`src/core/GameContext.ts`、五关 gameplay bridges | TestScene pipeline 与薄 GameContext 已存在，但正式关卡共同初始化/调度/退出仍按关复制；当前优先执行 TASK-ARCH-016A..D，以组合式 Runtime 分批收敛，专属遭遇/机关继续使用窄 adapter |
...
| M-047 | 角色普攻特效资源 | 已扒 | 部分复现 | `attack-effects-index.md`、`assets-index.md`、`combat-assets-gap-plan.md`、`evb-extraction-report.md`、`hero-combat-visuals-index.md`、`role1-combat-visuals-index.md`、`Role*.normalHit()`、`Role*.doHit*()`、`BaseBitmapDataPool.as` | Role1 `Role1Bullet1/3/4/5` 真资源序列、`src/assets/AssetManifest.ts`、`HeroNormalAttackSystem.ts`、测试场景 | Role1 四组对象已接并由 069A 补齐原版创建 tick、bounds 与本体动作合同；158A 负责本体/装备同步，Role2..5 仍待逐角色闭合 |

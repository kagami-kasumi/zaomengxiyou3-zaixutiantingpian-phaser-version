...
| M-026 | 关卡类命名 | 已扒 | 部分复现 | `levels-index.md`、`gameplay-index.md`、`MainGame.newGame()`、`PhysicsWorld.pWorldInit()`、RegiMA `assets/levels/level11.swf`、`assets/levels/level12.swf`、`assets/levels/level13.swf` 与 `assets/1.swf` | `AssetManifest.ts`、`Stage11Layout.ts`、`Stage12Layout.ts`、`Stage13Layout.ts`、Stage 1 scene bridges、`Stage11EntryScene.ts` | Stage 1-1/1-2/1-3 已接入；1-3 使用 `sl13` character 41、前景 13、`bg13` 119、门 40 和正式 1P/2P 流程 |
| M-027 | 地图标记 | 已扒 | 部分复现 | `levels-index.md`、`BaseGameSence.as`、`PhysicsWorld.addSubObj()`、`StopPoint.as`、`MonsterAppearPoint.as`、`ViewControllor.as`、`BaseHero.as`、恢复 `sl12.as` / `sl13.as` | `src/systems/LevelSystem.ts`、Stage 1 layout/traversal systems | Stage 1-3 的 3 个 ObsWall、1 个坠落墙、5 停点、14 刷怪点和普通门已复现，地面顶为 `y=501.0003962`；其他关卡仍后置 |
| M-028 | 第一个关卡 | 已扒 | 已复现 | `levels-index.md`、`Config.initData()`、`StageListener11.as`、`Monster3.as`、`BaseHero.as`、`GameWin.as`、`GameFail.as` | `Monster3System.ts`、`LevelSystem.ts`、`Stage11FlowSystem.ts`、`Stage11EntryScene.ts`、`TestSceneStage11FlowBridge.ts` | 关卡内流程、真场景、2.5 秒统一全灭检查、失败/胜利结果、重玩/返回和运行时验收全部完成，LINE-STAGE-1-1 已关闭 |
...
| M-030 | 怪物系统 | 已扒 | 部分复现 | `monsters-index.md`、`levels-index.md`、`BaseMonster.as`、`BaseObject.as`、`BaseBullet.as`、`PhysicsWorld.as`、`MainGame.as`、`Monster2/3/4/5/7/8/30.as` | `MonsterPhysicsSystem.ts`、`Monster30System.ts`、`Monster3System.ts`、Stage 1 flow/gameplay bridges | Stage 1 三关地面怪默认受共享重力并落到关卡平台，Monster30 仅以显式 `flying` 豁免；波次、上限和 boss 门已复现，怪物真素材与完整 AI 仍后置 |
...
| M-035 | 资源加载策略 | 已扒 | 部分复现 | `README_extract.md`、`evb-extraction-report.md`、`modern-architecture.md`、`attack-effects-index.md`、`assets-index.md`、`projectiles-index.md`、`combat-assets-gap-plan.md`、`crafting-ui-index.md`、`levels-index.md` | `src/assets/AssetManifest.ts` 的状态化 manifest、Role1 普攻真资源、炼丹炉 UI/图标真资源、Stage 1-1/1-2/1-3 真场景资源和项目外已恢复源 SWF | Stage 1-3 character 13/119/40 已选择性接入，源包、组合层级、几何、PNG 和 stable key 可追溯；未扩张怪物/角色资源 |
| M-036 | 装备 | 已扒 | 已复现 | `equipment-index.md`、`equipment-workshop-index.md`、`magic-weapons-index.md`、`my/AllEquipment.as`、`my/MyEquipObj.as`、`export/pack/BackPack.as`、`export/strength/Strength/Resolution/Making/SutraInterface.as`、`base/BaseRoleProperies.as` | `EquipmentSystem.ts`、`FormalInventoryPageSystem.ts`、`FeatureUiScene.ts` | 真背包页已支持 P1/P2 六槽穿脱、角色限制、属性预览和 V4 重载；工坊强化/分解/制作行为已扒，待 `TASK-SLICE-138A..138D` 分步复现，五行重置与完整 1.1 装备表仍按后续任务闭合 |
...
| M-039 | 合成 | 已扒 | 已复现 | `crafting-index.md`、`crafting-ui-index.md`、`reference/crafting-recipes-1.1.json`、`reference/crafting-item-catalog-1.1.json`、`reference/crafting-icon-catalog-1.1.json`、`Fusion.as`、`StrengthEquipment.as`、`AllEquipment.mixProduce()`、`getSunSutraValueEquip()`、`getMingDingHuaYanEquip()` | `src/systems/CraftingRecipeRegistry.ts`、`src/systems/CraftingItemDefinitionRegistry.ts`、`src/systems/CraftingSystem.ts`、`src/scenes/test-scene/TestSceneCraftingEntryBridge.ts`、`src/scenes/test-scene/TestSceneCraftingView.ts`、`src/scenes/test-scene/TestSceneUIHandlers.ts`、`tools/crafting-tests.ts`、`tools/crafting-coverage-audit.ts` | 112 配方、201 个物品定义、201/201 真图标与 17 条预览别名全部接入；112×P1/P2 共 224 条实际事务覆盖静态、默认继承、特殊继承和混合实例/堆叠材料；地图菜单入口、玩家切换、关闭退还和运行时真 UI 均已验收，`LINE-CRAFTING` 已关闭 |
...
| M-041 | 技能学习/绑定 | 已扒 | 已复现 | `skills-input-index.md`、`User.skillbykey`、`SkillControl.as`、`SkillSetControl.as`、`BuySkill.as`、`PassiveSkillControl.as` | `SkillUISystem.ts`、`FormalSkillPageSystem.ts`、`FormalSkillPageView.ts`、`FormalSkillRuntimeBridge.ts` | 真技能总页/双树/绑定/被动页已完成 5 角色规则、学习上限、升级等级/灵魂门禁、P1/P2 五槽顺序、HUD 同步与 V4 持久化 |
...
| M-044 | 存档 | 已扒 | 部分复现 | `save-slots-index.md`、`skills-input-index.md`、`pets-index.md`、`levels-index.md`、`User.getSaveObj()`、`MemoryClass.setStorage()`、`SaveInter.as` | `SaveSystem.ts` V4、`SaveSlotSystem.ts`、`TestSceneSaveBridge.ts`、专项测试 | 正式六槽与当前槽、旧单槽导入、V1/V2/V3 迁移、删除/损坏保护已复现；V4 同构保存双方成长、技能、库存/装备和宠物，Stage 2-1 内容、任务/活动仍后置 |
...
| M-048 | 战斗可读性与通关校准 | 已扒 | 已复现 | `stage1-combat-calibration.md`、`combat-rules-index.md`、`progression-index.md`、`attack-effects-index.md`、Stage 1 怪物/角色 AS3、恢复语料窄查、用户 2026-07-21 试玩反馈 | `Stage1CombatSystem.ts`、`HeroCombatSystem.ts`、`HeroNormalAttackSystem.ts`、Stage 1 三关 bridge、`stage1-combat-tests.ts`、`stage11-browser-audit.mjs` | 三关统一 enemy registry、80 HP/2 物防、attack id、windup/active/recovery、3000ms 保护、短按输入缓冲和死亡日志；1-2 为 `attrition-no-sustain`，1-3 为 `boss-physical`，1-1 三次完整无调试运行全部通关。现代占位反馈已闭合；怪物精确 SWF 几何仍作为视觉资源未知项公开 |
...
| M-050 | 启动与存档槽流程 | 已扒 | 已复现 | `save-slots-index.md`、`GMain.as`、`GameMenu.as`、`SaveInter.as`、`User.getSaveObj()`、`MemoryClass.setStorage()`、`MapMenu.as`、`OtherMat1.swf` 1149、`Common1.swf` 69/18 | `SaveSlotSystem.ts`、`SaveSlotScene.ts`、`SaveSystem.ts`、正式关卡入口/结果路由 | 真启动/六槽/确认资源、六槽读写、当前槽、迁移、损坏保护与删除确认均已闭合；删档与显式损坏反馈保留为明确现代选择，天庭地图由 M-051 继续闭合 |
| M-051 | 天庭地图选关 | 已扒 | 已复现 | `heaven-map-index.md`、`SelectPLace.as`、`MapMenu.as`、`GMain.as`、`MainGame.as`、`GameWin.as`、`GameFail.as`、`OtherMat1.swf` 1343/963/1311/1297/1304/1290 | `HeavenMapSystem.ts`、`HeavenMapScene.ts`、Stage 1 三关结果/退出桥、`heaven-map-tests.ts` | 真地图/菜单、四节点坐标与锁定/当前/已通关/未接入状态、现代 1P/2P 选择、单调进度和三关往返已闭合；Stage 2-1 内容仍明确排除 |
| M-052 | 完整功能 UI | 部分已扒 | 部分复现 | `full-function-ui-index.md`、`equipment-workshop-index.md`、`skills-input-index.md`、`equipment-index.md`、`pets-index.md`、`magic-weapons-index.md` 与 11 条完整 UI 资源标注 | 共享 `FeatureUiHost`、V4、正式背包、技能与宠物页面 systems/views | 共享 host、双玩家 V4、真背包/装备、技能四页和宠物页已完成；工坊三子页行为已扒，当前只推进 `GOAL-001` / `TASK-SLICE-138A`，后续由 `138B..138D` 独立 Goal 实现 |

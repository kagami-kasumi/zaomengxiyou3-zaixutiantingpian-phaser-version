...
| M-044 | 存档 | 已扒 | 部分复现 | `skills-input-index.md`、`pets-index.md`、`levels-index.md`、`User.getSaveObj()`、`MemoryClass.setStorage()`、`SaveInter.as` | `SaveSystem.ts` V3、Stage 1 flow systems、`TestSceneSaveBridge.ts`、关卡专项测试 | 现代 V3 支持 1-1/1-2/1-3/2-1 解锁边界，V1/V2 安全迁移默认 1-1；Stage 1 普通胜利幂等推进且重玩低关卡不降级，失败和 1-2 特殊入口不推进；Stage 2-1 内容、任务/活动和完整桌面存档仍后置 |
...
| M-049 | 正式战斗 HUD | 已扒 | 部分复现 | `combat-hud-index.md`、`GameInfo.as`、`RoleInfo.as`、`PhysicsWorld.as`、`BaseMonster.as`、恢复 `OtherMat1/bossblood.swf` | 测试状态栏、`SkillUISystem.ts`、最小宠物/背包面板；正式 Stage 1 HUD 待 `TASK-SLICE-131` | 固定 940×590 层、P1/P2 镜像与五槽键位映射、HP/MP/经验/等级、入口提示、Boss 即时/0.8s 追赶条、12 条 export-ready 真资源和现代 snapshot owner 已闭合 |
| M-050 | 启动与存档槽流程 | 部分已扒 | 部分复现 | `gameplay-index.md`、`SaveInter.as`、`User.getSaveObj()`、`MemoryClass.setStorage()`；完整启动链与视觉待 `TASK-SETTINGS-056` | `SaveSystem.ts` V3、现有关卡入口/结果路由 | 闭合 EXE 启动、新建/读取/删除、多槽、迁移与损坏反馈，再接正式页面 |
| M-051 | 天庭地图选关 | 未扒 | 部分复现 | 主流程/地图 AS3 与恢复源包待 `TASK-SETTINGS-057`；不能用关卡内 `M-027` 代替 | Stage 1 入口路由、V3 解锁状态、炼丹炉地图菜单局部入口 | 闭合地图节点视觉/坐标/命中区、锁定/解锁/通关状态和关卡往返后实现正式选关 |
| M-052 | 完整功能 UI | 部分已扒 | 部分复现 | `skills-input-index.md`、`equipment-index.md`、`pets-index.md`、`magic-weapons-index.md` 与各 UI AS3/恢复资源 | 背包、装备、宠物、心法/技能、法宝最小面板 | `TASK-SETTINGS-058` 建立逐页全集、真资源、双玩家、导航和存档覆盖矩阵，再生成同线小任务；最小切片不等于完整 UI |

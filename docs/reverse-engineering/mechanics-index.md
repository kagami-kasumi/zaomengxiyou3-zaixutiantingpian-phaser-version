...
| M-005 | 主流程 | 已扒 | 部分复现 | `gameplay-index.md`、`runtime-index.md`、`save-party-flow-index.md` | 既有启动/地图/关卡主循环 | 新建人数/选角、读档分流和地图直接进关合同已闭合；待 `TASK-ARCH-011/SLICE-151..153` 收敛正式队伍 owner |
| M-006 | 单人/双人规则 | 已扒 | 部分复现 | `controls-index.md`、`gameplay-index.md`、`save-party-flow-index.md` | 既有双玩家输入/关卡 | 原版 P1→P2、不同角色、playerNum/controlPlayer/roleid 合同已闭合；待存档队伍正式实现 |
...
| M-035 | 资源加载策略 | 已扒 | 部分复现 | `README_extract.md`、`evb-extraction-report.md`、`modern-architecture.md`、`assets-index.md`、`projectiles-index.md`、`levels-index.md`、`stage21-monster-visuals-index.md`、`asset-annotation/batches/stage22.md` | `src/assets/AssetManifest.ts` 的状态化 manifest、Stage 1/2-1 真场景、`public/assets/stage22/` 与恢复源 SWF | Stage 2-2 的 14 条场景、机关、Monster16 与六攻击 stable key 均已升级 ready；Monster9/10/19 继续复用 Stage 2-1 owner |
...
| M-037 | 背包 | 已扒 | 已复现 | `equipment-index.md`、`user/User.as`、`export/pack/BackPackElement.as`、`export/pack/PackThings.as`、`config/Config.as` | `InventorySystem.ts`、`FormalInventoryPageSystem.ts`、`FeatureUiScene.ts` | 真 940×590 页已闭合四分类、每页 25 格、P1/P2 owner、安全反馈与 V4 当前槽持久化；完整 1.1 物品表仍明确排除 |
...
| M-041 | 技能学习/绑定 | 已扒 | 部分复现 | `skills-input-index.md`、`User.skillbykey`、`SkillControl.as`、`SkillSetControl.as`、`BuySkill.as`、`PassiveSkillControl.as` | `SkillUISystem.ts`、`FormalSkillPageSystem.ts`、`FormalSkillPageView.ts`、`FormalSkillRuntimeBridge.ts` | 技能规则、真 UI、P1/P2 五槽、HUD 与 V4 字段已完成；2026-07-24 用户要求 owner/当前角色必须由新建存档队伍唯一决定，待 `TASK-SLICE-152` 收敛后恢复“已复现” |
...
| M-044 | 存档 | 已扒 | 部分复现 | `save-slots-index.md`、`save-party-flow-index.md`、`skills-input-index.md`、`pets-index.md`、`levels-index.md`、`User.getSaveObj()`、`MemoryClass.setStorage()`、`MainGame.levelClear()`、`GameWin.nextClick()` | `SaveSystem.ts` V4、`SaveSlotSystem.ts`、Stage 1/2-1/2-2 result bridges、专项测试 | 原版 playerNum+双方 roleid 与现代 `PartyConfiguration`/旧 V1..V4 默认 1P 迁移合同已闭合；待 `TASK-ARCH-011` 实现 |
...
| M-050 | 启动与存档槽流程 | 已扒 | 部分复现 | `save-slots-index.md`、`save-party-flow-index.md`、`GMain.as`、`GameMenu.as`、`SelectRole.as`、`SaveInter.as`、`User.getSaveObj()`、`MemoryClass.setStorage()`、`OtherMat1.swf` 1149/901、`Common1.swf` 69/18 | `SaveSlotSystem.ts`、`SaveSlotScene.ts`、`SaveSystem.ts`、正式关卡入口/结果路由 | 六槽安全边界保持；人数/五角色/原子 draft、显示列表、旧档默认 1P 和 P2 保留合同已闭合，待 `TASK-ARCH-011/SLICE-151` 实现 |
| M-051 | 天庭地图选关 | 已扒 | 部分复现 | `heaven-map-index.md`、`SelectPLace.as`、`MapMenu.as`、`GMain.as`、`MainGame.as`、`GameWin.as`、`GameFail.as`、`OtherMat1.swf` 1343/963/1311/1297/1304/1290、用户 2026-07-23/24 试玩反馈 | `HeavenMapSystem.ts`、`HeavenMapScene.ts`、Stage 1/2-1 结果/退出桥、`heaven-map-tests.ts` | 四节点状态、单调进度和往返已完成；逐关现代 1P/2P chooser 被用户否决，待 `TASK-SLICE-153` 改为直接消费当前存档 `PartyConfiguration` 后恢复“已复现” |
| M-052 | 完整功能 UI | 部分已扒 | 部分复现 | `full-function-ui-index.md`、`equipment-workshop-index.md`、`skills-input-index.md`、`skill-ui-native-index.md`、`equipment-index.md`、`pets-index.md`、`magic-weapons-index.md` 与 11 条完整 UI 资源标注 | 共享 `FeatureUiHost`、V4、正式背包、技能、宠物、工坊四事务与法宝页面 systems/views | 技能 250/868/417/213 已按 PG-007 完成原生文字、按钮三态、动态槽位和正式流程迁移；证据见 `TASK-SLICE-143-visual-audit.md`。宠物、法宝等存量页面继续由 PG-007 追踪，不以“真背景已接入”宣告 UI 原生化完成 |

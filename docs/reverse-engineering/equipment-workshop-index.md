...
### TASK-SLICE-141 原生页签实现

- `container-native-background.png` 从 character 119 扁平容器中只移除四个 up-state 字形并裁为 940×590；`generate-workshop-native-tabs.ps1` 记录可重生过程，恢复 SWF 和旧提取集保持不变。
- DefineButton2 95/99/109/113 的 up/over/down 单态 SVG 作为独立 Phaser child 接入；`FormalWorkshopNativeTabLayout` 保留 119 根坐标、原透明 hit bounds、标签顺序和 sourceCharacterId。
- 未选中使用白色 up，悬停使用橙色 over，按下和选中使用橙色 down；换 owner 仍回强化并返还上一 owner 暂存材料。
- 顶部现代标题/四页签和全屏暗罩已移除。确定性测试验证资产/标签/几何/不重叠/舞台内边界；940×590 运行样本验证 P1/P2、hover/selected、四页路由、拒绝事务、关闭回地图与零 console warning/error。

### TASK-SLICE-142 用户反馈整改

- 用户复验否决了“去字背景 + 另叠按钮状态图”的现代组合方案；该方案保留为历史实现记录，不再是当前运行时合同。
- 当前页面直接加载原始 `container.png`。四个原图页签、左右翻页、四类提交按钮、对应槽位和右上角“返回”均只叠加透明命中区，不再绘制替代按钮或额外关闭控件。
- 强化、合成、分解、打造四个操作主体统一以 `(316, 310)` 为左框中心；P1/P2 工坊使用左上角 26px 粗体描边文字，与原“炼丹炉”标题协调且没有矩形底板。
- `container-native-background.png`、12 个派生页签 SVG 和生成脚本已从现代工作树删除；恢复 SWF、旧提取结果和 Git 忽略的交叉验证资料均未修改。
- 专项测试验证原图资源、透明命中区、舞台边界和派生文件移除；940×590 运行样本逐页验证四操作、P1/P2、原图“返回”和零 console warning/error。
...
## TASK-SLICE-165D 嵌入式背包整改

165D 只迁移 character 119 右侧背包消费者，左侧强化、合成、分解、制作书及其事务 owner 均未改。机器真值为 `task-slice-165d.workshop-inventory`（`verified`），文件位于 `ground-truth/manifests/task-slice-165d-workshop-inventory.json`；它直接由现代 view 与专项测试消费。

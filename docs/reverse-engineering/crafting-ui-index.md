...
| `assets/EIcon2.swf` | `FE67CF769954A146981B22F906A4A4FD1AE28CBE3581DC683B1B9D00753FE02D` |
...
| --- | --- | --- | --- | --- |
| `crafting-ui.container` | `backpack1.swf` | `119 StrengthEquipment` / 1 | `ready` | 已派生完整容器并保留按钮四态和透明边缘 |
| `crafting-ui.fusion-panel` | `backpack1.swf` | `169 Fusion` / 1 | `ready` | 已派生三槽、预览/产物槽、合成按钮和文本底图 |
| `crafting-ui.selector.role1` | `OtherMat1.swf` | `218 SelectWK` / 1、2 | `ready` | 未选/选中帧已分别派生 |
| `crafting-ui.selector.role2` | `OtherMat1.swf` | `223 SelectTS` / 1、2 | `ready` | 同上 |
| `crafting-ui.selector.role3` | `OtherMat1.swf` | `233 SelectBJ` / 1、2 | `ready` | 同上 |
| `crafting-ui.selector.role4` | `OtherMat1.swf` | `228 SelectSS` / 1、2 | `ready` | 同上 |
| `crafting-ui.selector.role5` | `OtherMat1.swf` | `871 SelectBL` / 1、2 | `ready` | 同上 |
| `crafting-item.tlzsp` | `EIcon1.swf` | `813 tlzsp` / 1 | `ready` | 槽内材料图标；不使用 character 787 掉落态 |
| `crafting-item.wptlz` | `EIcon1.swf` | `807 wptlz` / 1 | `ready` | 预览与成功产物共用 |
...
`TASK-SETTINGS-045` 对 `kyg + kyz + kys -> kyl` 做了第二个窄资源族调查。四个直接图标都在 `EIcon1.swf`，`EIcon2.swf` 的 SymbolClass 只有 `gfcstx` 相关两项，不参与本配方。

四个目标都是静态 `DefineBits` 图片，不是 MovieClip/Sprite，因此没有可切换时间轴帧；现代侧各注册为一张单纹理。下表的 character id 即唯一选择性导出单位。

| Stable key | 用途 | 权威源 | Character ID / SymbolClass | Tag / 尺寸 | 当前状态 |
| --- | --- | --- | --- | --- | --- |
| `crafting-item.kyg` | 枯叶弓材料槽图标 | `EIcon1.swf` | `332` / `kyg` | `DefineBitsLossless2` / 50×50 | `ready` |
| `crafting-item.kyz` | 枯叶杖材料槽图标 | `EIcon1.swf` | `342` / `kyz` | `DefineBitsLossless2` / 50×50 | `ready` |
| `crafting-item.kys` | 枯叶衫材料槽图标 | `EIcon1.swf` | `323` / `kys` | `DefineBitsJPEG3` / 50×50 | `ready` |
| `crafting-item.kyl` | 枯叶灵预览/产物图标 | `EIcon1.swf` | `809` / `kyl` | `DefineBitsLossless2` / 50×50 | `ready` |

材料另有掉落态 `fall_kyg` character 202、`fall_kyz` character 214、`fall_kys` character 196，三者同为 50×50 `DefineBitsLossless2`。炼丹炉材料对象由 `ShowObj` 按 `myEquipObj.getFillName()` 取直接图标，因此槽内必须使用 character 332/342/323，不使用 `fall_*`。

`AllEquipment.mixProduce()` 明确把三材料映射为 `kyl/枯叶灵`。`Fusion.previewFun()` 的特殊别名分支不包含 `kyl`，最终调用 `AUtils.getImageObj(this.produceEquipName)`；所以枯叶灵预览和成功产物都直接使用 character 809，不需要别名 stableKey。

`TASK-SLICE-118` 已只选择性派生上述四个 character 到 `public/assets/ui/crafting/items/`，并通过 `CraftingItemTextureKeys` 接入背包、三材料槽、预览和成功产物。四张 50×50 PNG 已逐张人工目检；最终运行时截图已由 `TASK-SLICE-122` 补齐。

选择性派生要求：后续 `TASK-SLICE-118` 只按 character 332、342、323、809 导出四张原尺寸透明 PNG，登记到现有 crafting asset bundle；不得连带导出 196/202/214 掉落态或 EIcon 全包。

## 201 合成图标全集定位

`TASK-SETTINGS-049` 已用 `reference/crafting-item-catalog-1.1.json`、`Fusion.previewFun()` 和恢复 SWF 的 SymbolClass tag 建立 `reference/crafting-icon-catalog-1.1.json`。`TASK-SLICE-120A...120G` 已选择性导出并接入全部 201 个 fillName；当前 201/201 `ready`，没有 export-ready、ambiguous、unresolved 或 missing-original。

资源用途映射以 `assets/EIcon1.swf` 为主；`fbqpj/qpjy` 精确位于 `1_MainLoad__main1.swf` character 3/2，`mdcqg/wpxty/wpycjh/wpzty` 位于 `assets/MagicWeapon2.swf` character 18/4/6/17。`EIcon2.swf` 的 SymbolClass 只有 `gfcstx` 相关两项，不参与合成全集。

`Fusion.as:229-301` 的 17 条产物预览别名已逐项记录；材料槽继续按 fillName 直接 symbol，产物预览才使用别名。初始 195 项按首次配方分支与跨包例外拆成 7 个窄资源批次并全部完成，含主包/MagicWeapon2 的 6 个跨包例外，详见 `asset-annotation/batches/crafting-items-remaining.md`。

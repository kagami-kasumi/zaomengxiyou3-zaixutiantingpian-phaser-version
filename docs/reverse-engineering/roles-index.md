...
- `extracted_flash/resources_by_swf/[172845].swf/scripts/export/SelectRole.as`
...
- 五个角色普攻都应有特效；`attack-effects-index.md` 已确认前四名角色和白龙剑形态的映射，且确认当前 `extracted_flash/resources_by_swf/[172845].swf` 导出结果不足以直接提供这些素材。`M-047` 仍保留为部分已扒，因为白龙枪形态 `doSingleHit(...)` 还缺反编译证据。

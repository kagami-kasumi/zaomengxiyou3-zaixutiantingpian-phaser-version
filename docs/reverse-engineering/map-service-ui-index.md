...
| 商城 | 全部/宝石/道具/时装/宠物；49 件商品、每页 9 件、数量 1..100；第三大关后除 `zylhys` 外八折 | 当前 P1/P2 的背包与灵魂；确认购买后立即保存 | 购买写入当前槽；商品精确价格仍由逐页 task 从权威注册表闭合 |
...
商城不是必须联网才能运行的页面：该版本确认按钮以玩家灵魂结算并写入当前槽。“充值”、人民币/游币/点券和网络保存提示属于保留的旧静态表现，不得据此伪造在线支付、账户余额或后端服务。
...
| `TASK-SETTINGS-066B` | 商城 `Micropayment` / `ShopThing` / `SumInterface` | 49 件商品与权威价格、分类/分页/数量/确认全部状态、折扣例外、静态停服文字的允许现代例外 |
...
- 已确认：四入口实际调用链、四页面身份、三个恢复源包、四根 Symbol、normal 视觉基准、根层次/几何、主要内容与事务 owner、原版存档边界；丹药页完整六段证据已闭合。
- 仍为未知：商城、设置、任务三页的深层按钮/动态显示列表，以及商城完整价格、任务完整奖励与进度生产者、设置现代持久化产品裁决。
- 结论：父任务继续保持 `Split`；丹药页 `TASK-SETTINGS-066A` 已完成，功能线转入 `GOAL-042 / TASK-SETTINGS-066B`，不能把单页闭合当成四页完成。

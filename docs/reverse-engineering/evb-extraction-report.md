...
2026-07-15 使用官方 `evbunpack` 0.2.6 成功恢复 `再续天庭1.1.exe` 的 Enigma Virtual Box 虚拟文件系统。原始命名资源已经落到项目外目录，代表性角色、关卡和怪物 SWF 经 176-byte 重排后可被 FFDec 26.0.0 正常解析和导出。

这次结果推翻了“原 EXE 中可能没有真素材”的弱假设。此前 `extracted_flash/` 只是不包含原始命名的子资源包；旧提取集仍保持只读，用于 AS3 和旧结果对照。
...
`evbunpack.exe` 的大小和 SHA-256 与 GitHub 官方 0.2.6 Release 资产一致。完整提取命令：
...
| 路径 | 内容 |
| --- | --- |
| `D:\flash-unpacked` | EVB 原始文件系统，206 个文件，473,460,042 bytes |
| `D:\flash-unpacked-manifest.json` | 206 个原始解包文件的相对路径、大小和 SHA-256 |
| `D:\flash-restored-swfs` | 174 个已还原标准头的 SWF，保留原相对目录 |
| `D:\flash-restored-swfs-manifest.json` | 还原模式、签名、版本、声明长度及输入/输出 SHA-256 |
| `D:\flash-validation-images` | 三个代表包的 FFDec 图片抽检输出 |

这些目录不是项目源码，不应直接提交 Git，也没有写入或覆盖 `extracted_flash/`。
...
- `TASK-ASSET-002` 不再依赖用户提供外部源包，可以从 `D:\flash-restored-swfs` 选择一个最小真资源族。
...
- 不执行解包得到的 EXE/DLL，不修改旧的 `extracted_flash/` 证据集。

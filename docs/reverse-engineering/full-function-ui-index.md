...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| FUI-01 | 共享功能页 host | HUD 五真按钮/快捷键；地图技能入口；关闭回原场景 | 原版单页门、暂停/恢复、owner、同页快捷键关闭；Escape 只切设置 | P1/P2 独立数据；技能页原版默认 P1 后页内切人；设置为全局 owner | host 会话不持久化 | HUD 574、SetMenu 371、Help 444 与既有页面根 | 战斗 pointer/settings 业务已完成，但当前仍无 host verified manifest；地图态统一暗层/标题/跨页按钮/通用关闭是未获批准的现代占位 | `TASK-SETTINGS-175C` 先闭合战斗 host 真值与地图态负向证据，再生成宿主整改 task |

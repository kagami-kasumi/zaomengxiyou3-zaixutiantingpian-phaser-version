...
| FUI-04 | 技能总页 | P1 V、P2 num `*`、地图 `showBuySkill`；返回原场景 | 角色选择、主动/被动切换、灵魂显示 | 原版页可选 P1/P2；战斗快捷键默认打开对应 owner | 技能树、绑定、被动、灵魂 | 250 | 真总页、正式入口、地图双 owner 与 V4 保存已完成；当前仍有现代暗层/外框/标题/通用按钮 | `LINE-UI-NATIVE-SKILLS`：补原生文字/按钮/状态/槽位证据并移除替代覆盖层 |
| FUI-05 | 主动心法/技能学习 | FUI-04 主动标签 | 两棵心法、树升级、5×2 技能、学习上限、技能升级/预览 | 当前选中 User | `isstudyskill` 等价结构 | 868 | 业务、双 owner 与持久化已完成，交互仍由现代按钮/文字覆盖真布局 | 原生化两树、10 技能、学习/升级/预览按钮与动态槽位 |
| FUI-06 | 五槽技能绑定 | FUI-05 技能设置按钮；提交返回 | 已学技能列表、五槽、拖放/替换、P1/P2 键位映射 | 当前选中 User | `skillbykey` 等价结构 | 417 | 五槽顺序、HUD 同步和保存已完成；现代指针按钮等价尚未按 PG-007 复核原生槽位/按钮 | 独立证明拖放对象、已学列表、五槽、提交/返回与最小可访问等价 |
| FUI-07 | 被动技能 | FUI-04 被动标签 | 5 个被动等级与说明 | 当前选中 User | `ispassiveskill` | 213 | 五被动业务、双 owner 与保存已完成，现代按钮/文字仍承担主要视觉 | 原生化五被动按钮、等级/说明槽位和状态反馈 |
...
| `full-ui.skill-hub` | `OtherMat1.swf` 250 | 940×590 | 根容器 |
| `full-ui.skill-active` | `OtherMat1.swf` 868 | 888.05×425 | 嵌入技能根页 |
| `full-ui.skill-bind` | `OtherMat1.swf` 417 | 506×356 | 模态/子页 |
| `full-ui.skill-passive` | `OtherMat1.swf` 213 | 746×429 | 嵌入技能根页 |

...
| 技能学习与绑定 | `BuySkill`、`SkillControl`、`SkillSetControl`、`PassiveSkillControl` | `User.isstudyskill/skillbykey/ispassiveskill/lhValue`；关闭后 `RoleInfo.refreshShowSkill` | `OtherMat1.swf` 250/868/417/213；`task-settings-175d.skill-pages` verified：250 对象/32 状态/50 图标×3 帧 | 交叉确认 | 原版拖拽的点击/键盘等价不能改变五槽几何、顺序或 x_btn 提交；现代 view 直连仍待 183 | 真值生成/Schema + 学习/升级/绑定/双玩家/保存 + 逐状态页面观察 |
| 宠物页 | `PetInterface`、`PetHeadSprite`、`PutPetSkill` | 每页 5、最多 10；选择/出战/休息/放生/洗练/进化只改构造时注入的 `User`；对应英雄重建宠物 | `pet1.swf` 932，`task-settings-175a.pet-page` verified：74 对象/16 状态/940×590；技能区 8 槽 | 交叉确认 | 行为/几何未知为 0；现代原生化仍待 TASK-SLICE-180 | 真值生成/Schema + roster/动作/owner/保存 + 逐状态页面观察 |
| 法宝页 | `SutraInterface` | `RoleInfo.fbClick()` 只在已装备 `zbfb` 时打开；关闭时重算装备；强化消耗灵魂、五行重置有确认框 | `backpack1.swf` 596；`task-settings-175b.magic-weapon-page` verified：28 对象/21 状态/940×590 | 交叉确认 | 行为/几何未知为 0；原版无 P2 面板入口；现代原生化仍待 TASK-SLICE-181 | 真值生成/Schema + 装备门禁/强化/重置/保存 + 逐状态页面观察 |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| FUI-01 | 共享功能页 host | HUD 五真按钮/快捷键；地图技能入口；关闭回原场景 | 原版单页门、暂停/恢复、owner、同页快捷键关闭；Escape 只切设置 | P1/P2 独立数据；技能页原版默认 P1 后页内切人；设置为全局 owner | host 会话不持久化 | `task-settings-175c.stage-feature-host` verified；HUD 574、SetMenu 371、Help 444 与各页根 | 25 对象/42 状态已闭合；战斗 pointer/settings 业务已有，地图态统一暗层/标题/跨页按钮/通用关闭仍是未获批准现代占位 | `TASK-SLICE-182` 按 175A..I 实现批次排序整改宿主，不抢占当前 175E |
| FUI-02 | 背包/角色总览 | P1 C、P2 num `/`；再次按或关闭按钮退出 | 角色名/等级/经验/战力/HP/MP/属性、6 装备槽、时装显示、出售白装 | `BackPack.setpack(1/2)` 绑定对应 hero/User | 装备、分类背包、时装显示、角色成长 | 304 | 166B 完成动态 child；166D 按用户最终澄清使经验文字整槽居中、页码显示 `n/5`，166C 灵魂位置继续有效 | 正式页已满足；炼丹炉嵌入式消费者由 165D 处理 |
| FUI-03 | 背包分类与物品详情 | FUI-02 内嵌 | 装备/道具/时装/技能书；5 页×25 格；选择、预览、穿脱、使用/丢弃/给予门禁 | 跟随 FUI-02 owner | `zblist/djlist/szlist/jnslist/curarray` | 246、628、358、610；428 真图标 | 166B 完成 `61×60` 阵列与操作层；166D 保留 628 外层格框、只裁去图标内层框，358/610 继续置顶 | 正式页已满足；专属物品效果继续显式后置 |
| FUI-04 | 技能总页 | P1 V、P2 num `*`、地图 `showBuySkill`；返回原场景 | 角色选择、主动/被动切换、灵魂显示 | 原版页可选 P1/P2；战斗快捷键默认打开对应 owner | 技能树、绑定、被动、灵魂 | 250；`task-settings-175d.skill-pages` verified | 250 对象/32 状态真值已闭合；正式业务/owner 已有，现代覆盖与手写视觉源待删除 | `TASK-SLICE-183` 直接消费 manifest，不保留暗层/外框/标题/通用按钮 |
| FUI-05 | 主动心法/技能学习 | FUI-04 主动标签 | 两棵心法、树升级、5×2 技能、学习上限、技能升级/预览 | 当前选中 User | `isstudyskill` 等价结构 | 868/865；manifest 覆盖十树与 50 图标×3 帧 | 真值已闭合；现代 layout/view 直连待 183 | `TASK-SLICE-183` 消费十树、按钮、字段与动态 `LV.n` |
| FUI-06 | 五槽技能绑定 | FUI-05 技能设置按钮；关闭提交返回 | source、五槽、拖放/替换、P1/P2 键位映射 | 当前选中 User | `skillbykey` 等价结构 | 417；manifest 覆盖 P1/P2、拖放/回退/提交 | 真值已闭合；点击/键盘等价限于原 76×76 槽 | `TASK-SLICE-183` 删除现代列表/槽按钮/独立提交并直连 manifest |
| FUI-07 | 被动技能 | FUI-04 被动标签 | 5 个被动等级与说明 | 当前选中 User | `ispassiveskill` | 213/212；manifest 覆盖五行、字段、207 和满级隐藏 | 真值已闭合；现代动态字段投影待 183 | `TASK-SLICE-183` 直连五行显示列表与字段，不新增表头 |
| FUI-08 | 宠物管理 | P1 B、P2 num `-`；再次按或关闭 | 每页 5/最多 10、全属性、8 技能、出战/休息/放生、洗练、超进化 | 构造参数绑定具体 User；两玩家可各出战 1 只 | 两套 pet roster、选中/出战、技能与成长字段 | pet1 932；`task-settings-175a.pet-page` verified | 业务、双 owner/runtime/当前存档已完成；74 对象/16 状态真值已闭合，现代覆盖仍待删除 | `TASK-SLICE-180` 直接消费真值，补并排/叠图和逐状态运行证据 |
| FUI-09 | 法宝强化/五行重置 | P1 N 或 HUD 法宝按钮；已装备 `zbfb` 才打开 | 名称/等级/成长/五行/攻防/HP/MP、灵魂进度、强化、重置确认 | P1 owner；原版 P2 无面板快捷键，现代明确不伪造 | `zbfb` 实例等级/成长/五行/属性、灵魂与材料 | backpack1 596；`task-settings-175b.magic-weapon-page` verified | 强化/重置业务与保存已完成；28 对象/21 状态真值已闭合，现代覆盖仍待删除 | `TASK-SLICE-181` 直接消费真值，补并排/叠图和逐状态运行证据 |
| FUI-10 | 装备工坊容器 | 地图炼丹炉入口；四标签切换 | 强化、合成、分解、制作书 | 页面角色选择/当前 inventory owner | 装备实例、库存材料 | 119 已 ready | 正式 host、四标签、双 owner、四类事务及右侧四分类 5×5 背包格已复现 | `138A..138D`、`165D` 已闭合 |
...
`TASK-SETTINGS-175` 的证据等级审计见 `evidence/TASK-SETTINGS-175-functional-ui-truth-audit.md`。本索引中 2026-08-08 前的“真页面/闭合”只保留其业务与旧视觉审计含义；没有对应 `verified` manifest 和直接消费链时，不再作为当前 UI 原生化完成证明。

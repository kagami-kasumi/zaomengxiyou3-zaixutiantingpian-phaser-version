...
| FUI-09 | 法宝强化/五行重置 | P1 N 或 HUD 法宝按钮；已装备 `zbfb` 才打开 | 名称/等级/成长/五行/攻防/HP/MP、灵魂进度、强化、重置确认 | P1 owner；原版 P2 无面板快捷键，现代明确不伪造 | `zbfb` 实例等级/成长/五行/属性、灵魂与材料 | backpack1 596 | 强化/重置业务与保存已完成；`FormalMagicWeaponPageView` 仍叠加现代标题、面板、摘要和通用按钮，属于明确占位 | `TASK-SETTINGS-175B` 先生成 596 verified 真值，再生成单页实现 task |

...
玄龟首批链路（2026-09-15由[221完整行为证据](pet-turtle-family-index.md)校正；本节旧最小切片建议不作为整族完成标准）：
...
`findPetUsedMagic()` 给出玄龟消耗：`sld/txlj/sybh` 都消耗 20 MP，`xwaoyi` 门禁要求30 MP，但原释放函数不扣MP。`getPetHarmObj()` 给出数值基数：`sld = atk`，`txlj.first = 5 * technique`、`txlj.second = 4 * warpower`，`sybh = 5.4 * atk`，`xwaoyi = 0`；switch结束后first统一乘1.05。`sld` 和 `sybh` 在 `getRealPower()` 中继续叠加 `fsnl` 的 `getMagicAddValue()`、`sxkb` 的宠物暴击 2 倍、`isGXP` 的 1.2 倍；四阶 `PetTurtle4` 还额外乘 `hurtBaseEffectRate()`。

| 类 | 技能槽 | 条件 | 动作/伤害或状态 |
| --- | --- | --- | --- |
| `PetTurtle1..4` | skill1 `sld` | 已学 `sld`、MP 足够、目标距离 `50..200`、CD1 就绪 | `hit2`，扣 20 MP；cell2的countdown10回调生成 `FollowBaseObjectBullet("PetTurtle1Bullet2")` / `hit2`，魔法命中；按 `sld = atk` 派生伤害，同时宠物按本次 `hit2` 伤害量治疗自身 |
...
| `PetTurtle3..4` | skill3 `sybh` | 已学 `sybh`、MP 足够、存在目标、CD3 就绪 | `hit3`，扣 20 MP；cell2的countdown10回调在宠物自身附近生成 `SpecialEffectBullet("PetTurtle3Bullet3")` / `hit3`，三阶scale1、四阶scale2；按 `sybh = 5.4 * atk` 派生范围伤害 |
...
`PETTURTKE_BUFF` 的可观察战斗边界来自 `BaseHero`：主人受伤时，如果主人和宠物都持有该 buff，会让宠物承受 `ceil(原伤害 * 0.05)`，主人实际伤害变为 `int(原伤害 * 0.95)`（int复合赋值后ceil）；主人被治疗时，如果双方都持有该 buff，会先让宠物按 `治疗量 * 1.05` 回血，再把主人治疗量放大到 `int(治疗量 * 1.05)`（int复合赋值后ceil）。`PetTurtle2/3/4.doHit2()` 还会在宠物 `sld` 治疗自身后，如果双方存在链接 buff，同步给主人增加同等治疗量。

玄龟资源键已由 AS3 确认：本体为 `PetTurtleBmd1..4`，一阶普攻为`PetTurtle1Bullet1`、二三四阶为 `PetTurtle2Bullet1`，技能/弹体包括 `PetTurtle1Bullet2`、`PetTurtle3Bullet3`。`attackBackInfoDict` 中 `hit1` 为 physics；`hit2` 在一至三阶为 magic、四阶配置为 physics 但仍用于 `sld`，`hit3` 为 magic；现代首批只登记占位资源 key，不重新提取资源。
...
`TASK-SLICE-202` 让正式战斗 HUD 直接消费上述真值：`PetCombatHudHeadAssets` 解析 35 fixture 并把 33 个唯一终端 child 注册为专属 `pet-combat-hud-heads` bundle；运行时按 frame/child/matrix/registration/visible bounds 投影。旧宠物页/身体头像路径、`104.8×93.6` 联合 bounds 拉伸和硬编码头像坐标已删除；P1/P2、五关、出战/休息/换宠/重载与关键字段变异测试通过，九物种代表 baseline 与 runtime 资源像素差为 0。

...
历史结论：当时 Role1、Role4、Role5 真素材在旧导出资源中均未命中。此结论已被 `source/restored-swfs/` 的恢复包调查逐角色取代；Role5 最新权威状态见 `role5-combat-visuals-index.md`。
...
| Role5 本体动作 | 白龙枪/剑动作族 | 双形态 body/equip 全表 | `derived-ready` | 069E 已闭合，158E 接入 |
| Role5 普攻附属 | 枪/剑普攻 stable keys | `Role5Bullet1..5`、`swordhit1..6` 与 `_1` 变体；仅 `Role5runattack` 全集未命中 | `derived-ready` / `corpus-negative` | 158E 消费已确认对象，不伪造跑攻对象 |
| Role5 枪系/状态 | 枪系技能与状态 stable keys | 恢复包精确对象矩阵 | `derived-ready` | 069E 已闭合，158E 接入 |
| Role5 剑系/随身箭 | 剑系技能与随身箭 stable keys | 恢复包精确对象矩阵 | `derived-ready` | 069E 已闭合，158E 接入 |
...
- Role5 包与 P-code 已在069E复核完毕；后续直接按 `role5-combat-visuals-index.md` 串行进入158E，`Role5runattack` 的 corpus-negative 不得被占位素材改写成原版事实。

...
- 既有 Role5 标注原位升级为 `derived-ready + integrate`，并补齐枪五段、状态、cast/companion与瞬移对象行。
- 允许的现代可见例外为空；下一唯一去向为 `TASK-SLICE-158E` 选择性接入。
...
- [x] 行为完成与视觉完成分列；当前只关闭权威实现输入，不宣称Role5真视觉已接入。

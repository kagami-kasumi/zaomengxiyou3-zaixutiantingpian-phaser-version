...
- 四页跨三包与四套事务 owner；丹药页深层显示列表已闭合，其余按钮状态、动态 child 与内容全集分别交给 `066B..D`。

## 去向

- `TASK-SETTINGS-066A` 的丹药证据见 `../../immortality-ui-index.md`；`066B..D` 继续闭合商城、设置、任务。
- `TASK-SLICE-155A..D` 在对应证据未知清零后选择性接入；不得把根 SVG 当作可交互整页截图。

## 关闭检查

- [x] 15 条记录均有唯一 stableKey、精确源包、symbol id、状态与唯一后续去向。
...
- [x] 仅丹药页在完整六段证据后标记深层显示列表闭合；其余三页仍未越权关闭。

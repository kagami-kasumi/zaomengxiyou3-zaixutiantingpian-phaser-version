...
- 本轮排除：完整背包/宠物/心法/法宝页面、全部 50 个技能图标批量导出、宠物本体与技能动画；192B 已单独接入战斗 character 662 摘要皮肤。
...
- 已确认：16 条资源均有精确源包、character id、AS3 用途与几何/时间轴证据。
- 推测：0。
- 未知：0；191/192B 已定位并接入宠物摘要 `ShowPetInfo` character 662，宠物本体/技能动画属于 193 的独立范围。
...
`TASK-SLICE-192B` 追加：从 `assets/pet1.swf` 选择性导出 character 605、610/614 各 25 帧，并复用同源宠物头像，直接消费 191 verified 真值；运行证据位于 `docs/tasks/evidence/TASK-SLICE-192B/`。

## 去向

- 可直接接入：16（原有 12 条通过两个保留内部子件的组合 SVG 接入；宠物战斗 HUD 的 4 条通过原生 PNG 帧族接入）。
- 待定位符号：0（首切片范围）。
- 可选择性导出：0。
...
- [x] 16 条记录已转为 `ready`；组合 SVG、原生 PNG 帧族、manifest provenance 和运行态验收均已落盘。

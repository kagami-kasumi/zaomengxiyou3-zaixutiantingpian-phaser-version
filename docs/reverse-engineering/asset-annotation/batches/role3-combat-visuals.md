...
- 推测：0。
- 影响实现的未知：0。
- 15 条可见资源标注均已更新为 `ready + none`；`xgq` cast 行为继续为 `ready + none`，现代运行时只保留行为计时并跳过独立 view。
...
- [x] 本体/装备、397 个对象帧、HUD frame3、唯一 bundle owner、专项门禁与 940×590 单/双人零 console 验收完成。

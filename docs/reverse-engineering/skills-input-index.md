...
技能总页、主动页、绑定页和被动页的完整显示列表、按钮状态、命中区、动态槽位与原版视觉基准见 `skill-ui-native-index.md`；本文继续作为行为/数据合同，不再用现代 UI 布局反推原版视觉。
...
- 被动五行的显示/升级效果已由恢复包 `PassiveSkill` AS3/P-code 闭合：生命上限 `level×100`、魔法上限 `level×100`、暴击率 `level%`、每秒回血 `level×3`、每秒回魔 `level`；完整字段与帧映射见 `skill-ui-native-index.md`。

...
## 11. 2026-07-25 被动表格可读性反馈

用户复验指出被动页顶部表头为空，且效果列只显示裸数值，无法判断参数含义。重新核对 character 213、212 与 `PassiveSkill.as`：

- `确认事实`：character 213 的根显示列表只有 character 198 静态背景和五个 character 212 行，没有表头文字 child；恢复帧中顶部黑色表头区域为空。
- `确认事实`：212 的 `lastvalue` / `attvalue` 原版写入完整效果说明，不是裸数值。五类文案分别为“生命上限增加”“魔法上限增加”“暴击率增加”“每秒回血增加”“每秒回魔增加”。
- `用户反馈作废的现代例外`：首轮曾在原空白区域补六列表头；用户复验要求完全复现原版后，该可见例外撤销，表头恢复为空。
- `确认事实`：OtherMat1 character 200 为内嵌 `FZCuYuan-M03`；当前项目已从恢复 SWF 派生同一 TTF。浏览器系统字体或仅声明字体名不能证明字形一致。
- `现代实现映射`：`FormalSkillPageView` 继续使用原 213/212/207 资源和字段坐标；`formatFormalPassiveEffect` 恢复原版完整文案；技能页资源进入前加载原 `FZCuYuan-M03`，所有动态 TextField 使用该字体，不再回退微软雅黑。

验证要求：专项测试覆盖空表头、原字体资产与五类完整效果文案；940×590 单人被动页确认字形、基线、长文案、按钮和成本无交叠。

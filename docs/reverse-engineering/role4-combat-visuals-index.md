...
- 当前玩法状态只公开铲/弓模式而未公开任意衣装/装备视觉id，因此 `combat-hero-4` 入场包只预载当前正式装束的铲身0、弓身0、装备0、装备4、普攻和HUD；897帧技能对象由 `combat-hero-4-skills` 在关卡可见后后台加载。其余46张仍由manifest完整编目，待未来换装状态提供明确id后按同一owner选择。这是数据模型映射边界，不是当前装束的可见替代。

...
每次执行先识别可独立返回的有界工作包；存在此类工作且主 agent 能同时推进其他有用工作、运行环境允许时，默认启用 subagent。简单的资料定位、合同清单核对、独立验证、局部文档或隔离文件小修优先委派；强顺序工作、持续共享中间结论、同文件写入冲突或派发成本高于工作本身时由主 agent 直接处理，并在协作计划或执行说明中简述原因，不为代理数量制造工作。

用户指定的模型分工按工作内容选择，不以主/子 agent 身份或文件数量单独判定难度：

| 准入条件 | 模型与分工 |
| --- | --- |
| 输入资料、修改范围和验收标准均明确，结果可用现成检查独立验证；如资料定位、合同清单核对、已有测试执行、明确小修 | 优先 `gpt-5.6-luna`；整个任务都满足时可由 Luna 主 agent 完成，复杂任务中的此类独立包交给 Luna 子 agent |
| 需要先定义问题、确定范围或设计方案，涉及共享运行时、架构决策、原版证据冲突、复杂生命周期，或正确性主要依赖综合判断 | 优先 `gpt-6-astra` 主 agent 负责判断、实现与最终归并；复杂独立包确需子 agent 时才选择 GPT-6 |
| 边界不清，但可以提出一个有界调查问题 | 先由 Luna 调查并返回证据、推断和未知，主 agent 再决定实现或接管；仍须满足主 agent 能同时推进其他有用工作的委派条件，不让 Luna 无限重试 |

当前主会话的模型不因文字规则自动切换；主 agent 已是 GPT-6 时，极小或强顺序任务可直接完成，不为改用 Luna 强行委派。Codex 子 agent 默认模型配置为 `gpt-5.6-luna`，派发遵守工具实际支持的模型覆盖方式；其他客户端只使用实际支持的模型标识。Luna 遇到超范围、证据不足或验证失败时返回精确阻塞与已得证据，由主 agent 接管或重新划定工作包，不反复盲试。模型不可用时说明原因并由主 agent 接管，不静默增加高成本子 agent。

本阶段采用默认模型配置与文字准入规则；不增加运行记录校验、模型启动拦截或上下文额度/裁剪管理要求。模型选择不改变原任务的证据与验收标准，Luna 主 agent 同样承担最终质量责任。

现有 task 的“单 agent”是初始计划，不是永久禁令；满足上述条件时，主 agent 可先在执行记录中更新协作计划再派发，无需再次请求用户批准。默认由主 agent 写实现，subagent 只读核对源证据、测试或差异；不得多人同时修改共享核心文件。

派发前由主 agent 明确：

- 待回答问题、允许读取的资料族和预期输出；
- 所选模型、选择理由和验收标准；简单有界包默认 `gpt-5.6-luna`；
- 允许操作与禁止范围，默认设为只读；
...
7. 新游戏 task 必须预计 0 次 compact；治理项也必须保持一个有界 PG 方案。执行前核对工作包、验收批次和拆分触发；实际范围超出声明时，本次只重排并交接。compact 次数不作为停止、拆分或新开对话条件；恢复后窄读当前合同、改动和必要证据，继续原 task，不借此扩张范围。仅在存在尚未保存且影响恢复的信息或需要实际交接时更新简短检查点；已有记录足够时直接复用，不重复计数、落盘或运行无变化的检查。
...
- 对话管理判断：默认继续当前对话；实际交接时说明原因并按下方规则保存必要恢复信息。
...
- 结构检查与 warning/error 处理以 [AGENTS.md 必须遵守第 7 条](../../AGENTS.md#必须遵守) 为准；先运行 `npm run check:structure`，本协议不重复定义拆分条件。
...
同一个正式游戏 task 尚未完成时，默认继续当前对话。预计零次 compact 仅为规模规划目标。compact 次数不作为停止、拆分或新开对话条件；恢复后窄读当前合同、改动和必要证据，继续原 task，不借此扩张范围。仅在存在尚未保存且影响恢复的信息或需要实际交接时更新简短检查点；已有记录足够时直接复用，不重复计数、落盘或运行无变化的检查。
...
- 继续当前对话：同一 task 还在验证、修 bug、补文档或仍未完成；不以 compact 次数设停止门禁。
- 安全交接：用户要求交接、实际范围超出合同或出现无法继续的真实阻塞时，保存尚未落盘的恢复信息并明确承接范围；不因 compact 自动交接。
...
检查点仅在存在尚未保存且影响恢复的信息或需要实际交接时更新，写入当前执行合同的执行记录，不新建平行状态文件。已有记录足够时直接复用；不维护 compact 计数，不为重复自动续跑追加同义检查点，不重复运行输入未变的检查。需要更新时只记录变化，并包含适用项：


- 当前执行项与合同路径、最近一次验证过的工作范围。
- 已完成产物与文件路径、未提交改动及其归属。
- 运行中检查的命令、会话标识、已知结果和待收集输出。
- 未完成验收、未知项和剩余工作包。
- 下一具体动作；仅在实际拆分或交接时注明承接项，不按 compact 次数禁止实现或资源派生。

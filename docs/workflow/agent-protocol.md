...
- 继续当前对话：同一 task 还在验证、修 bug、补文档或仍未完成，且未触发首次 compact 收束门禁。
- 安全交接：上下文已无法容纳剩余工作时先落盘检查点；首次 compact 后只结束正在运行的检查、窄读关键合同、回写并拆分，不恢复新增实现。
...
检查点写入当前执行合同的执行记录，不新建平行状态文件。必须包含：

- 当前执行项与合同路径、最近一次验证过的工作范围。
- 已完成产物与文件路径、未提交改动及其归属。
- 运行中检查的命令、会话标识、已知结果和待收集输出。
- 未完成验收、未知项和剩余工作包。
- 下一允许动作及拆分后的承接项；首次 compact 后不得新增实现或派生资源。

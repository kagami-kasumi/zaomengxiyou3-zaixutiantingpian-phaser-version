...
代码、架构、游戏 task 或工作流变更收尾前，必须执行问题适用性扫描：只读取 `problem-governance.md` 的“活跃问题索引”，用其中 `PG-*` 的“适用触发与反馈记录”扫描当前 diff；归档索引不参与常规扫描，只在门禁/回归失败、用户或 reviewer 报告复发、新设计推翻旧决策或直接引用 PG id 时定向重开。命中时运行指定检查并回写样本；结论为“复发”或“方案不充分”时，必须转入“复盘中”，保留原方案版本并评估补强/替换方案。满足关闭标准、可定位证据、持久防线和必要增量样本的问题应在同次治理或下一次适用收尾时出清。交接时报告命中的 PG 与样本结论；无命中时明确说明已扫描活跃索引。

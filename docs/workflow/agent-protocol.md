...
存在边界清晰、可以独立返回的调查/验证工作包，且主 agent 能同时推进其他有用工作时，应适当启用 subagent；轻量小修、强顺序实现、需要持续共享中间结论或会修改同一批文件的工作默认不启用。具体运行环境限制主动委派时，遵守环境限制并退回单 agent 执行。
...
7. 新游戏 task 必须预计 0 次 compact；治理项也必须保持一个有界 PG 方案。执行前核对工作包、验收批次和拆分触发；实际范围超出声明时，本次只重排并交接。允许一次 compact：先落盘检查点，恢复后窄读关键合同和当前代码，在原 task 范围内继续；第二次 compact 后只允许结束正在运行的检查、重读当前状态、回写安全检查点并拆分剩余工作，不再读取新资料族、派生新资源或新增实现。
...
- 对话管理判断：继续当前对话、安全交接，或建议新开对话；compact 后按下方检查点收束。
...
同一个正式游戏 task 尚未完成时，默认继续当前对话。task 默认应在不 compact 的情况下完成，但允许一次恢复余量；首次 compact 前落盘检查点（自动触发时恢复后立即补记），恢复后只窄读关键合同和当前代码，再继续原 task，不重新全文读取或扩张范围；若第二次 compact 已发生，应重新检查当前 task 的关键文档和正在修改的文件，只完成当前检查、回写检查点并拆分下一 task，不凭摘要继续扩张当前任务。
...
- 继续当前对话：同一 task 还在验证、修 bug、补文档或仍未完成，且未触发第二次 compact 收束门禁。
- 安全交接：上下文已无法容纳剩余工作时先落盘检查点；第二次 compact 后只结束正在运行的检查、窄读关键合同、回写并拆分，不恢复新增实现。
...
检查点写入当前执行合同的执行记录，不新建平行状态文件。同一对话的 compact 次数持续累计，不因拆分或改名清零；新对话重新计数。第二次发生后强制交接。必须包含：

- 本对话已发生 compact 次数（含本次），以及本次是首次恢复还是第二次交接。

- 当前执行项与合同路径、最近一次验证过的工作范围。
- 已完成产物与文件路径、未提交改动及其归属。
- 运行中检查的命令、会话标识、已知结果和待收集输出。
- 未完成验收、未知项和剩余工作包。
- 下一允许动作及拆分后的承接项；第二次 compact 后不得新增实现或派生资源。

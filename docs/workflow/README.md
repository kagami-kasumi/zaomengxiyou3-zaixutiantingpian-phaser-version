...
仅检查 harness 的入口规则、文档链接、唯一 Active/Ready、当前推荐快照和 PG 合同：

```bash
npm run check:harness
```

该命令不读取游戏源码、资源语料库，不运行游戏架构或设计门禁。它包含决策路由与调度负向测试；自然语言语义仍需人工复核。结构 warning/error 决策的权威来源是 `AGENTS.md` 第 7 条，客户端入口和详细协议只引用该规则。
...
`check:workflow` 保留为兼容的完整组合入口：harness 检查、旧 workflow/领域/证据合同检查、资源标注、关卡架构检查；`check:all` 继续覆盖该完整入口。拆出轻量命令不取消现有收尾要求。

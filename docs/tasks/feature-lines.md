...
| LINE-PRE-STAGE-2-3-COMPLETION | Active | 在继续 Stage 2-3 逆向前，先闭合原版 1.1 可入包资源全集与正式背包基础，再补齐天庭地图四个服务入口、关卡内五个功能入口、已完成关卡全部小怪真动画、通用关卡生命周期/可玩运行框架、五角色战斗 UI/技能动画，并以既有本地六槽存档完成正式旅程回归 | TASK-SLICE-157D | `feature-line-coverage/LINE-PRE-STAGE-2-3-COMPLETION.md` | Stage 1 三关真怪物已接入，当前只做五关 owner 与 Stage 2 防回归 | `TASK-SLICE-157D` 为唯一 Ready；`TASK-ARCH-016A..D` 已登记为同线 Planned |

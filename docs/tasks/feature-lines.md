...
| LINE-CORE-PROGRESSION-COMPLETION | Active | Stage 2-3 前完成炼丹炉左页与四功能、全装备 UI/数值、五角色成长、存档扩展、关卡左下五入口、用户确认的 UI 整改及复评确认的同线纠错 | TASK-SLICE-171B | `feature-line-coverage/LINE-CORE-PROGRESSION-COMPLETION.md` | 无 | 176 已关闭用户反馈回归；171B 继续关闭百分比迁移/继承属性/灵魂整数风险，再恢复 172 成长证据 |

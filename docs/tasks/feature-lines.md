...
| LINE-PRE-STAGE-2-3-PRESENTATION | Active | 在执行 Stage 2-3 前按用户复验闭合装备悬停数值、宠物页/战斗 UI 与真动画、五角色动作流畅度/视觉完整性、战斗技能 HUD 可见与原生化 | TASK-SETTINGS-191 | `feature-line-coverage/LINE-PRE-STAGE-2-3-PRESENTATION.md` | 宠物资源族与受影响角色须由证据审计后再生成家族/角色子 task | 190C 已闭合正式背包/炼丹炉分页一致性；现恢复宠物 UI 可见性证据 task |

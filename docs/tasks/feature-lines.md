...
| LINE-PRE-STAGE-2-3-PRESENTATION | Active | 在执行 Stage 2-3 前按用户复验闭合装备悬停数值、宠物页/战斗 UI 与真动画、五角色动作流畅度/视觉完整性、战斗技能 HUD 可见与原生化 | TASK-SLICE-193D | `feature-line-coverage/LINE-PRE-STAGE-2-3-PRESENTATION.md` | 猴系真值/运行投影已闭合；马系 716 状态/20 对象 verified 真值已闭合；马系现代接入及其他七族待完成 | 193D 只直接消费 193C 真值接入马系本体/技能/冰效，不改玩法数值、AI、owner 或存档 |

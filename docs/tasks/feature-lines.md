...
2026-09-21猴马时序反证排期：224C仍唯一Ready，之后执行TASK-SLICE-226，再推进下一新家族。旧P1R/P1H绿色不证明timeCount相位正确，两族完整闭合结论降级；不撤销原资源/伤害/owner证据。
...
| LINE-PRE-STAGE-2-3-PRESENTATION | Active | 在执行 Stage 2-3 前按用户复验闭合装备悬停数值、宠物页/战斗 UI 与真动画、完整宠物战斗公共类、怪物/角色/宠物伤害数字与连击反馈、五角色动作流畅度/视觉完整性、战斗技能 HUD 可见与原生化 | TASK-SLICE-226 | `feature-line-coverage/LINE-PRE-STAGE-2-3-PRESENTATION.md` | 玄龟全32合同/P1T及五关联合通过→226猴马时序重做Ready | 青龙与玄龟已闭合；猴马时序反证待226重做，再推进新家族；功能线保持Active |
...
2026-08-27 PG-017 V2 已落地：41 项字段覆盖、四形态 × P1/P2 黑盒范围链和 `attackRange`/命中帧/source owner mutation-kill 自测通过；当前错误实现的真实 verifier/P1R 稳定返回 1，旧专项不再能单独假绿。治理队列出清，唯一 Ready 改为 `TASK-SLICE-208A`；209 保持 Planned，待猴系语义 P1R=0 与 Skill 修订后恢复。

2026-08-27 `TASK-SLICE-208A` 已归档：`PetCombatRuntime` 直接消费 207 冻结的四形态 `attackRange`，范围外改为追击 sticky target、入围后保持位置并普通攻击；normal projectile 保留 target/action token，在 verified hit frame 由正式伤害入口形成 pet-source HP decrease 与 hit cleanup。八条 P1/P2 黑盒 trace、三类 mutation-kill、`pet P1R=0`、五关旅程、全系统与 build 通过；`$pet-family-reverse` 已按反证修订，唯一 Ready 恢复为 `TASK-SETTINGS-209`。当前会话缺少内置浏览器控制接口，未新增 940×590 手工观察结论。

2026-08-28 `TASK-SETTINGS-209` 已归档：`task-settings-209.pet-horse-family` 以 43 项同集合同闭合 BasePet 公共 AI/双随机/追击/时钟、horse1..4 普攻、全部继承 `sp/bd/bz/tmaoyi`、共享冰效、奥义 tracking/ice/explosion 组合、碰撞/命中/伤害、三包 owner 与 P1/P2 生命周期；复核引用 193C 的 716 状态/20 对象/716 原版基准，`unresolved=[]`。独立 handoff 与关键字段 mutation self-test 通过；Skill 第二家族证据阶段通过但 MO-003 不提前裁决。功能线继续 Active，唯一 Ready 切换为 `TASK-SLICE-210`。

2026-08-31 `TASK-SLICE-210` 已归档：horse1..4 直接消费 209 的 43 项 verified 合同，P1/P2 范围外追击、双随机普攻、全部继承技能、共享冰效、`tmaoyi` tracking/ice/explosion 组合、真实 projectile/attack-id 与 Stage1 HP decrease 同链；Formal horse body 改由 `PetCombatSnapshot/actionToken` 驱动，TestScene 删除马系具体技能直连。八条范围链、独立 verifier/mutation-kill、`pet P1H=0`、正式五关回归与 940×590 双人 Stage 1-2 零 console 通过。MO-003 第二样本满足标准并裁决“采纳”；功能线保持 Active，唯一 Ready 切换为 `TASK-SETTINGS-211`。

2026-08-31 用户反证整改 210A 已归档：原版怪物攻击对象在 `BaseBullet.checkAttack()` 中会继续检查对应出战宠物，现代缺口位于正式桥只解析英雄。现有 active attack 以宠物 Runtime 位置、宠物防御和共享 hit registry 生成 typed `PetCombatDamageEvent`，下一帧由 P1/P2 独立 Runtime 消费并驱动 HP/hurt/dead；同 attack-id 与范围外目标不重复/不误伤。另新增 localhost-only 5173 全宠物 QA 档 fixture，第 6 槽 P1/P2 各 35 形态、9 物种。唯一 Ready 仍为 `TASK-SETTINGS-211`。

2026-09-02 `TASK-SETTINGS-211` 已归档：`task-settings-211.combat-hit-feedback` 以恢复 `OtherMat1.swf` 的 20 个伤害位图、10 个五帧连击 MovieClip、character 299/298 与 AS3 producer→queue→view→reset 链，冻结普通/暴击字形、20px/50px 位距、目标/固定 HUD 锚点、弹出/上浮/淡出、五项扇出、2/9/10/99/100、40-tick 无增长清零、最高连击和 P1/P2 来源语义；23 状态 940×590 基准、Schema/重复生成通过，`unresolved=[]`。持续效果“显示数字但不新增连击”和原版 0 数字已作为反证/现代收紧差异明确记录；功能线保持 Active，唯一 Ready 切换为 `TASK-SLICE-212`。

2026-09-02 `TASK-SLICE-212` 已归档：成功且唯一的实际怪物 HP decrease 现在生成共享 `CombatFeedbackEvent`，正式五关与 TestScene 直接消费 211 的普通/暴击位图、队列、Batter 五帧动画、40-host-tick 清零和最高值；effect 只显示不增连击，miss/0/dedup/dead 无假反馈。结果页读取同一战斗会话最高连击，专项、全系统、build 与 940×590 双人 Stage 1-2 source trace/零 console 通过。功能线继续 Active，按已采纳 `$pet-family-reverse` 新建青龙证据/实现对 213/214，唯一 Ready 切换为 `TASK-SETTINGS-213`。

2026-09-03 `TASK-SETTINGS-213` 已归档：`task-settings-213.pet-dragon-family` 以 44 项同集合同闭合 dragon1..4 的 `attackRange=150`、0.8 双随机、完整本体动作、真普攻、fs 分身、sdcc、九对象 ltwj、qlaoyi 四次可选分身/trigger/免费技能链、命中治疗、pet1/StageCommon owner 与 P1/P2 生命周期；11 个显示对象和 111 个 940×590 基准均可重复生成，`unresolved=[]`。独立 verifier 与 range/ltwj-count/qlaoyi-MP/source-field mutation 通过；功能线保持 Active，唯一 Ready 切换为 `TASK-SLICE-214`。

2026-09-03 用户复验补充：5173 正式双人战斗中角色/宠物 HP 会下降，但头顶没有承伤数字；运行 trace 也只记录以怪物为 target 的 211/212 反馈。原版 `BaseHero.reduceHp()` 与 `BasePet.reduceHp()` 分别调用 `addHeroHurtMc()` / `addMonHurtMc()`，共同使用恢复 `OtherMat1.swf` 的 `pnum0..9`，现有索引已明确 211/212 未覆盖该方向。新增 `M-054/VS-072` 与 `TASK-SETTINGS-215 -> TASK-SLICE-216`，排在 213/214 之后；先冻结数值、特殊防御、P1/P2 owner、几何及时序，再实现正式五关/TestScene 消费，不重开怪物数字与连击。

2026-08-27 用户在正式保存档观察到马宠物已有攻击表现，但怪物没有可辨识受击效果，且现代缺少原版伤害数字/连击动画。代码与 940×590 运行诊断确认：共享怪物结算并不限制 Role 来源，英雄与猴系成功命中会实际扣 HP 并设置 hurt；当前马系正式桥尚无伤害解析，且所有来源的 `DamageEvent` 没有共享可见消费者。新增 `M-053/VS-071` 与 `TASK-SETTINGS-211 -> TASK-SLICE-212`，固定排在 209 生成的马系实现 210 之后；209 归档后当前唯一 Ready 已切到 210。

2026-09-05 `TASK-SLICE-214` 执行前命中资源派生、多个运行实例、端到端校准三类规模门禁，标记 Split；唯一 Ready 改为 `TASK-SLICE-214A`（完整资源准备），随后 `TASK-SLICE-214B` 原样承接 44 项合同与 P1G/正式验收。仅重排交接，未改 src 或提升复现状态；青龙完整闭合前不执行 215 或下一家族。

2026-09-05 213A 已归档并恢复214A Ready：补齐31本体动作、234逐cell状态，修正六张trigger基准与倒计时误读；同一213真值verified，15类mutation与完整检查通过。功能线继续Active，214B完整正式验收仍待实施。

...
| LINE-PRE-STAGE-2-3-PRESENTATION | Active | 在执行 Stage 2-3 前按用户复验闭合装备悬停数值、宠物页/战斗 UI 与真动画、五角色动作流畅度/视觉完整性、战斗技能 HUD 可见与原生化 | TASK-SLICE-193B | `feature-line-coverage/LINE-PRE-STAGE-2-3-PRESENTATION.md` | 猴系 626 状态/20 对象真值已 verified；其他物种仍按证据→实现串行门禁 | 193B 只消费 193A 真值接入猴系本体/对象，禁止改玩法数值或触及其他物种 |

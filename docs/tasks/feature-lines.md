...
| LINE-PRE-STAGE-2-3-PRESENTATION | Active | 在执行 Stage 2-3 前按用户复验闭合装备悬停数值、宠物页/战斗 UI 与真动画、五角色动作流畅度/视觉完整性、战斗技能 HUD 可见与原生化 | TASK-SLICE-192A | `feature-line-coverage/LINE-PRE-STAGE-2-3-PRESENTATION.md` | 宠物资源族与受影响角色须由证据审计后再生成家族/角色子 task | 191 已定位页面链可见、战斗 character 662 HUD 未消费；现固化正式冷启动页面旅程 |

...
| LINE-CORE-PROGRESSION-COMPLETION | Active | Stage 2-3 前完成炼丹炉左页与四功能、全装备 UI/数值、五角色成长、存档扩展、关卡左下五入口、用户确认的 UI 整改及复评确认的同线纠错 | TASK-SETTINGS-175A | `feature-line-coverage/LINE-CORE-PROGRESSION-COMPLETION.md` | 无 | 175 已分级九个页面/宿主：宠物、法宝、地图 host 为明确占位，其余为旧审计 manifest 迁移债务；下一步宠物 932 真值 |

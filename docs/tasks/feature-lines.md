...
| LINE-PRE-STAGE-2-3-PRESENTATION | Active | 在执行 Stage 2-3 前按用户复验闭合装备悬停数值、宠物页/战斗 UI 与真动画、完整宠物战斗公共类、怪物伤害数字/连击反馈、五角色动作流畅度/视觉完整性、战斗技能 HUD 可见与原生化 | TASK-SLICE-212 | `feature-line-coverage/LINE-PRE-STAGE-2-3-PRESENTATION.md` | 211 已冻结普通/暴击数字、队列、连击/最高值与来源参与矩阵；下一步让实际 HP decrease 成为 Role/宠物/法宝共享反馈 producer | 212 必须直接消费 211 truth；不得用攻击动画/hurt phase 自证数字，不扩玩家/宠物受伤或治疗数字 |

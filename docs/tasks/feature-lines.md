...
| LINE-PRE-STAGE-2-3-PRESENTATION | Active | 在执行 Stage 2-3 前按用户复验闭合装备悬停数值、宠物页/战斗 UI 与真动画、五角色动作流畅度/视觉完整性、战斗技能 HUD 可见与原生化 | TASK-SETTINGS-193A | `feature-line-coverage/LINE-PRE-STAGE-2-3-PRESENTATION.md` | 宠物九物种已按证据→实现串行拆分；每族证据未 verified 前实现保持 Planned | 193 已定位 35 形态/38 技能映射与五恢复包 owner；现闭合猴系逐帧真值 |

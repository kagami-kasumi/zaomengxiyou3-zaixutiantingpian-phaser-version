...
| LINE-CORE-PROGRESSION-COMPLETION | Active | Stage 2-3 前完成炼丹炉左页与四功能、全装备 UI/数值、五角色成长、存档扩展、关卡左下五入口、用户确认的 UI 整改及复评确认的同线纠错 | TASK-SLICE-181 | `feature-line-coverage/LINE-CORE-PROGRESSION-COMPLETION.md` | 无 | 宠物页已直接消费 verified 真值并删除现代覆盖；继续 181..187 逐页 manifest 直连，下一页为法宝 |

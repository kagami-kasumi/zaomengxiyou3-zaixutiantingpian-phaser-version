...
| LINE-CORE-PROGRESSION-COMPLETION | Active | Stage 2-3 前完成炼丹炉左页与四功能、全装备 UI/数值、五角色成长、存档扩展、关卡左下五入口和用户确认的 UI 整改 | TASK-SLICE-170D | `feature-line-coverage/LINE-CORE-PROGRESSION-COMPLETION.md` | 无 | 170C 已接入 164 项唯一目录、原字段单位、111 character 选择性资源和既有穿脱 owner；现闭合 verified 页面真值直连与五角色/双 owner 逐状态校准 |

...
| LINE-PRE-STAGE-2-3-PRESENTATION | Active | 在执行 Stage 2-3 前按用户复验闭合装备悬停数值、宠物页/战斗 UI 与真动画、完整宠物战斗公共类、怪物伤害数字/连击反馈、五角色动作流畅度/视觉完整性、战斗技能 HUD 可见与原生化 | TASK-SETTINGS-211 | `feature-line-coverage/LINE-PRE-STAGE-2-3-PRESENTATION.md` | 猴系 P1R=0、马系 P1H=0；下一步先冻结怪物伤害数字/连击真值，再由 212 建立共享可见反馈闭环 | 211 只做恢复源真值，不修改 `src/`；不得把攻击动画或 hurt phase 直接当作数字/连击语义 |

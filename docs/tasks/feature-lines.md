...
| LINE-PRE-STAGE-2-3-PRESENTATION | Active | 在执行 Stage 2-3 前按用户复验闭合装备悬停数值、宠物页/战斗 UI 与真动画、完整宠物战斗公共类、怪物/角色/宠物伤害数字与连击反馈、五角色动作流畅度/视觉完整性、战斗技能 HUD 可见与原生化 | TASK-SLICE-214C5 | `feature-line-coverage/LINE-PRE-STAGE-2-3-PRESENTATION.md` | C4真实战斗及源隔离trace完成；C5接正式投影/TestScene/生命周期/P1GC，父合同保留 | 214 必须保持九对象 `ltwj`、gate-only MP `qlaoyi`、P1/P2 owner 与正式/TestScene 同源；215/216 不抢占当前Ready |

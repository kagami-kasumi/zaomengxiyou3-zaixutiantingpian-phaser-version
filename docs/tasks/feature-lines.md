...
| LINE-PRE-STAGE-2-3-PRESENTATION | Active | 在执行 Stage 2-3 前按用户复验闭合装备悬停数值、宠物页/战斗 UI 与真动画、五角色动作流畅度/视觉完整性、战斗技能 HUD 可见与原生化 | TASK-SETTINGS-193C | `feature-line-coverage/LINE-PRE-STAGE-2-3-PRESENTATION.md` | 猴系 193A/193B 真值与正式运行投影已闭合；其他八族仍按证据→实现串行门禁 | 193C 只闭合马系逐帧真值和恢复 owner，不修改 `src/` 或现代 atlas |

...
| LINE-PRE-STAGE-2-3-PRESENTATION | Active | 在执行 Stage 2-3 前按用户复验闭合装备悬停数值、宠物页/战斗 UI 与真动画、完整宠物战斗公共类、五角色动作流畅度/视觉完整性、战斗技能 HUD 可见与原生化 | TASK-SLICE-208A | `feature-line-coverage/LINE-PRE-STAGE-2-3-PRESENTATION.md` | PG-017 V2 已让错误猴系的四形态 × P1/P2 trace 和 P1R 稳定失败；当前只整改 frozen attackRange 的追击→攻击→命中/来源伤害→清理链，Skill/MO-003 仍修订中 | 208A 语义 P1R=0 并修订 Skill 后恢复 209 |

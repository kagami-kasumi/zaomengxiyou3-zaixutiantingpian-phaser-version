...
| LINE-PRE-STAGE-2-3-PRESENTATION | Active | 在执行 Stage 2-3 前按用户复验闭合装备悬停数值、宠物页/战斗 UI 与真动画、完整宠物战斗公共类、五角色动作流畅度/视觉完整性、战斗技能 HUD 可见与原生化 | TASK-SETTINGS-209 | `feature-line-coverage/LINE-PRE-STAGE-2-3-PRESENTATION.md` | 207/208 已闭合首个完整猴系并使 P1R=0；Skill 已按实际案例重写但仍需第二家族验证 | 用更新后的 Skill 闭合 horse1..4 完整证据，再单独生成正式实现/运行 task |

...
- [x] V6 与现代运行时只以 `player.soulCount` 为灵魂唯一 owner，V1..V5 无损迁移。
- [x] 技能、炼丹炉、法宝等已知消费者统一消费当前玩家字段，P1/P2 隔离并经重载验证。
...
2026-07-24 回归补充：技能页与工坊页现统一使用 `FormalSoulBalanceView` 投影当前 `player.soulCount`；跨页进入目标功能时丢弃缓存 model 并从当前存档重建，避免旧余额显示或回写。用户先后否决黑底遮盖和浏览器字体近似后，工坊改用从恢复 SWF character 119 选择性派生、仅剥离 `txtlh` 实例的原生 SVG；共享组件直接复用 DefineEditText 103 嵌入的 `FZCuYuan-M03` 数字轮廓、原缩放/基线/advance，并按两页原字段坐标右对齐。双页 940×590 证据见 `FORMAL-SOUL-BALANCE-*-original-ui.png`。

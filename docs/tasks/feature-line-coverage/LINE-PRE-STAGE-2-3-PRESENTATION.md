...
| 宠物战斗公共类 | 203/204A 建立旧骨架；205 闭合 35 形态基类证据；206 冻结组合设计；204B 让结构 P1/P1B gate=0；207 冻结猴系 41 项完整证据与 P1R 输入 | 玩家可见合同仍失败：Runtime 无 Scene/正式消费者，`basicAttack` 无真实命中/伤害，尚无完整家族从 verified 证据跑到正式自主战斗；其余物种与旧入口后置 | 208 以 `P1R=0`、P1/P2 五关真实自主战斗和 940×590 运行证明首个参考家族；后续逐族生成，最终 all=0 |
| 宠物真动画 | 193 已冻结 35 形态/38 技能映射；193A..193D 已闭合猴/马局部视觉真值与投影；207 已把猴系视觉引用与 AI/技能/命中伤害/owner/lifecycle 合为完整证据 | 猴系尚未证明正式 Runtime 对 41 项合同的运行消费；其余七族未开始 | 208 完成猴系正式复现并形成可追溯 Skill；旧 193E..R 撤销，后续每个家族按“完整逆向→完整正式复现”生成；194 最终跨族校准 |
...
18. `TASK-SLICE-208`：Ready；直接消费 207，完成猴系四形态正式 P1/P2 自主战斗和全部运行验收；只有通过后才使用 `$skill-creator` 从完整案例重写 `$pet-family-reverse`。
19. 旧 `TASK-ARCH-204C..G` 与 `TASK-SETTINGS-193E..TASK-SLICE-193R` 全部撤销；208 后只依据经验证 Skill 为一个家族生成一组连续完整任务，完成前不切换家族。
20. `TASK-SLICE-194`：所有按新方法生成的完整家族任务完成后，做 P1/P2、跨物种、页面↔战斗↔存档的最终校准。
21. `TASK-SETTINGS-195`：建立五角色同一帧时序/转移/加载对照，按证据生成“每受影响角色一 task”并插入 196 之前。
22. `TASK-SLICE-196`：五角色统一动作流畅度、UI 完整度和正式 Runtime 校准；不代替单角色修复。
23. `TASK-SETTINGS-197 -> TASK-SLICE-198 -> TASK-SLICE-199`：技能功能页保持独立已有证据；本批只闭合战斗技能 HUD 真值、可见投影和运行联动。
24. `TASK-SLICE-200`：集中正式旅程与整线关闭；通过后才恢复 `LINE-STAGE-2-3 / TASK-SETTINGS-064`。
...
- [ ] 首个完整参考家族 monkey1..4 从原版证据、普通攻击/全部技能、真动画、命中/伤害、owner/生命周期到正式 P1/P2 运行全部闭合，`P1R=0`。
- [x] 205 已闭合原版 `BasePet` 专项证据，206 已冻结唯一现代设计和真实失败基线；其 204B..G 横向串行安排已由用户运行反证撤销。
- [ ] 从完整猴系案例沉淀并经第二家族验证 `$pet-family-reverse`；再逐族完成差异接缝、TestScene/正式五关/功能页迁移和旧入口清零，最终 `all=0` 并退出设计验收。

...
| 宠物真动画 | 九物种技能行为与部分占位 projectile 已有；193 已冻结 35 形态/38 技能映射、五恢复包与 0 未定位符号 | 精确动作行、帧时序/持帧、注册点/矩阵、load precedence 与正式运行消费仍未闭合 | 193A..193R 按猴/马/UFO/虎/龟/凤/龙/兔/鼠执行 verified 证据→实现；194 最终跨族校准 |
...
6. `TASK-SETTINGS-193A -> TASK-SLICE-193B -> ... -> TASK-SETTINGS-193Q -> TASK-SLICE-193R`：逐物种生成 verified 逐帧真值后接入；证据未闭合时配对实现不得 Ready。
7. `TASK-SLICE-194`：所有宠物资源族子 task 完成后，做 P1/P2、跨物种、页面↔战斗↔存档的最终校准。
8. `TASK-SETTINGS-195`：建立五角色同一帧时序/转移/加载对照，按证据生成“每受影响角色一 task”并插入 196 之前。
9. `TASK-SLICE-196`：五角色统一动作流畅度、UI 完整度和正式 Runtime 校准；不代替单角色修复。
10. `TASK-SETTINGS-197 -> TASK-SLICE-198 -> TASK-SLICE-199`：技能功能页保持独立已有证据；本批只闭合战斗技能 HUD 真值、可见投影和运行联动。
11. `TASK-SLICE-200`：集中正式旅程与整线关闭；通过后才恢复 `LINE-STAGE-2-3 / TASK-SETTINGS-064`。

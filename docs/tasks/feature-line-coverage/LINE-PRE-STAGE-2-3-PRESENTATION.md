...
| 宠物真动画 | 九物种技能行为与部分占位 projectile 已有；193 已冻结 35 形态/38 技能映射；193A/193B 已闭合猴系真值/运行投影；193C 已闭合马系 716 状态/20 对象真值 | 马系现代接入及其余七族逐帧证据/实现未闭合 | 猴系 4 本体 atlas、9 唯一对象序列已直连真值；马系本体、sp/bd/bz/tmaoyi、共享冰效与三包 owner 已 verified，下一步 193D 接入；193E..193R 继续其余七族；194 最终跨族校准 |
...
9. `TASK-SLICE-193D -> ... -> TASK-SETTINGS-193Q -> TASK-SLICE-193R`：先接入马系 verified 真值，再对其余七族按 verified 证据→实现串行；证据未闭合时配对实现不得 Ready。
10. `TASK-SLICE-194`：所有宠物资源族子 task 完成后，做 P1/P2、跨物种、页面↔战斗↔存档的最终校准。
11. `TASK-SETTINGS-195`：建立五角色同一帧时序/转移/加载对照，按证据生成“每受影响角色一 task”并插入 196 之前。
12. `TASK-SLICE-196`：五角色统一动作流畅度、UI 完整度和正式 Runtime 校准；不代替单角色修复。
13. `TASK-SETTINGS-197 -> TASK-SLICE-198 -> TASK-SLICE-199`：技能功能页保持独立已有证据；本批只闭合战斗技能 HUD 真值、可见投影和运行联动。
14. `TASK-SLICE-200`：集中正式旅程与整线关闭；通过后才恢复 `LINE-STAGE-2-3 / TASK-SETTINGS-064`。

...
| 宠物战斗公共类 | `PetCombatRuntime/PetBehavior/Registry/Targeting` 四个 P1 角色、同步跟随/目标/策略/效果/销毁顺序和专项合同已存在 | 缺九物种真实 Behavior、35 形态默认 Registry、TestScene/五关/功能页消费者和旧入口清零 | 203 仅证明 `pet P1=0`；204A..F 必须闭合 P1B/P1C/P1D/P2/P3/P4 与 `all=0` 后才算完整宠物公共类 |
...
13. `TASK-ARCH-204`：Split；204A Done，公共 Runtime 与 Monkey/Horse 8 形态已通过 P1B；当前从 204B 连续闭合其余七物种 Behavior、35 形态 Registry、TestScene/正式五关、功能页同步与旧入口，`pet all=0` 前不恢复逆向任务。
14. `TASK-SETTINGS-193E -> ... -> TASK-SETTINGS-193Q -> TASK-SLICE-193R`：204F 完成后恢复，对其余七族按 verified 证据→实现串行；七个证据 task 强制使用 `$pet-family-reverse` 和 `MO-001`，在单 task 内以 Luna A/B 并行只读调查、主 agent 单写归并、Luna 独立完整性复核，不并行 task 状态；证据未闭合时配对实现不得 Ready。
15. `TASK-SLICE-194`：所有宠物资源族子 task 完成后，做 P1/P2、跨物种、页面↔战斗↔存档的最终校准。
16. `TASK-SETTINGS-195`：建立五角色同一帧时序/转移/加载对照，按证据生成“每受影响角色一 task”并插入 196 之前。
17. `TASK-SLICE-196`：五角色统一动作流畅度、UI 完整度和正式 Runtime 校准；不代替单角色修复。
18. `TASK-SETTINGS-197 -> TASK-SLICE-198 -> TASK-SLICE-199`：技能功能页保持独立已有证据；本批只闭合战斗技能 HUD 真值、可见投影和运行联动。
19. `TASK-SLICE-200`：集中正式旅程与整线关闭；通过后才恢复 `LINE-STAGE-2-3 / TASK-SETTINGS-064`。
...
- 在 204A..F 之外顺手扩大迁移范围，或把已冻结的 Runtime + Behavior 组合设计改成深继承体系。
...
- [ ] 204A..F 完成九物种 Behavior、35 形态 Registry、TestScene/正式五关/功能页迁移和旧入口清零，`pet all=0` 且设计验收退出。

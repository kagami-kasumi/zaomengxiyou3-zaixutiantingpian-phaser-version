...
本文是 `LINE-STAGE-1-1` 的权威覆盖入口。该线曾由 `TASK-SLICE-123`、`TASK-SETTINGS-050` 与 `TASK-SLICE-124` 关闭；2026-07-23 用户试玩回归由 `TASK-SLICE-147`、`TASK-SLICE-148`、`TASK-SLICE-149` 修复后重新关闭。
...
无。`TASK-SLICE-147`、`TASK-SLICE-148`、`TASK-SLICE-149` 已归档，本线没有未完成 task。
...
- 2026-07-23 回归：停点不再锁死已上行玩家镜头；Monster72 调试靶已移除；门内上键与结果 UI 合同有专项保护；940×590 正式进入 1-1、返回地图、再选 1-2 成功且 console 干净。
- 2026-07-23 二次回归：1-1 删除矩形 frame/glow/DOOR 文本，直接复用 Stage 1-3 已接入的原版 W 门；最高层 Boss 触发与所有停点、小怪清理状态解耦，并由专项测试保护。
- 2026-07-23 最高层镜头：原版 `-1950 + 2370 = 420` 屏幕坐标已映射为现代 `420/590` 构图；Boss 立即出现后相机继续用原版 2 秒时长平滑收敛。

...
| VS-044 炼丹炉第二配方真图标 | 待机制 | 扩展 `kyg + kyz + kys -> kyl` 四图标并复用已完成视觉/事务管线 | M-035、M-039、VS-043 | `TASK-SETTINGS-045` 图标证据；后续 `TASK-SLICE-118` 选择性派生、manifest 与视觉测试 | 四图标可追溯到精确源包/character；材料槽与预览显示真图标，既有配方/库存事务不变 |

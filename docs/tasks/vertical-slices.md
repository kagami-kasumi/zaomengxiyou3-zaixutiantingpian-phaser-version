...
| VS-066 装备悬停数值闭环 | 部分实现 | 正式背包已穿戴槽/装备格及原版存在的其他消费者 hover 显示同一装备实例的权威有效数值 | M-036、M-037、M-052 | `TASK-SETTINGS-189`（已完成）、`TASK-SLICE-190A/190B1/190B2/190B3`（已完成）、`TASK-SLICE-190B4` | 正式背包、强化、合成、分解均已闭合；190B3 的目标实例/拒绝/成功/返还/P1-P2 通过且六结果排除。只余 190B4 打造产物与商城负合同，不新建 equipment owner |

...
| VS-064 完整背包资源基础闭环 | 待机制 | 原版 1.1 可入包资源全集使用统一身份、真图标、四分类、堆叠/实例、容量、P1/P2 owner 与 V6 往返，并能被丹药/商城/任务/掉落复用 | M-035、M-036、M-037、M-044、M-052 | `GOAL-049 / TASK-SETTINGS-070` 先闭合权威目录；`GOAL-050 / TASK-SLICE-160` 再实现 | 现有页面切片不等于全量内容闭合；物品专属使用效果不属于本切片，但未实现状态必须显式可见 |

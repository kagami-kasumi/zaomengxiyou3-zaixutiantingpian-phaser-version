...
| VS-019 `monkey3/lyq` 宠物技能最小闭环 | 可开始 | 当前出战 `monkey3` 释放 `lyq` 主动技能 | M-042、M-032、M-033、`pets-index.md` | `PetSystem.ts`、`ProjectileSystem.ts`、`AssetManifest.ts`、`TestScene.ts`、`system-tests.ts` | P1 可切到或种下出战 `monkey3`，并持有已学 `lyq`；MP `>= 20`、冷却就绪、存在 `Monster30` 目标且距离不超过 `400` 时释放；释放扣 20 MP、重置冷却，生成可见占位效果并造成 `6.8 * pet.atk` 等价伤害；系统测试覆盖未学习、MP 不足、冷却、距离门禁、无目标、伤害和扣 MP |

## 第一批推荐执行顺序

1. `TASK-SLICE-044`：`monkey3/lyq` 宠物技能最小闭环。

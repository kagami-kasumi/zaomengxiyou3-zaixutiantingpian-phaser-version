...
| VS-045 Stage 1-2 真场景与布局基础 | 可开始 | 接入真实横向场景、背景、公共地面、普通门、特殊入口和完整显式地图标记，并从现有入口进入已解锁 1-2 | M-026、M-027、M-035、`levels-index.md` | `AssetManifest.ts`、Stage 1-2 layout/scene bridge、入口路由、专项测试 | character 25/135/1/22/52 provenance 与视觉可核对；3+1 墙、5 停点、13 刷怪点、普通门和特殊入口数据完整；运行时可进入 1-2，但波次/门禁/特殊入口行为由后续切片实现 |

...
| VS-053 天庭地图选关闭环 | 整改中 | 从当前存档进入天庭地图，点击已解锁节点后按存档队伍直接进入，结算/退出后返回并保持进度 | M-005、M-006、M-027、M-044、M-051 | `HeavenMapSystem.ts`、`HeavenMapScene.ts`、结果/退出桥、`heaven-map-tests.ts`、`TASK-SLICE-153` | 真地图、节点状态、进度和往返保持；用户否决逐关人数 chooser，待地图/关卡/HUD/功能页/重试统一消费存档队伍 |
...
| VS-055 技能页面 UI 原生化闭环 | 整改中 | 技能总页、主动页、绑定页和被动页复用原图片视觉，并且只显示当前存档活动 owner 的当前角色技能 | M-016、M-041、M-044、M-052、PG-007 | `skill-ui-native-index.md`、`TASK-SLICE-143-visual-audit.md`、`FormalSkillPageView.ts`、`TASK-SLICE-152` | 真 UI 与技能事务保持闭合；待移除临时 `playerCount` owner 来源，单人只显示 P1，双人 P1/P2 各自只显示存档当前角色技能 |

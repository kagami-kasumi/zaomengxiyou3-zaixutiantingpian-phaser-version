...
| VS-054 完整功能 UI 闭环 | 炼丹炉原生导航待整改，之后再做端到端旅程 | 背包、装备、宠物、心法/技能和法宝从正式导航可达，具备真 UI、双玩家边界和存档集成 | M-016、M-036、M-037、M-041、M-042、M-043、M-052 | `full-function-ui-index.md`、`equipment-workshop-index.md`、共享 host/V4、正式背包/技能/宠物/工坊/法宝页面、已完成 `TASK-SETTINGS-060`、`TASK-SLICE-141`、`TASK-SLICE-140` | 工坊业务事务与原生按钮证据已闭合；当前须移除现代顶层导航，直接复用左到右“强化 / 合成 / 分解 / 打造”的原生位置、四态和命中区，完成整改后再进行完整玩家旅程和功能线关闭验收 |

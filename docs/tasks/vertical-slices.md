...
| VS-042 合成最小闭环 | 已完成 | 三材料无序合成，验证灵魂门禁、原子库存事务和首个装备属性继承 | M-039、M-036、M-037、`crafting-index.md` | `CraftingRecipeRegistry.ts`、`CraftingSystem.ts`、库存/UI 接线、`crafting-tests.ts` | `tlzsp x3 -> wptlz` 兼容入口保留；67 个 `direct_static` 唯一配方可合成；`kyg + kyz + kys -> kyl` 按三个装备实例平均继承 HP/MP/攻击/防御；混合/重复材料按无序多重集合匹配，1000 灵魂、容量预检、失败无副作用和 P1/P2 隔离保持不变 |

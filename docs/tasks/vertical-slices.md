...
| VS-043 炼丹炉视觉最小闭环 | 已完成 | 用 RegiMA 真资源呈现 1000×600 炼丹炉并复用既有合成交互 | M-039、M-035、M-037、VS-042、`crafting-ui-index.md` | `public/assets/ui/crafting/`、`AssetManifest.ts`、`CraftingUILayout.ts`、`TestSceneCraftingView.ts`、`crafting-tests.ts` | character 119/169、五角色双帧选择器与 `tlzsp/wptlz` 已选择性派生；P1/P2、三槽、预览、灵魂门禁、成功回包、失败保留、指定槽移除与关闭退回均复用既有系统并自动验证；派生 PNG 人工检查通过，运行时截图因会话无浏览器后端未生成 |
| VS-044 炼丹炉第二配方真图标 | 已完成 | 扩展 `kyg + kyz + kys -> kyl` 四图标并复用已完成视觉/事务管线 | M-035、M-039、VS-043 | `public/assets/ui/crafting/items/`、`AssetManifest.ts`、`CraftingSystem.ts`、`InventorySystem.ts`、`PlayerInventoryOwnershipSystem.ts`、`TestSceneCraftingView.ts`、`crafting-tests.ts` | EIcon1 character 332/342/323/809 已选择性派生并可追溯；P1/P2 各有独立装备实例，三槽、预览、1000 灵魂失败/成功、清槽与回包自动验证通过；首配方不回归；四 PNG 人工目检通过，运行时截图受浏览器后端与本地服务网络隔离限制未生成 |

...
| VS-059 天庭地图四服务页闭环 | 待机制 | 完成丹药、商城、设置、任务四入口的原版内容、真 UI、离线边界、正式往返与存档影响 | M-035、M-044、M-046、M-052、VS-064 | 四页六段证据均已闭合；先完成 `GOAL-049/050` 背包资源前置，再执行 `GOAL-045..048 / TASK-SLICE-155A..D` | 丹药事务依赖全量物品目录与统一背包原子事务；前置和四页现代实现、正式往返、存档验收均未完成，不能提升状态 |
| VS-060 关卡内五功能入口闭环 | 待机制 | 设置、技能、背包、法宝、宠物在正式关卡内按原版状态可点击并正确暂停/返回 | M-016、M-035、M-043、M-052 | `GOAL-038` / `TASK-SETTINGS-067`，实现父任务 `TASK-SLICE-156` | 既有四类功能页业务可复用；设置页、真按钮三态、P1/P2 限制与逐关接线须重新证明 |
| VS-061 已完成关卡怪物真动画闭环 | 待机制 | Stage 1-1/1-2/1-3 全部实际怪物及攻击对象真动画闭合，Stage 2-1/2-2 防回归 | M-030、M-034、M-035 | `GOAL-039` / `TASK-SETTINGS-068`，实现父任务 `TASK-SLICE-157` | Stage 1-3 明确仍为占位外观；禁止用代表性怪物或单帧替代全集 |
...
| VS-064 完整背包资源基础闭环 | 可开始 | 原版 1.1 可入包资源全集使用统一身份、真图标、四分类、堆叠/实例、容量、P1/P2 owner 与 V6 往返，并能被丹药/商城/任务/掉落复用 | M-035、M-036、M-037、M-044、M-052 | `GOAL-049 / TASK-SETTINGS-070` 已闭合 431 项权威目录；当前 `GOAL-050 / TASK-SLICE-160` 接入现代实现 | 证据输入已清零身份、分类、容量和存档语义未知；3 项原版图标缺陷必须按目录默认排除，不得静默使用占位，物品专属使用效果继续显式标记待实现 |

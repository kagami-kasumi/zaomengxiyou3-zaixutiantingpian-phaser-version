...
## 2026-07-25 被动页表头与效果说明反馈修正

用户复验指出被动页空表头和裸数值不满足当前项目可读性标准。复核原版显示列表与 AS3 后确认：

- character 213 原显示列表没有表头文字，顶部黑色区域确实为空；
- `PassiveSkill.analy()` 会向 `lastvalue` / `attvalue` 写入“生命上限增加 100”等完整说明，现代实现此前只保留数值，属于信息丢失；
- 当前效果与下级效果恢复五类原版完整说明，原 212 行、207 按钮、成本、等级与存档规则不变。

首轮按“标准项目”理解在空白区域补了六列表头，但用户复验指出位置和字体仍偏离，并明确要求完全复现原版。最终撤销该现代可见例外，恢复原版空表头；所有动态字段改用恢复 SWF character 200 的内嵌 `FZCuYuan-M03`，进入技能资源时先等待原字体加载，不再回退浏览器系统字体。

最终浏览器按单人存档 1 从正式地图进入技能页：`document.fonts` 明确返回 `FZCuYuan-M03` 已加载，表头保持原版为空，五种下级效果使用原字体完整显示且未侵入升级按钮或成本列；当前等级为 0 时当前效果保持原版 `----`。940×590 游戏画面 console warning/error 为 0。

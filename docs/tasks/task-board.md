...
`TASK-SETTINGS-222` 是当前 Ready 游戏执行项。执行合同见 [定义](task-definitions/TASK-SETTINGS-222.md)。
...
| TASK-SETTINGS-222 | Ready | LINE-PRE-STAGE-2-3-PRESENTATION | 玄龟完整家族视觉/碰撞真值 | 13源对象全行/逐帧/动态组合与独立碰撞输入 | M-032、M-034、M-035、M-042、VS-012、VS-067 | verified视觉合同与正式实现交接 | 同线玄龟资源准备/正式实现 | [定义](task-definitions/TASK-SETTINGS-222.md) |

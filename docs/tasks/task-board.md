...
`TASK-SLICE-140` 是唯一当前推荐，属于唯一 `Active` Goal `GOAL-008` 和功能线 `LINE-FORMAL-GAME-LOOP`。上一 task 已移除工坊顶部现代导航与全屏暗罩，按 119/95/99/109/113 证据接回左下侧“强化 / 合成 / 分解 / 打造”四态按钮；当前只执行完整功能 UI 和正式主循环旅程验收。

## 待完成任务

| Task | 状态 | Goal | 功能条线 | 类型 | 目标 | 目标机制/切片 | 输出 | 下一步 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| TASK-SLICE-140 | Ready | GOAL-008 | LINE-FORMAL-GAME-LOOP | 端到端闭环 | 验收完整功能 UI 与正式主循环旅程 | M-005、M-016、M-044、M-050、M-051、M-052、VS-054 | 自动旅程、P1/P2 浏览器验收、覆盖台账与功能线关闭证据 | 关闭本线后恢复 `TASK-SETTINGS-053` |

...
- `TASK-SLICE-045`：`monkey3/xj` 宠物技能最小闭环。`monkey3/lyq` 已接通，下一步扩展三阶猴 skill2 `xj` 的已学技能、MP/冷却门禁、占位效果和 `Monster30` 伤害闭环。

## 待完成任务

| Task | 状态 | 类型 | 目标 | 目标机制/切片 | 产物 | 下一步 |
| --- | --- | --- | --- | --- | --- | --- |
| TASK-SLICE-045 | Ready | TASK-SLICE | `monkey3/xj` 宠物技能最小闭环 | VS-020、M-042 | `PetSystem.ts`、`ProjectileSystem.ts`、`AssetManifest.ts`、`TestScene.ts`、`system-tests.ts`、`mechanics-index.md`、`vertical-slices.md`、`task-board.md`、`task-history.md` | 实现 P1 出战 `monkey3` 的已学 `xj`、MP/冷却门禁、占位效果和 `Monster30` 伤害闭环 |
...
- P1 可切到或种下出战 `monkey3`，并持有已学 `xj`。
- `xj` 只在已学习、宠物 MP `>= 20`、冷却就绪且存在 `Monster30` 目标时释放。
...
- `npm run test:systems` 通过，覆盖已学习/未学习、MP 不足、冷却门禁、无目标、伤害和扣 MP。
- `npm run build` 通过。
...
- 不重构宠物系统；只复用 `VS-016` 至 `VS-019` 已建立的最小技能链路并扩展 `monkey3/xj`。

状态更新：

- 完成后更新 `docs/reverse-engineering/mechanics-index.md` 的 `M-042` 和 `docs/tasks/vertical-slices.md` 的 `VS-020` 状态。
- 完成后从本文移入 `docs/tasks/task-history.md`。

推荐后续任务：

- `TASK-SLICE-045` 完成后，再决定是扩展 `monkey3/lj` 受击触发链路，还是补宠物技能存档/面板。

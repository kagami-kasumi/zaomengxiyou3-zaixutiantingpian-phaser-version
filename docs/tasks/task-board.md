...
`TASK-SETTINGS-175A` 是唯一 Ready 游戏 task；175 已完成逐页分级，下一步只闭合宠物 932 的显示列表、原版基准与 verified 真值。

## 待完成任务

| Task | 状态 | 功能条线 | 类型 | 目标 | 目标机制/切片 | 输出 | 下一步 | 定义 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| TASK-SETTINGS-175A | Ready | LINE-CORE-PROGRESSION-COMPLETION | 宠物页真值逆向 | 闭合 pet1 932 的完整显示列表、动态状态与原版基准 | M-035、M-042、M-052、VS-054 | `task-settings-175a.pet-page` verified manifest 与单页实现合同 | TASK-SETTINGS-175B；verified 后生成宠物实现 task | [定义](task-definitions/TASK-SETTINGS-175A.md) |
| TASK-SETTINGS-175B | Planned | LINE-CORE-PROGRESSION-COMPLETION | 法宝页真值逆向 | 闭合 backpack1 596 的字段、按钮/确认态与基准 | M-035、M-043、M-052、VS-054 | `task-settings-175b.magic-weapon-page` verified manifest | TASK-SETTINGS-175C | [定义](task-definitions/TASK-SETTINGS-175B.md) |

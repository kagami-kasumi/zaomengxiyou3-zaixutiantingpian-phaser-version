...
`TASK-SLICE-180` 是唯一 Ready 游戏 task；175A..I 真值迁移全部闭合，现按单页有界批次开始宠物页 manifest 直连。

## 待完成任务

| Task | 状态 | 功能条线 | 类型 | 目标 | 目标机制/切片 | 输出 | 下一步 | 定义 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| TASK-SLICE-180 | Ready | LINE-CORE-PROGRESSION-COMPLETION | 宠物页原生化实现 | 直接消费 175A verified 真值，删除宠物页现代覆盖层 | M-035、M-042、M-052、VS-054 | 932 原显示列表投影、逐状态视觉/owner/存档回归 | 按 175A..I 调度结果排入同线实现批次 | [定义](task-definitions/TASK-SLICE-180.md) |

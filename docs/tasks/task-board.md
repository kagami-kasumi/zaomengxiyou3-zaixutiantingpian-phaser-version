...
`TASK-SLICE-216` 是当前 Ready 游戏执行项。执行合同见 [定义](task-definitions/TASK-SLICE-216.md)。
...
| TASK-SLICE-216 | Ready | LINE-PRE-STAGE-2-3-PRESENTATION | 角色/宠物承伤数字实现 | 直接消费 215 真值，让215声明的角色/宠物producer生成原版 `pnum`；同时让 localhost 5173 全宠物 QA 存档入口可发现，不影响正式存档、怪物数字/连击 | M-032、M-035、M-042、M-049、M-054、VS-072 | 正式五关/TestScene P1/P2 incoming-damage trace、5173 全宠物档可见性、逐状态差异与回归 | 继续下一未闭合宠物家族 | [定义](task-definitions/TASK-SLICE-216.md) |

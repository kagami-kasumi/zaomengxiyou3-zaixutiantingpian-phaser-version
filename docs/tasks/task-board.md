...
`TASK-SLICE-208` 是唯一 Ready 游戏 task。207 已以 `task-settings-207.pet-monkey-family` 闭合 monkey1..4 的 41 项原版合同、三套碰撞真值、现代消费者反证与 P1R 输入，`unresolved=[]`。当前直接消费该真值完成猴系正式 P1/P2 自主战斗。

现有 `$pet-family-reverse` 降级为未验证草案，旧 193E..R 的“视觉证据→动画实现”批次全部撤销，不再强制使用。207 完成猴系完整逆向但不更新 Skill；208 完成猴系正式自主战斗和 P1/P2 运行闭环后，才从该完整案例复盘并使用 `$skill-creator` 重写 Skill。之后每次只为一个家族生成完整连续任务。

## 待完成任务

| Task | 状态 | 功能条线 | 类型 | 目标 | 目标机制/切片 | 输出 | 下一步 | 定义 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| TASK-ARCH-204 | Split | LINE-PRE-STAGE-2-3-PRESENTATION | 完整宠物战斗公共类父任务 | 以一个完整参考家族证明逆向、Runtime、正式消费者和 Skill 沉淀链，再逐族扩展并最终 all=0 | M-032、M-034、M-042、VS-012、VS-067 | 207/208 先闭合猴系；后续任务只由已验证 Skill 逐族生成 | TASK-SLICE-208 | [定义](task-definitions/TASK-ARCH-204.md) |
| TASK-SLICE-208 | Ready | LINE-PRE-STAGE-2-3-PRESENTATION | 猴系完整正式复现与 Skill 沉淀 | 四形态正式自主战斗、真普通攻击/技能/动画/伤害、P1/P2 五关生命周期全部闭合后才重写 Skill | M-032、M-034、M-035、M-042、M-044、VS-012、VS-067 | pet P1R=0、正式运行闭环、完整案例复盘与经验证 `$pet-family-reverse` | 依据验证后的 Skill 生成下一个单一家族完整任务 | [定义](task-definitions/TASK-SLICE-208.md) |

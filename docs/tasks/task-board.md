...
`TASK-SLICE-186` 是唯一 Ready 游戏 task；设置页已直接消费 175G 真值并删除手写视觉源，现进入任务页 manifest 直连。

## 待完成任务

| Task | 状态 | 功能条线 | 类型 | 目标 | 目标机制/切片 | 输出 | 下一步 | 定义 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| TASK-SLICE-186 | Ready | LINE-CORE-PROGRESSION-COMPLETION | 任务页 manifest 直连 | 直接消费 175H verified 真值，删除任务页手写视觉真值源 | M-044、M-046、M-052、VS-059 | 85 投影、28 状态视觉/owner/事务/存档回归 | 按 175A..I 调度结果排入同线实现批次 | [定义](task-definitions/TASK-SLICE-186.md) |

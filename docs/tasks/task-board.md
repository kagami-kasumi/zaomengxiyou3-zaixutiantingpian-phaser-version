...
`TASK-SLICE-141` 是唯一当前推荐，属于唯一 `Active` Goal `GOAL-007` 和功能线 `LINE-FORMAL-GAME-LOOP`。上一 Goal 已闭合炼丹炉左下侧四个原生按钮的显示对象、四态、矩阵、命中区、标签顺序和切页调用链；本 task 直接按该合同移除现代覆盖导航并复用原生位置与按钮，之后才恢复端到端关闭验收。

## 待完成任务

| Task | 状态 | Goal | 功能条线 | 类型 | 目标 | 目标机制/切片 | 输出 | 下一步 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| TASK-SLICE-141 | Ready | GOAL-007 | LINE-FORMAL-GAME-LOOP | 真 UI 整改 | 移除炼丹炉现代覆盖导航，直接复用原生中文位置和原生按钮 | M-035、M-039、M-052、VS-054 | 原生导航接线、页面布局整改、自动与浏览器视觉证据 | `TASK-SLICE-140` |
| TASK-SLICE-140 | Planned | GOAL-008 | LINE-FORMAL-GAME-LOOP | 端到端闭环 | 验收完整功能 UI 与正式主循环旅程 | M-005、M-016、M-044、M-050、M-051、M-052、VS-054 | 自动旅程、P1/P2 浏览器验收、覆盖台账与功能线关闭证据 | 关闭本线后恢复 `TASK-SETTINGS-053` |
| TASK-SETTINGS-053 | Planned | GOAL-009 | LINE-STAGE-2-1 | 关卡逆向 | 正式游戏主循环关闭后，闭合 Stage 2-1 真场景、地图标记、怪物/专属机制和结果流程 | M-026、M-027、M-030、M-035、M-044、VS-049 | 六段证据矩阵、资源标注、覆盖台账与最小可玩切片任务 | 等待 `LINE-FORMAL-GAME-LOOP` 关闭后恢复 |
...
目标机制/切片：
- `M-035`、`M-039`、`M-052`、`VS-054`

输入资料：
- `TASK-SETTINGS-060` 闭合的原生按钮证据矩阵。
- `FormalWorkshopPageView.ts`、`FormalWorkshopPageSystem.ts`、character 119 容器和 198/169/177/152 四个真子页资源。

输出产物：
- 删除炼丹炉顶部现代标题、现代四标签按钮和遮住原版导航/主体的全屏暗色覆盖层。
- 直接在原版左下侧“强化 / 合成 / 打造 / 分解”的原生位置接线；使用原生按钮显示对象/帧状态，透明命中区只能承载事件，不得绘制替代按钮。
- 四个业务页继续复用既有 owner-aware 事务系统；现代动态信息只放入原版页面既有槽位或证据允许的区域，不另造悬浮面板覆盖原版布局。

完成定义：
- 四个原生按钮均可点击切换正确页面，选中/悬停/按下反馈与已确认原版状态一致；切页、换 owner、关闭返还和保存合同不回归。
- 940×590 下不再出现现代顶部页签或全屏暗罩；原版中文导航保持清晰、位置不漂移、命中区不重叠。

验收标准：
- 原生按钮位置/状态自动测试、工坊事务专项、`npm run check:all`、`git diff --check` 通过。
- 从正式地图入口完成 P1/P2 四页切换、至少一条成功/拒绝事务、关闭返回和重载；保存 940×590 截图并检查 console 无 error/warning。

禁止范围：
- 不重写强化、合成、打造、分解业务规则；不增加第五套导航；不以现代文字或矩形覆盖原生文字；不顺带执行 `TASK-SLICE-140` 的完整主循环关闭验收。

状态更新：
- 更新 `VS-054`、`M-052`、本线覆盖台账、Goal/task 状态；完成后激活 `GOAL-008` / `TASK-SLICE-140`。

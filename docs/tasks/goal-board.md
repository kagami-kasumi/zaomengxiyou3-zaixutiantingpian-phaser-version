...
| --- | --- | --- | --- | --- | --- | --- |
| GOAL-001 | Active | LINE-FORMAL-GAME-LOOP | TASK-SLICE-138A | 工坊容器、四标签、Fusion 迁入与双 owner 切页/关闭返还 | 最多 1 次 | `GOAL-002` |
| GOAL-002 | Planned | LINE-FORMAL-GAME-LOOP | TASK-SLICE-138B | 装备实例 schema/V4 迁移与强化事务 | 最多 1 次 | `GOAL-003` |
| GOAL-003 | Planned | LINE-FORMAL-GAME-LOOP | TASK-SLICE-138C | 分解事务、可注入随机与双 owner 库存一致性 | 最多 1 次 | `GOAL-004` |
| GOAL-004 | Planned | LINE-FORMAL-GAME-LOOP | TASK-SLICE-138D | 78 本可达制作书、可选宝石与产物实例持久化 | 最多 1 次 | `GOAL-005` |
...
`GOAL-001` 仅包含 `TASK-SLICE-138A`。完成条件是该 task 的代码、自动验证、必要运行验收和状态文档全部收尾。完成后将 `GOAL-001` 标为 `Done`、`GOAL-002` 标为 `Active`，然后结束当次 `/goal` 并向用户交接；不直接执行 `TASK-SLICE-138B`。

...
拆分触发：
- 一旦范围越出 虎系 单物种族、开始现代接入，或发现两个无法在本 task 内独立验证的恢复 owner，立即停止并拆分。
...
- `docs/reverse-engineering/pet-animation-corpus.json`、`pets-index.md`、`projectiles-index.md`、tigress1..4 对应 AS3、恢复源 `assets/pet1.swf`、`assets/20120203.swf`。

输出产物：
- tigress1..4 的 wait/follow、walk/warp、普攻、hy/sxhz/hsqj/bhaoyi、hurt、死亡/0 HP 动作行与行为触发/销毁矩阵。
- 精确帧数/持帧、clock、注册点、局部/世界矩阵、可见/碰撞边界及恢复包 load precedence 裁决。
- Schema-valid `task-settings-193g.pet-tigress-animation` 原版机器真值 JSON、原版逐状态基准和六段证据矩阵。

完成定义：
- 真值达到 `verified`、`unresolved=[]`，全部 虎系 body/技能对象与现代 key 一一映射；本 task 不修改 `src/` 或派生现代 atlas。
...
状态更新：
- 归档本 task，并仅激活 `TASK-SLICE-193H`。

推荐后续任务：
- `TASK-SLICE-193H`：直接消费本 task verified 真值接入 虎系 真动画。

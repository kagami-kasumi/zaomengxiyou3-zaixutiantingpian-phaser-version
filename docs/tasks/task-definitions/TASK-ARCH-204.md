...
拆分触发：
- 已触发：具体差异接缝全集、TestScene、正式五关和旧入口清零具有独立验收边界，原拆为 204A..F；204A 完成后用户指出缺少原版 `BasePet` 专项逆向，现由 205 与其后的现代设计裁决前插，204B..F 在裁决前暂停。
...
状态更新：
- `TASK-SETTINGS-205` 已完成；当前先完成 `TASK-ARCH-206` 的同线现代设计裁决，再决定如何恢复/重排 204B..F；最终迁移 task 完成时收束父任务并仅激活 `TASK-SETTINGS-193E`。

...
2026-08-02 复盘说明：上述实体 owner 是目标合同，不是全部关卡的当前实现事实。V2B..V2F 已把 Stage 1-2/1-3/2-1/2-2 正式与 DEV 以及 Stage 1-1/TestScene 兼容面的英雄生命周期迁入共享 `HeroPartyRuntime`；四个正式关卡及 TestScene 的怪物 owner 仍未迁移。在 V2 全部迁移前不得继续把其余 GameplayBridge 称为已经闭合的窄 adapter。
...
- Stage 1-2/1-3/2-1/2-2 正式与 DEV 及 Stage 1-1/TestScene 的 Hero owner token 预算已归零；横向关卡强制包含 `createHeroPartyRuntime`，TestScene 强制经 `createTestSceneHeroPartyRuntime` 提交分阶段帧。允许值只能随迁移缩减。任何新正式关卡/adapter 声明 `PlayerRuntime`、`EnemyRuntime/MonsterRuntime`、实体 `Map`，或直接调用英雄/怪物内部 update/resolve 都失败。
...
- V2B..V2F Hero owner：`HeroPartyRuntimeSystem.ts`、`HeroPartyRuntimeBridge.ts`、`TestSceneHeroPartyRuntimeBridge.ts`、`hero-party-runtime-tests.ts`；四个横向正式关卡及 Stage 2-2 DEV 只提交环境并消费快照/事件，Stage 1-1 只经兼容桥提交分阶段帧并查询共享成员。

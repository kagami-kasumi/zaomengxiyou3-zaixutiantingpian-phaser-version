...
export function verifyDragonConsumers(forms: readonly number[], output: string, levels?: readonly (11 | 12 | 13 | 21 | 22)[]) {
...
const rows:any[]=[];
for(const form of forms) for(const stage of levels ?? ['formal','TestScene']) for(const reason of ['retry','return','reload']) {
  const level = typeof stage === 'number' ? stage as 11 | 12 | 13 | 21 | 22 : undefined;
  const entry = level === 11 || stage === 'TestScene' ? 'TestScene' : 'formal';
  const environment = level ? getPetGroundEnvironment(level) : {ownerRootOffsetY:0,walls:[{id:'floor',left:-10000,right:10000,top:0,bottom:20,usesWallTolerance:true}]};
  const wall = level ? environment.walls.find(w=>!w.throughDown && w.right-w.left>800)! : environment.walls[0]!;
  const x = level ? wall.left+300 : 0;
  const y = wall.top - 0.1 - getPetDragonCollision(form).height/2;
  const displays:any[]=[];
  ...
  const rosters=Object.fromEntries(['p1','p2'].map(slot=>{
    ...
    roster.pets.forEach(p=>{p.id=slot+'-'+p.id;p.isActive=p.species==='dragon'&&p.form===form;
      if(p.isActive)Object.assign(p,{hp:500,maxHp:1000,mp:1000,maxMp:1000,atk:100,skills:['fs','sdcc',...(form>=3?['ltwj']:[]),...(form===4?['qlaoyi']:[])]});});
    return [slot,roster];
  }));
  const runtimes={p1:new PetCombatRuntime(),p2:new PetCombatRuntime()};
  const snapshots:any={};
  const combat=createStage1CombatRuntime();
  const model={combat,members:['p1','p2'].map(slot=>({movement:{x,y:y-environment.ownerRootOffsetY,facingX:1},combat:{slot,combat:{state:'ready'}}}))};
  const projectiles=createProjectileSystem();
  const monster=createMonster30(x+100,y);monster.hp=monster.maxHp=1000000;
  const owners:string[]=[];
  ...
  const step=()=>{
    ...
    update({targets:[{id:monster.id,x:monster.x,y:monster.y,isAlive:monster.hp>0}],combatEnemies:enemies,
      ...
      groundEnvironmentFor:()=>environment});
  };
  for(let i=0;i<220;i++) {
    step();
    if (form === 4 && i === 1) for (const roster of Object.values(rosters)) {
      const pet = roster.pets.find(p=>p.isActive)!;
      for (const key of ['dragon1Fs','dragon2Sdcc','dragon3Ltwj'] as const) pet.skillState![key].cooldownMs=100000;
      pet.skillState!.dragon4Qlaoyi.cooldownMs=0;
    }
  }
  ...
  if(form===4) {
    assert.ok(displays.some(d=>d.key.includes('PetDragonBullet4')),'production presenter must display the real trigger');
    assert.ok(displays.some(d=>d.key.includes('AoyiBuff')),'disabled buff remains visible');
    for(const owner of ['p1','p2']) assert.ok(combat.audit.damageEvents.some(e=>e.sourceId.startsWith(owner+'-')&&e.actionName==='hit4'));
  }
  ...
  model.members[0]!.combat.combat.state='dead';step();assert.ok(snapshots.p1.runtime);
  {
    ...
    rows.push({form,level,entry,reason,oldKey,damage:combat.audit.damageEvents.length,
...
writeFileSync(`${output}/consumer-traces.json`,JSON.stringify({
...
console.log(`Dragon production consumers: ${rows.length} form/level/lifecycle groups, P1/P2 damage and cleanup passed.`);
}

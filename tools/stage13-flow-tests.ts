...
assert.equal(single.tryComplete(createTestLevelCompletionAttempt()), true, 'remaining flying adds do not block victory');
assert.deepEqual(single.unlockProgress, { unlockedStage: 2, unlockedLevel: 1 });
assert.equal(single.tryComplete(createTestLevelCompletionAttempt()), false, 'victory is idempotent');
...
assert.equal(failure.updatePartyFailure(1, 10_000), 'playing');
assert.equal(failure.updatePartyFailure(0, 16), 'failure-pending');
assert.equal(failure.updatePartyFailure(0, Stage13FailureDelayMs - 1), 'failure-pending');
assert.equal(failure.updatePartyFailure(0, 1), 'failed');
...
assert.equal(ahead.tryComplete(createTestLevelCompletionAttempt()), true);

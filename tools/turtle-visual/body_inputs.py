...
OUT = ROOT/'docs/tasks/evidence/TASK-SETTINGS-222A/body-inputs.json'
...
def main():
    ...
    for form in range(1, 5):
        path = source/f'export/pet/PetTurtle{form}.as'
        ...
        for row, count in enumerate(counts):
            assert count == len(holds[row])
            tick, cells = 0, []
            for column, hold in enumerate(holds[row]):
                cells.append(dict(column=column, holdTicks=hold, entryHostTick=tick+1, lastHostTick=tick+hold))
                tick += hold
            rows.append(dict(row=row, cells=cells, totalHostTicks=tick))
        records = []
        ...
        forms.append(dict(form=form, symbol=f'PetTurtleBmd{form}', cellSize=[int(size[1]), int(size[2])],
    ...
    report = dict(taskId='TASK-SETTINGS-222A', status='expected-only', forms=forms,
    ...
    print('222A body input:',len(forms),'forms,',report['cellCount'],'declared cells')

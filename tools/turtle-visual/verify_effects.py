...
def main():
    ...
    for state in measurement['states']:
        ...
        compare(state['tree'], owner['symbols'][state['symbol']], owner, failures, f"{state['symbol']}:{state['tick']}")
        count += 1
    ...
    for name in ['state', 'matrix', 'child-count', 'alpha']:
        ...
        compare(changed['tree'], owner['symbols'][changed['symbol']], owner, detected, name)

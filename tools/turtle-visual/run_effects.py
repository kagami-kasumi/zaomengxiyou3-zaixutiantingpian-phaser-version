"""222A full restored-source native playback; no modern atlas/renderer input."""
import hashlib
import json
import shutil
import subprocess
...
WORK = ROOT / 'local-resources/regima/task-outputs/TASK-SETTINGS-222A/effects-air'
...
def main():
    ...
    effects = [dict(symbol=symbol, owner=Path(source['path']).stem, characterId=cid, sourceIndex=i,
                    scales=[1, 2] if symbol == 'PetTurtle3Bullet3' else [1])
               for source in located['sources'] for i,(symbol,cid) in enumerate((s,c) for s,c in source['symbols'].items() if 'Bmd' not in s)]
    fixtures = dict(taskId='TASK-SETTINGS-222A', status='frozen-before-measurement', sources=sources,
                    effects=effects, ticks=121, stage=[940, 590],
                    scope='Native MovieClip playback after explicit recursive gotoAndStop(1), synchronous baseline then recursive play(). Host AS3 creation/lifetime/follow/combinations require separate fixtures.')
    (WORK / 'fixtures.json').write_text(json.dumps(fixtures, indent=2), encoding='utf-8')
    (OUT / 'effects-fixtures.json').write_text(json.dumps(fixtures, indent=2)+'\n', encoding='utf-8', newline='\n')
...
<id>regima.task222a.visual</id><versionNumber>1.0.0</versionNumber><filename>VisualProbe</filename>
...
<width>940</width><height>590</height><systemChrome>none</systemChrome><renderMode>direct</renderMode></initialWindow></application>''', encoding='utf-8')
    ...
    assert result.returncode == 0 and f'COMPLETE 121 {len(effects)}' in lines, '\n'.join(lines[-15:])
    states = [json.loads(line[6:]) for line in lines if line.startswith('STATE ')]
    assert len(states) == len(effects)*122
    for state in states:
    ...
    report = dict(status='measured-not-promoted', command=command, compileCommand=compile_args,
                  ...
                  fixturesSha256=sha(OUT/'effects-fixtures.json'), states=states)
    (WORK/'measurement.json').write_text(json.dumps(report, indent=2)+'\n', encoding='utf-8')
    print('222A native effect states:',len(states),'; measurement:',WORK/'measurement.json')

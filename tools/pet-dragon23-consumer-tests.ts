...
const rows:any[]=[];
for(const form of [2,3]) for(const entry of ['formal','TestScene']) for(const reason of ['retry','return','reload']) {
  const displays:any[]=[];
  ...
  const rosters=Object.fromEntries(['p1','p2'].map(slot=>{
    ...
    roster.pets.forEach(p=>{p.id=slot+'-'+p.id;p.isActive=p.species==='dragon'&&p.form===form;
      if(p.isActive)Object.assign(p,{hp:500,maxHp:1000,mp:1000,maxMp:1000,atk:100,skills:['fs','sdcc',...(form===3?['ltwj']:[])]});});
    return [slot,roster];
  }));
  ...
  model.members[0]!.combat.combat.state='dead';step();assert.ok(snapshots.p1.runtime);
  {
    ...
    rows.push({form,entry,reason,oldKey,damage:combat.audit.damageEvents.length,
...
writeFileSync('docs/tasks/evidence/TASK-SLICE-214D/consumer-traces.json',JSON.stringify({
...
console.log('Dragon23 production consumers: shared P1/P2 combat, real sandbox HP, rest/replacement and disposal passed.');

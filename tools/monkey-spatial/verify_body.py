...
def main():
    ...
    source_tags=runpy.run_path(str(ROOT/'tools/dragon-collision-swf-fixtures.py'))['source_tags']
    planes={}
    for spec in source['bitmaps']:
        code,data=source_tags(ROOT/spec['sourcePath'])[spec['characterId']]
        if code==35:
            offset=struct.unpack_from('<I',data,2)[0]
            alpha=zlib.decompress(data[6+offset:])
        else:
            assert code==36 and data[2]==5
    ...
    for actual in observed['clocks']:
        ...
        cell=next(c for c in row['sequence'] if c['entryHostTick']<=tick<=c['lastHostTick'])

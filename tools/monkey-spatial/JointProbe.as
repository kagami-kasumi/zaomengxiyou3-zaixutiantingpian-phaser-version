package {
    ...
    public class JointProbe extends Sprite {
        ...
        private function next():void {
            ...
            trace("LOAD "+index);
            loader.contentLoaderInfo.addEventListener(Event.COMPLETE,function(e:Event):void{trace("LOADED "+index);if(index<2)loader.unload();index++;next();});
            ...
            var context:LoaderContext=new LoaderContext(false,index<2?ApplicationDomain.currentDomain:new ApplicationDomain());context.allowCodeImport=true;
            loader.loadBytes(bytes,context);
        }
        private function start():void {
            var atlases:DisplayObjectContainer=loaders[2].content as DisplayObjectContainer;
        ...
        private function addCase(spec:Object,action:String,owner:int,skills:int):void {
            var classes:Array=[Monkey1,Monkey2,Monkey3,Monkey4],klass:Class=classes[spec.form-1],actor:*=new klass(spec);
            ...
            var target:BaseMonster=new BaseMonster();target.x=600;target.y=350;actor.gc.gameSence.addChild(target);actor.gc.pWorld.monsterArray=[target];
            if(skills>=0){actor._petInfo.skills={lyq:Boolean(skills&1),xj:Boolean(skills&2),lj:Boolean(skills&4)};actor.startAoyi();}
            else actor.setAction(action);
            cases.push({id:spec.form+"-"+action+"-P"+owner+"-"+skills,actor:actor,births:[],seen:0});
        }

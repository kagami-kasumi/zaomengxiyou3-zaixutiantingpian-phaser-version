...
def main():
    ...
    for s in data['sources']:
        for bitmap in s['bitmaps'].values():bitmap['absolutePath']=str(ROOT/bitmap['argbPath'])
    ...
    fixture=dict(sources=data['sources'],groups=list(groups.values()),vectorPath=str(BASE/'geometry-air/decoded-vectors.swf'),vectorSwap=False,suppressPending=False)
    fixture_path=WORK/'fixtures.json';fixture_path.write_text(json.dumps(fixture,separators=(',',':')))
    builder=ROOT/'tools/monkey-spatial/GeometryProbe.as';hook=ROOT/'tools/monkey-spatial/CollisionGeometryRun.as.inc'
    ...
    (WORK/'application.xml').write_text('''<application xmlns="http://ns.adobe.com/air/application/51.1"><id>regima.task228.rebuiltcollision</id>
...
<content>RebuiltCollisionProbe.swf</content><visible>false</visible><width>940</width><height>590</height><renderMode>direct</renderMode></initialWindow></application>''')
    ...
    assert len(rows)==349164 and {r['index'] for r in rows}==set(range(349164))
    ...
    print('228 rebuilt collision:',len(rows),'cases;',len(groups),'phase/target groups;',len(failures),'differences')

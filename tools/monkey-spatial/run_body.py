...
def main():
    probe=ROOT/'tools/monkey-spatial/BodyProbe.as'
...
<id>regima.task228.body</id><versionNumber>1.0.0</versionNumber><filename>BodyProbe</filename>
...
<width>940</width><height>590</height><systemChrome>none</systemChrome><renderMode>direct</renderMode></initialWindow></application>''',encoding='utf-8')
    ...
    expected = json.loads((OUT/'body-inputs.json').read_text())['cellCount']*4
    ...
    report=dict(status='measured-not-promoted',cells=cells,clocks=clocks,command=command,compileCommand=args,
                ...
                scope='Native original BBDC bitmap-mode methods; row completion callback resets column for observation. Not original PetMonkey action completion/lifecycle.')
    (WORK/'measurement.json').write_text(json.dumps(report,indent=2)+'\n',encoding='utf-8')
    print('228 body:',len(cells),'P1/P2 cell baselines,',len(clocks),'source-clock steps')

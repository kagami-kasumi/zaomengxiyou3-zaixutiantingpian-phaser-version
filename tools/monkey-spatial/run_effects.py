...
def main():
    ...
    effects = [dict(symbol=symbol, owner=source['id'], characterId=cid, sourceIndex=i, scales=[1])
               ...
               if symbol.startswith('PetMonkey') or symbol == 'FireBuff']
    fixtures = dict(taskId='TASK-SETTINGS-228', status='frozen-before-measurement', sources=sources,
...
<id>regima.task228.visual</id><versionNumber>1.0.0</versionNumber><filename>VisualProbe</filename>
...
<width>940</width><height>590</height><systemChrome>none</systemChrome><renderMode>direct</renderMode></initialWindow></application>''', encoding='utf-8')
    ...
    print('228 native effect states:',len(states),'; measurement:',WORK/'measurement.json')

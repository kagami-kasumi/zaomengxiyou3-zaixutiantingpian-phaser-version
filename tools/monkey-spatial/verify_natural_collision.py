...
def main():
    ...
    mutations = dict(firstFrame=0, direction=0, trackedIdentity=0)
    for row in rows:
        ...
        if row['x'] == 100000 and not row['hit']: mutations['trackedIdentity'] += 1
    assert all(mutations.values()), mutations
    native = json.loads((BASE/'color-bounds-air/verification.json').read_text())
    ...
    report = dict(status='passed-bounded', cases=len(rows), hits=sum(r['hit'] for r in rows),
                  mismatchCount=0, rejectedMutationCases=mutations,
                  nativeColorBounds=native, measurementSha256=hashlib.sha256(path.read_bytes()).hexdigest(),
                  scope='Native raster oracle and independent color reduction, limited sampled coordinates/phases; not an independent decoded geometry renderer or modern consumer.')
    (OUT/'natural-collision-verification.json').write_text(json.dumps(report,indent=2)+'\n')
    print(json.dumps({k:report[k] for k in ['status','cases','hits','mismatchCount','rejectedMutationCases']}))

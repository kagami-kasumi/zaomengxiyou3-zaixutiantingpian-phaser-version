...
def main():
    ...
    for form in range(1,5):
        path=f'export/pet/PetHorse{form}.as'
        names=['setAction','enterFrameFunc','scriptFrameOverFunc','normalHit']
        if form==4:names += ['releSkill4']
        ...
        prefix='package {import flash.geom.*; public class Horse'+str(form)+' extends CallbackBase {'
        fields='private var skill1Release:Boolean=false;'
        constructor='public function Horse'+str(form)+'(spec:Object){super(spec);bbdc.setEnterFrameCallBack(enterFrameFunc,exitFrameFunc);bbdc.setAddScriptWhenFrameOver(scriptFrameOverFunc);}'
        wrapper='public function startAoyi():void{'+('releSkill4();' if form==4 else '')+'}'
        write(f'Horse{form}.as',prefix+fields+constructor+'\n'.join(methods)+sinks+wrapper+'}}')
...
public function addAoyiBuff():void{events.push({kind:"aoyiBuff"});}public function setYourFather(n:*):void{events.push({kind:"protect",ticks:n});}
...
public function sendPetAction(...args):void{events.push({kind:"sendAction",args:args});}}}''')
    report=dict(status='prepared-not-promoted',methods=records,bodyClipSha256=hashlib.sha256(clip.read_bytes()).hexdigest(),
                instrumentation='Only ambient Math.random replaced with configured sequence; original callback/action methods retained. Bullet creation, network, protection, destruction and MP service are explicit observation sinks. Blank body bitmap has source cell dimensions and source BBDC methods; actual pixels verified separately. Target and buff event sinks do not prove collision or native attachment; separate horse integration fixture required.')
    (OUT/'callback-methods.json').write_text(json.dumps(report,indent=2)+'\n',encoding='utf-8')
    print('229 callback source methods:',len(records))

...
WORK = ROOT/'local-resources/regima/task-outputs/TASK-SETTINGS-229/natural-collision-air'
...
def main():
    ...
    sources = [dict(id=s['id'], path=str(ROOT/s['path']), sha256=s['sha256']) for s in extracted['sources']]
    ...
    effects = [o for o in objects if o['symbol'].startswith('PetHorse') and o['symbol']!='PetHorseIceEffect']
    ...
    (OUT/'natural-collision-fixtures.json').write_text(json.dumps(fixture,indent=2)+'\n',encoding='utf-8')
    ...
    assert sha(WORK/'my/HitTest.as') == sha(hit)
    probe = ROOT/'tools/monkey-spatial/NaturalCollisionProbe.as'
...
<id>regima.task229.naturalcollision</id><versionNumber>1.0.0</versionNumber><filename>NaturalCollisionProbe</filename>
<supportedProfiles>desktop</supportedProfiles><initialWindow><content>NaturalCollisionProbe.swf</content><visible>false</visible>
<width>940</width><height>590</height><systemChrome>none</systemChrome><renderMode>direct</renderMode></initialWindow></application>''',encoding='utf-8')
    compile_args = ['java','-Dflexlib='+str(SDK/'frameworks'),'-jar',str(SDK/'lib/mxmlc-cli.jar'),'+configname=air',
                    '-debug=true','-default-frame-rate=24','-default-size=940,590','-output=NaturalCollisionProbe.swf','NaturalCollisionProbe.as']
    ...
    expected = len(effects)*2*len(targets)*(len(positions)+24)*122
    ...
    report = dict(status='measured-not-promoted',cases=cases,phases=[json.loads(line[6:]) for line in lines if line.startswith('PHASE ')],command=command,compileCommand=compile_args,
                  environment=[json.loads(line[4:]) for line in lines if line.startswith('ENV ')],
                  referenceScope='Independent exact-cyan pixel reduction of the same native sequential DIFFERENCE raster blend; not an independent geometry reconstruction.',
                  hitTestPath=hit.relative_to(ROOT).as_posix(),hitTestSha256=sha(hit),probeSha256=sha(probe),
                  fixturesSha256=sha(OUT/'natural-collision-fixtures.json'),runtimeDllSha256=sha(runtime/'Adobe AIR/Versions/1.0/Adobe AIR.dll'))
    (WORK/'measurement.json').write_text(json.dumps(report,separators=(',',':'))+'\n',encoding='utf-8')
    print('Independent mask differences:',sum(c['hit']!=c['reference'] for c in cases))
    print('229 natural collision cases:',len(cases),'; hits:',sum(c['hit'] for c in cases))

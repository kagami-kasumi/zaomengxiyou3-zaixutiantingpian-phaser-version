...
WORK=ROOT/'local-resources/regima/task-outputs/TASK-SETTINGS-229/joint-air'
...
def main():
    probe=ROOT/'tools/horse-spatial/JointProbe.as'
...
<id>regima.task229.joint</id><versionNumber>1.0.0</versionNumber><filename>JointProbe</filename>
...
<width>940</width><height>590</height><systemChrome>none</systemChrome><renderMode>direct</renderMode></initialWindow></application>''',encoding='utf-8')
    ...
    report=dict(status='measured-not-promoted',fixtures=fixture,rows=rows,command=command,compileCommand=args,
                ...
                probeSha256=sha(probe),compiledSha256=sha(WORK/"JointProbe.swf"),methodsSha256=sha(OUT/'joint-methods.json'),
    ...
    print('229 native joint:',fps,'fps;',len(rows),'rows')

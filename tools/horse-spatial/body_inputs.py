...
OUT = ROOT/'docs/tasks/evidence/TASK-SETTINGS-229/body-inputs.json'
...
def main():
    ...
    for form in range(1, 5):
        path = source/f'export/pet/PetHorse{form}.as'
        ...
        forms.append(dict(form=form, symbol=f'PetHorseBmd{form}', cellSize=[int(size[1]), int(size[2])],
    ...
    report = dict(taskId='TASK-SETTINGS-229', status='expected-only', forms=forms,
    ...
    print('229 body input:',len(forms),'forms,',report['cellCount'],'declared cells')

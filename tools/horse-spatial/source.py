"""229: byte-preserved horse effect/target dependency closures.
...
LOCAL = ROOT/'local-resources/regima/task-outputs/TASK-SETTINGS-229/source'
OUT = ROOT/'docs/tasks/evidence/TASK-SETTINGS-229'
EFFECTS = ['PetHorse1Bullet1','PetHorse1Bullet2','PetHorse2Bullet1','PetHorse2Bullet2',
...
SOURCE_ROOTS = [('20120203', EFFECTS),
                ('StageCommon', TARGETS+['ObjectBaseSprite3','ObjectBaseSprite4','PetHorseIceEffect','AoyiBuff']),
                ('pet1',['PetHorse4Bullet5','PetHorse4Bullet5Explode'])]
...
def main():
    ...
    for name, names in SOURCE_ROOTS:
    ...
    result=dict(taskId='TASK-SETTINGS-229',status='extracted-not-promoted',sources=sources,scripts=scripts,
                unresolved=unresolved,scope='Ten primary horse effects, three formal target shapes, two pet colliders, ice and AoyiBuff; source extraction only, no host lifecycle or collision claim')
    (OUT/'source-definitions.json').write_text(json.dumps(result,indent=2)+'\n',encoding='utf-8')
    print('229 closures:',[(s['id'],len(s['definitions'])) for s in sources],'; script count:',len(scripts),'; unresolved scripts:',len(unresolved))

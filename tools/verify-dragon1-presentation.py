...
for state in data['projections']:
    ...
    if WRITE_IMAGES:
        path = ROOT / '.tmp/verification-images' / OUT.name / 'projections' / (state['id'] + '.png')
...
for key, state_id in [('geometry','dragon1.wait.left'),('alpha','dragon1.fs-clone-active.right'),
    ...
    kills.append(key)
if LATER:
    state = copy.deepcopy(next(s for s in data['projections'] if s['id'] == 'dragon3-ltwj.nine-object-wave.left'))
    state['layers'].reverse()
    assert render.different(render.render(state), Image.open(ROOT/state['baseline']).convert('RGBA'))
    kills.append('wave-insertion-order')
if WRITE_IMAGES: sheet.save(ROOT/'.tmp/verification-images'/OUT.name/'contact-sheet.png')
...
print(f'{len(results)} production presenter states match original; {len(kills)} visual mutations rejected')
